...
class MpcController : public mrs_uav_managers::Controller {
 ...
 private:
  ...
  void PIDVelocityOutput(const mrs_msgs::msg::UavState& uav_state,
                         const mrs_msgs::msg::TrackerCommand& tracker_command,
                         const common::CONTROL_OUTPUT& control_output,
                         const double& dt);

  void MPC(const mrs_msgs::msg::UavState& uav_state,
           const mrs_msgs::msg::TrackerCommand& tracker_command,
  ...
  double calculateGainChange(const double dt, const double current_value,
                             const double desired_value, const bool bypass_rate,
                             std::string name, bool& updated);

  double getHeadingSafely(const mrs_msgs::msg::UavState& uav_state,
                          const mrs_msgs::msg::TrackerCommand& tracker_command);
...
bool MpcController::initialize(
  ...
  private_handlers->parent_param_loader->loadParamReusable("enable_profiler",
                                                           _profiler_enabled_);

  if (!private_handlers->parent_param_loader->loadedSuccessfully()) {
    RCLCPP_ERROR(node_->get_logger(), "[%s]: Could not load all parameters!",
                 name_.c_str());
    return false;
  }

  dynparam_mgr_ =
      std::make_shared<mrs_lib::DynparamMgr>(node_, mutex_drs_params_);

  // | -------------------- loading my params ------------------- |

  private_handlers->param_loader->addYamlFile(
      ament_index_cpp::get_package_share_directory("mrs_uav_controllers") +
  ...
  private_handlers->param_loader->addYamlFile(
      ament_index_cpp::get_package_share_directory("mrs_uav_controllers") +
      "/config/private/" + private_handlers->name_space + ".yaml");
  private_handlers->param_loader->addYamlFile(
      ament_index_cpp::get_package_share_directory("mrs_uav_controllers") +
      "/config/public/" + private_handlers->name_space + ".yaml");

  dynparam_mgr_->get_param_provider().copyYamls(
      private_handlers->param_loader->getParamProvider());

  // load the dynamicall model parameters
  private_handlers->param_loader->loadParam("mpc/mpc_model/number_of_states",
                                            _n_states_);
  ...
  private_handlers->param_loader->loadParam("mpc/mpc_parameters/horizon_length",
                                            _horizon_length_);

  private_handlers->param_loader->loadParam(
      "mpc/mpc_parameters/horizontal/max_speed", _max_speed_horizontal_);
  private_handlers->param_loader->loadParam(
      "mpc/mpc_parameters/horizontal/max_acceleration",
      _max_acceleration_horizontal_);
  private_handlers->param_loader->loadParam(
      "mpc/mpc_parameters/horizontal/max_jerk", _max_jerk_);

  private_handlers->param_loader->loadParam("mpc/mpc_parameters/horizontal/Q",
                                            _mat_Q_);
  private_handlers->param_loader->loadParam("mpc/mpc_parameters/horizontal/S",
                                            _mat_S_);

  private_handlers->param_loader->loadParam(
      "mpc/mpc_parameters/vertical/max_speed", _max_speed_vertical_);
  private_handlers->param_loader->loadParam(
      "mpc/mpc_parameters/vertical/max_acceleration",
      _max_acceleration_vertical_);
  private_handlers->param_loader->loadParam("mpc/mpc_parameters/vertical/max_u",
                                            _max_u_vertical_);

  private_handlers->param_loader->loadParam("mpc/mpc_parameters/vertical/Q",
                                            _mat_Q_z_);
  private_handlers->param_loader->loadParam("mpc/mpc_parameters/vertical/S",
                                            _mat_S_z_);

  private_handlers->param_loader->loadParam("mpc/solver/verbose",
                                            _mpc_solver_verbose_);
  private_handlers->param_loader->loadParam("mpc/solver/max_iterations",
                                            _mpc_solver_max_iterations_);

  // | ------------------------- rampup ------------------------- |

  private_handlers->param_loader->loadParam("so3/rampup/enabled",
                                            _rampup_enabled_);
  private_handlers->param_loader->loadParam("so3/rampup/speed", _rampup_speed_);

  // | --------------------- integral gains --------------------- |

  private_handlers->param_loader->loadParam("so3/gains/integral_gains/kiw",
                                            gains_.kiwxy);
  private_handlers->param_loader->loadParam("so3/gains/integral_gains/kib",
                                            gains_.kibxy);

  // integrator limits
  private_handlers->param_loader->loadParam("so3/gains/integral_gains/kiw_lim",
                                            gains_.kiwxy_lim);
  private_handlers->param_loader->loadParam("so3/gains/integral_gains/kib_lim",
                                            gains_.kibxy_lim);

  // | ------------- height and attitude controller ------------- |

  // attitude gains
  private_handlers->param_loader->loadParam("so3/gains/attitude/roll_pitch",
                                            gains_.kq_roll_pitch);
  private_handlers->param_loader->loadParam("so3/gains/attitude/yaw",
                                            gains_.kq_yaw);

  // attitude rate gains
  private_handlers->param_loader->loadParam(
      "so3/gains/attitude_rate/roll_pitch", gains_.kw_rp);
  private_handlers->param_loader->loadParam("so3/gains/attitude_rate/yaw",
                                            gains_.kw_y);

  // mass estimator
  private_handlers->param_loader->loadParam("so3/mass_estimator/km", gains_.km);
  dynparam_mgr_->register_param("so3/mass_estimator/fuse_acceleration",
                                &drs_params_.fuse_acceleration);
  private_handlers->param_loader->loadParam("so3/mass_estimator/km_lim",
                                            gains_.km_lim);

  // constraints
  private_handlers->param_loader->loadParam(
      "so3/constraints/tilt_angle_failsafe/enabled",
      _tilt_angle_failsafe_enabled_);
  private_handlers->param_loader->loadParam(
      "so3/constraints/tilt_angle_failsafe/limit", _tilt_angle_failsafe_);
  ...
  if (_tilt_angle_failsafe_enabled_ && std::abs(_tilt_angle_failsafe_) < 1e-3) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[%s]: constraints/tilt_angle_failsafe/enabled = 'TRUE' but "
  ...
  private_handlers->param_loader->loadParam(
      "so3/constraints/throttle_saturation", _throttle_saturation_);

  // gain filtering
  private_handlers->param_loader->loadParam(
      "so3/gain_filtering/perc_change_rate", _gains_filter_change_rate_);
  private_handlers->param_loader->loadParam(
      "so3/gain_filtering/min_change_rate", _gains_filter_min_change_rate_);
  private_handlers->param_loader->loadParam("so3/gain_filtering/rate",
                                            _gain_filtering_rate_);
  private_handlers->param_loader->loadParam(
      "so3/gain_filtering/gain_mute_coefficient", _gain_mute_coefficient_);

  // angular rate feed forward
  dynparam_mgr_->register_param("so3/angular_rate_feedforward/jerk",
                                &drs_params_.jerk_feedforward);

  // output mode
  dynparam_mgr_->register_param("so3/preferred_output",
                                &drs_params_.preferred_output_mode,
                                mrs_lib::DynparamMgr::range_t<int>(0, 3));

  // | ------------------- position pid params ------------------ |

  private_handlers->param_loader->loadParam(
      "position_controller/translation_gains/p", _pos_pid_p_);
  private_handlers->param_loader->loadParam(
      "position_controller/translation_gains/i", _pos_pid_i_);
  private_handlers->param_loader->loadParam(
      "position_controller/translation_gains/d", _pos_pid_d_);

  private_handlers->param_loader->loadParam(
      "position_controller/heading_gains/p", _hdg_pid_p_);
  private_handlers->param_loader->loadParam(
      "position_controller/heading_gains/i", _hdg_pid_i_);
  private_handlers->param_loader->loadParam(
      "position_controller/heading_gains/d", _hdg_pid_d_);

  // | ------------------ finish loading params ----------------- |

  if (!private_handlers->param_loader->loadedSuccessfully() ||
      !dynparam_mgr_->loaded_successfully()) {
    RCLCPP_ERROR(node_->get_logger(), "[%s]: Could not load all parameters!",
                 this->name_.c_str());
    return false;
  }
  ...
  sh_imu_ = mrs_lib::SubscriberHandler<sensor_msgs::msg::Imu>(
      shopts, "/" + common_handlers->uav_name + "/hw_api/imu");
  ...
  mpc_solver_x_ = std::make_shared<mrs_mpc_solvers::mpc_controller::Solver>(
      name_, _mpc_solver_verbose_, _mpc_solver_max_iterations_, _mat_Q_,
      _mat_S_, _dt1_, _dt2_, 0, 1.0);
  mpc_solver_y_ = std::make_shared<mrs_mpc_solvers::mpc_controller::Solver>(
      name_, _mpc_solver_verbose_, _mpc_solver_max_iterations_, _mat_Q_,
      _mat_S_, _dt1_, _dt2_, 0, 1.0);
  mpc_solver_z_ = std::make_shared<mrs_mpc_solvers::mpc_controller::Solver>(
      name_, _mpc_solver_verbose_, _mpc_solver_max_iterations_, _mat_Q_z_,
      _mat_S_z_, _dt1_, _dt2_, 0.5, 0.5);

  // | --------------- declare dynamic parameters --------------- |

  dynparam_mgr_->register_param(
      "horizontal.kiwxy", &drs_params_.kiwxy, gains_.kiwxy,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 10.0));
  dynparam_mgr_->register_param(
      "horizontal.kibxy", &drs_params_.kibxy, gains_.kibxy,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 10.0));
  dynparam_mgr_->register_param(
      "horizontal.kiwxy_lim", &drs_params_.kiwxy_lim, gains_.kiwxy_lim,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 10.0));
  dynparam_mgr_->register_param(
      "horizontal.kibxy_lim", &drs_params_.kibxy_lim, gains_.kibxy_lim,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 10.0));

  dynparam_mgr_->register_param(
      "attitude.kq_roll_pitch", &drs_params_.kq_roll_pitch,
      gains_.kq_roll_pitch, mrs_lib::DynparamMgr::range_t<double>(0.0, 20.0));

  dynparam_mgr_->register_param(
      "attitude.kq_yaw", &drs_params_.kq_yaw, gains_.kq_yaw,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 40.0));

  dynparam_mgr_->register_param(
      "mass.km", &drs_params_.km, gains_.km,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 2.0));

  dynparam_mgr_->register_param(
      "mass.km_lim", &drs_params_.km_lim, gains_.km_lim,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 50.0));
  ...
  ss_set_integral_terms_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
          node_, "~/set_integral_terms",
          std::bind(&MpcController::callbackSetIntegralTerms, this,
...

  {
    std::function<void()> callback_fcn =
        std::bind(&MpcController::timerGains, this);

    timer_gains_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_gain_filtering_rate_), callback_fcn);
  }
  ...
  position_pid_heading_.setParams(_hdg_pid_p_, _hdg_pid_d_, _hdg_pid_i_, -1,
                                  0.1);

  // | ------------------------ profiler ------------------------ |

  profiler_ = mrs_lib::Profiler(common_handlers->parent_node, "MpcController",
                                _profiler_enabled_);
...
void MpcController::destroy() { timer_gains_->stop(); }
...
bool MpcController::activate(const ControlOutput& last_control_output) {
  ...
  if (activation_control_output_.diagnostics.mass_estimator) {
    uav_mass_difference_ =
        activation_control_output_.diagnostics.mass_difference;
    activation_mass += uav_mass_difference_;
    RCLCPP_INFO(
        node_->get_logger(),
        "[%s]: setting mass difference from the last control output: %.2f kg",
        this->name_.c_str(), uav_mass_difference_);
  }
  ...
  if (activation_control_output_.diagnostics.disturbance_estimator) {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[%s]: setting disturbances from the last control output: "
                "Ib_b_: %.2f, %.2f N, Iw_w_: "
                "%.2f, %.2f N",
  ...
  auto throttle_last_controller =
      common::extractThrottle(activation_control_output_);

  // rampup check
  if (_rampup_enabled_ && throttle_last_controller) {
    double hover_throttle = mrs_lib::quadratic_throttle_model::forceToThrottle(
        common_handlers_->throttle_model,
        activation_mass * common_handlers_->g);

    double throttle_difference =
        hover_throttle - throttle_last_controller.value();
    ...
    RCLCPP_INFO(
        node_->get_logger(),
        "[%s]: activating rampup with initial throttle: %.4f, target: %.4f",
        name_.c_str(), throttle_last_controller.value(), hover_throttle);
  }
...
void MpcController::updateInactive(
    const mrs_msgs::msg::UavState& uav_state,
    [[maybe_unused]] const std::optional<mrs_msgs::msg::TrackerCommand>&
        tracker_command) {
...
MpcController::ControlOutput MpcController::updateActive(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command) {
  mrs_lib::Routine profiler_routine = profiler_.createRoutine("updateActive");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "MpcController::updateActive",
                          common_handlers_->scope_timer.logger,
                          common_handlers_->scope_timer.enabled);
  ...
  if (first_iteration_) {
    ...
    dt = rclcpp::Time(uav_state.header.stamp).seconds() -
         last_update_time_.seconds();
  }
  ...
  if (std::abs(dt) < 0.001) {
    RCLCPP_DEBUG(node_->get_logger(),
                 "[%s]: the last odometry message came too close (%.2f s)!",
                 name_.c_str(), dt);
  ...
  auto lowest_modality =
      common::getLowestOuput(common_handlers_->control_output_modalities);

  if (!lowest_modality) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[%s]: output modalities are empty! This error should never appear.",
        name_.c_str());
  ...
  if (drs_params.preferred_output_mode == OUTPUT_ATTITUDE_RATE &&
      common_handlers_->control_output_modalities.attitude_rate) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: prioritizing attitude rate output",
                          name_.c_str());
    lowest_modality = common::ATTITUDE_RATE;
  } else if (drs_params.preferred_output_mode == OUTPUT_ATTITUDE &&
             common_handlers_->control_output_modalities.attitude) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: prioritizing attitude output", name_.c_str());
    lowest_modality = common::ATTITUDE;
  } else if (drs_params.preferred_output_mode == OUTPUT_CONTROL_GROUP &&
             common_handlers_->control_output_modalities.control_group) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: prioritizing control group output",
                          name_.c_str());
    lowest_modality = common::CONTROL_GROUP;
  } else if (drs_params.preferred_output_mode == OUTPUT_ACTUATORS &&
             common_handlers_->control_output_modalities.actuators) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
  ...
  switch (lowest_modality.value()) {
    ...
    case common::VELOCITY_HDG_RATE: {
      PIDVelocityOutput(uav_state, tracker_command, common::VELOCITY_HDG_RATE,
                        dt);
      break;
    }
...
void MpcController::switchOdometrySource(
    const mrs_msgs::msg::UavState& new_uav_state) {
  RCLCPP_INFO(node_->get_logger(), "[%s]: switching the odometry source",
              this->name_.c_str());
  ...
  auto res = common_handlers_->transformer->transformSingle(
      world_integrals, new_uav_state.header.frame_id);
  ...
  if (res) {
    ...
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[%s]: could not transform world integral to the new frame",
        this->name_.c_str());
...
const std::shared_ptr<mrs_msgs::srv::DynamicsConstraintsSrv::Response>
MpcController::setConstraints(
    [[maybe_unused]] const std::shared_ptr<
        mrs_msgs::srv::DynamicsConstraintsSrv::Request>& constraints) {
  std::shared_ptr<mrs_msgs::srv::DynamicsConstraintsSrv::Response> response =
      std::make_shared<mrs_msgs::srv::DynamicsConstraintsSrv::Response>();
  ...
  mrs_lib::set_mutexed(mutex_constraints_, constraints->constraints,
                       constraints_);

  RCLCPP_INFO(node_->get_logger(), "[%s]: updating constraints",
              this->name_.c_str());
...
void MpcController::MPC(const mrs_msgs::msg::UavState& uav_state,
                        const mrs_msgs::msg::TrackerCommand& tracker_command,
                        const double& dt,
  ...
  if (tracker_command.use_full_state_prediction) {
    ...
    max_speed_vertical = 1.5 * (constraints.vertical_ascending_speed <
    ...
    max_acceleration_vertical =
        1.5 * (constraints.vertical_ascending_acceleration <
    ...
    max_u_vertical = 1.5 * (constraints.vertical_ascending_jerk <
                                    constraints.vertical_descending_jerk
                                ? constraints.vertical_ascending_jerk
                                : constraints.vertical_descending_jerk);
  }
  ...
  Rp << tracker_command.position.x, tracker_command.position.y,
      tracker_command.position.z;  // fill the desired position
  Rv << tracker_command.velocity.x, tracker_command.velocity.y,
  ...
  Eigen::Vector3d Op(uav_state.pose.position.x, uav_state.pose.position.y,
                     uav_state.pose.position.z);
  Eigen::Vector3d Ov(uav_state.velocity.linear.x, uav_state.velocity.linear.y,
  ...
  // Ow - UAV angular rate
  Eigen::Vector3d Ow(uav_state.velocity.angular.x, uav_state.velocity.angular.y,
...

  {
    ...
    if (std::abs(uav_state.acceleration.linear.x) <
        coef * max_acceleration_horizontal) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: odometry x acceleration exceeds constraints (%.2f > %.1f * "
          "%.2f m), using reference for initial condition",
          name_.c_str(), std::abs(uav_state.acceleration.linear.x), coef,
          max_acceleration_horizontal);
    }
    ...
    if (std::abs(uav_state.velocity.linear.x) < coef * max_speed_horizontal) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: odometry x velocity exceeds constraints (%.2f > %0.1f * %.2f "
          "m), using reference for initial condition",
          name_.c_str(), std::abs(uav_state.velocity.linear.x), coef,
...

  {
    ...
    if (std::abs(uav_state.acceleration.linear.y) <
        coef * max_acceleration_horizontal) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: odometry y acceleration exceeds constraints (%.2f > %.1f * "
          "%.2f m), using reference for initial condition",
          name_.c_str(), std::abs(uav_state.acceleration.linear.y), coef,
          max_acceleration_horizontal);
    }
    ...
    if (std::abs(uav_state.velocity.linear.y) < coef * max_speed_horizontal) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: odometry y velocity exceeds constraints (%.2f > %0.1f * %.2f "
          "m), using reference for initial condition",
          name_.c_str(), std::abs(uav_state.velocity.linear.y), coef,
          max_speed_horizontal);
    }
...

  {
    ...
    if (std::abs(uav_state.acceleration.linear.z) <
        coef * max_acceleration_horizontal) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: odometry z acceleration exceeds constraints (%.2f > %.1f * "
          "%.2f m), using reference for initial condition",
          name_.c_str(), std::abs(uav_state.acceleration.linear.z), coef,
          max_acceleration_horizontal);
    }
    ...
    if (std::abs(uav_state.velocity.linear.z) < coef * max_speed_vertical) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: odometry z velocity exceeds constraints (%.2f > %0.1f * %.2f "
          "m), using reference for initial condition",
          name_.c_str(), std::abs(uav_state.velocity.linear.z), coef,
          max_speed_vertical);
    }
  ...
  Eigen::MatrixXd mpc_reference_x =
      Eigen::MatrixXd::Zero(_horizon_length_ * _n_states_, 1);
  Eigen::MatrixXd mpc_reference_y =
      Eigen::MatrixXd::Zero(_horizon_length_ * _n_states_, 1);
  Eigen::MatrixXd mpc_reference_z =
      Eigen::MatrixXd::Zero(_horizon_length_ * _n_states_, 1);
  ...
  if (tracker_command.use_full_state_prediction) {
    ...
    // TODO !! this is a very crude way of sampling from the desired full-state
    // prediction, which only works with the MpcTracker. Rework this please.

    for (int i = 1; i < _horizon_length_; i++) {
      mpc_reference_x((i * _n_states_) + 0, 0) =
          tracker_command.full_state_prediction.position.at(i).x;
      mpc_reference_y((i * _n_states_) + 0, 0) =
          tracker_command.full_state_prediction.position.at(i).y;
      mpc_reference_z((i * _n_states_) + 0, 0) =
          tracker_command.full_state_prediction.position.at(i).z;
    }

    for (int i = 1; i < _horizon_length_; i++) {
      mpc_reference_x((i * _n_states_) + 1, 0) =
          tracker_command.full_state_prediction.velocity.at(i).x;
      mpc_reference_y((i * _n_states_) + 1, 0) =
          tracker_command.full_state_prediction.velocity.at(i).y;
      mpc_reference_z((i * _n_states_) + 1, 0) =
          tracker_command.full_state_prediction.velocity.at(i).z;
    }

    for (int i = 1; i < _horizon_length_; i++) {
      mpc_reference_x((i * _n_states_) + 2, 0) =
          tracker_command.full_state_prediction.acceleration.at(i).x;
      mpc_reference_y((i * _n_states_) + 2, 0) =
          tracker_command.full_state_prediction.acceleration.at(i).y;
      mpc_reference_z((i * _n_states_) + 2, 0) =
          tracker_command.full_state_prediction.acceleration.at(i).z;
    }
  ...
  mpc_solver_x_->setLimits(max_speed_horizontal, 999,
                           max_acceleration_horizontal, max_jerk, _dt1_, _dt2_);
  ...
  mpc_solver_y_->setLimits(max_speed_horizontal, 999,
                           max_acceleration_horizontal, max_jerk, _dt1_, _dt2_);
  ...
  mpc_solver_z_->setLimits(max_speed_vertical, max_acceleration_vertical,
                           max_u_vertical, 999.0, _dt1_, _dt2_);
...

  {
    ...
    auto res = common_handlers_->transformer->transformSingle(
        Ib_b_stamped, uav_state_.header.frame_id);
    ...
    if (res) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: could not transform the Ib_b_ to the world frame",
          name_.c_str());
    }
  }
  ...
  if (tracker_command.use_acceleration &&
      !tracker_command.use_full_state_prediction) {
    Ra << tracker_command.acceleration.x + mpc_solver_x_u_,
        tracker_command.acceleration.y + mpc_solver_y_u_,
  ...
  Eigen::Vector3d feed_forward =
      total_mass * (Eigen::Vector3d(0, 0, common_handlers_->g) + Ra);
...

  {
    ...
    if (!tracker_command.disable_antiwindups) {
      if (rampup_active_ || sqrt(pow(uav_state.velocity.linear.x, 2) +
                                 pow(uav_state.velocity.linear.y, 2)) > 0.3) {
        temp_gain = 0;
        RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "[%s]: anti-windup for world integral kicks in",
                              this->name_.c_str());
      }
    }
    ...
    if (!std::isfinite(Iw_w_(0))) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: NaN detected in variable 'Iw_w_(0)', setting it to 0!!!",
          this->name_.c_str());
    ...
    if (gains.kiwxy_lim >= 0 && world_integral_saturated) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[%s]: MPC's world X integral is being saturated!",
                           this->name_.c_str());
    }
    ...
    if (!std::isfinite(Iw_w_(1))) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: NaN detected in variable 'Iw_w_(1)', setting it to 0!!!",
          this->name_.c_str());
    ...
    if (gains.kiwxy_lim >= 0 && world_integral_saturated) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[%s]: MPC's world Y integral is being saturated!",
                           this->name_.c_str());
    }
  }
...

  {
    ...
    Eigen::Vector2d Ep_fcu_untilted = Eigen::Vector2d(
        0, 0);  // position error in the untilted frame of the UAV
    Eigen::Vector2d Ev_fcu_untilted = Eigen::Vector2d(
        0, 0);  // velocity error in the untilted frame of the UAV
    ...
    // get the position control error in the fcu_untilted frame
    {
      ...
      auto res = common_handlers_->transformer->transformSingle(Ep_stamped,
                                                                "fcu_untilted");
      ...
      if (res) {
        ...
        RCLCPP_ERROR_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "[%s]: could not transform the position error to fcu_untilted",
            name_.c_str());
      }
    }
    ...
    // get the velocity control error in the fcu_untilted frame
    {
      ...
      auto res = common_handlers_->transformer->transformSingle(Ev_stamped,
                                                                "fcu_untilted");
      ...
      if (res) {
        ...
        RCLCPP_ERROR_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "[%s]: could not transform the velocity error to fcu_untilted",
            name_.c_str());
      }
    }
    ...
    if (!tracker_command.disable_antiwindups) {
      if (rampup_active_ || sqrt(pow(uav_state.velocity.linear.x, 2) +
        ...
        temp_gain = 0;
        RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "[%s]: anti-windup for body integral kicks in",
                              this->name_.c_str());
      }
    }
    ...
    if (!std::isfinite(Ib_b_(0))) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: NaN detected in variable 'Ib_b_(0)', setting it to 0!!!",
          this->name_.c_str());
    ...
    if (gains.kibxy_lim > 0 && body_integral_saturated) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: MPC's body pitch integral is being saturated!",
          this->name_.c_str());
    }
    ...
    if (!std::isfinite(Ib_b_(1))) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: NaN detected in variable 'Ib_b_(1)', setting it to 0!!!",
          this->name_.c_str());
    ...
    if (gains.kibxy_lim > 0 && body_integral_saturated) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[%s]: MPC's body roll integral is being saturated!",
                           this->name_.c_str());
    }
  }
  ...
  if (output_modality == common::ACCELERATION_HDG ||
      output_modality == common::ACCELERATION_HDG_RATE) {
    if (output_modality == common::ACCELERATION_HDG) {
      ...
      double hdg_err = mrs_lib::geometry::sradians::diff(
          tracker_command.heading, uav_heading);

      double des_hdg_rate =
          position_pid_heading_.update(hdg_err, dt) + des_hdg_ff;
...

    {
      ...
      auto res =
          common_handlers_->transformer->transformSingle(world_accel, "fcu");

      if (res) {
        unbiased_des_acc << res.value().vector.x, res.value().vector.y,
            res.value().vector.z;
      }
    }
    ...
    last_control_output_.diagnostics.controller_enforcing_constraints =
        !tracker_command.use_full_state_prediction;

    last_control_output_.diagnostics.horizontal_speed_constraint =
        0.5 * max_speed_horizontal;
    last_control_output_.diagnostics.horizontal_acc_constraint =
        0.5 * max_acceleration_horizontal;

    last_control_output_.diagnostics.vertical_asc_speed_constraint =
        0.5 * max_speed_vertical;
    last_control_output_.diagnostics.vertical_asc_acc_constraint =
        0.5 * max_acceleration_vertical;

    last_control_output_.diagnostics.vertical_desc_speed_constraint =
        0.5 * max_speed_vertical;
    last_control_output_.diagnostics.vertical_desc_acc_constraint =
        0.5 * max_acceleration_vertical;
...

  {
    ...
    if (tracker_command.use_acceleration && sh_imu_.hasMsg() &&
        drs_params.fuse_acceleration) {
      auto imu = sh_imu_.getMsg();
      const double measured_bodyz_acc = imu->linear_acceleration.z;
      const double desired_bodyz_acc =
          mrs_lib::quadratic_throttle_model::throttleToForce(
              common_handlers_->throttle_model, last_throttle_) /
          total_mass;

      if (last_throttle_ < (_throttle_saturation_ - 0.01) &&
          last_throttle_ > 0) {
        uav_mass_difference_ +=
            1.0 * gains.km * (desired_bodyz_acc - measured_bodyz_acc) * dt;
      }
      ...
      if (rampup_active_ ||
          (std::abs(uav_state.velocity.linear.z) > 0.3 &&
           ((Ep(2) > 0 && uav_state.velocity.linear.z > 0) ||
            (Ep(2) < 0 && uav_state.velocity.linear.z < 0)))) {
        temp_gain = 0;
        RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "[%s]: anti-windup for the mass kicks in",
                              this->name_.c_str());
      }
    ...
    if (!std::isfinite(uav_mass_difference_)) {
      ...
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: NaN detected in variable 'uav_mass_difference_', setting it "
          "to 0 and returning!!!",
          this->name_.c_str());
    ...
    if (uav_mass_saturated) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[%s]: The UAV mass difference is being saturated to %.2f!",
          this->name_.c_str(), uav_mass_difference_);
    }
  }
  ...
  // the downwards force produced by the position and the acceleration feedback
  // should not be larger than the gravity

  // if the downwards part of the force is close to counter-act the gravity
  // acceleration
  if (f(2) < 0) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "[%s]: the calculated downwards desired force is "
                         "negative (%.2f) -> mitigating flip",
  ...
  double tilt_safety_limit = _tilt_angle_failsafe_enabled_
                                 ? _tilt_angle_failsafe_
                                 : std::numeric_limits<double>::max();

  auto f_normed_sanitized =
      common::sanitizeDesiredForce(node_, f.normalized(), tilt_safety_limit,
                                   constraints.tilt, "MpcController");

  if (!f_normed_sanitized) {
    RCLCPP_INFO(node_->get_logger(), "[%s]: f = [%.2f, %.2f, %.2f]",
                this->name_.c_str(), f(0), f(1), f(2));
    RCLCPP_INFO(node_->get_logger(),
                "[%s]: integral feedback: [%.2f, %.2f, %.2f]",
                this->name_.c_str(), integral_feedback(0), integral_feedback(1),
                integral_feedback(2));
    RCLCPP_INFO(node_->get_logger(), "[%s]: feed forward: [%.2f, %.2f, %.2f]",
                this->name_.c_str(), feed_forward(0), feed_forward(1),
                feed_forward(2));
    RCLCPP_INFO(node_->get_logger(),
                "[%s]: tracker_cmd: x: %.2f, y: %.2f, z: %.2f, heading: %.2f",
                this->name_.c_str(), tracker_command.position.x,
                tracker_command.position.y, tracker_command.position.z,
                tracker_command.heading);
    RCLCPP_INFO(node_->get_logger(),
                "[%s]: odometry: x: %.2f, y: %.2f, z: %.2f, heading: %.2f",
                this->name_.c_str(), uav_state.pose.position.x,
                uav_state.pose.position.y, uav_state.pose.position.z,
                uav_heading);
  ...
  if (tracker_command.use_orientation) {
    // fill in the desired orientation based on the desired orientation from the
    // control command
    ...
    if (tracker_command.use_heading) {
      try {
        ...
        RCLCPP_WARN_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "[%s]: failed to add heading to the desired orientation matrix",
            this->name_.c_str());
      }
    }
    ...
    if (tracker_command.use_heading) {
      ...
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "[%s]: desired heading was not specified, using "
  ...
  if (rampup_active_) {
    ...
    if (std::abs((clock_->now() - rampup_start_time_).seconds()) >=
        rampup_duration_) {
      ...
      rampup_throttle_ +=
          double(rampup_direction_) * _rampup_speed_ * rampup_dt;

      rampup_last_time_ = clock_->now();

      RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 100,
                           "[%s]: ramping up throttle, %.4f", name_.c_str(),
                           rampup_throttle_);
    }
    ...
    if (desired_thrust_force >= 0) {
      throttle = mrs_lib::quadratic_throttle_model::forceToThrottle(
          common_handlers_->throttle_model, desired_thrust_force);
    } else {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[%s]: just so you know, the desired throttle force "
                           "is negative (%.2f)",
                           name_.c_str(), desired_thrust_force);
    }
  }
  ...
  if (throttle_saturated) {
    ...
    RCLCPP_WARN_THROTTLE(
        ...
        "[%s]: desired state: pos [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]",
        name_.c_str(), tracker_command.position.x, tracker_command.position.y,
        tracker_command.position.z, tracker_command.heading);
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[%s]: desired state: vel [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]",
        name_.c_str(), tracker_command.velocity.x, tracker_command.velocity.y,
        tracker_command.velocity.z, tracker_command.heading_rate);
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[%s]: desired state: acc [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]",
        name_.c_str(), tracker_command.acceleration.x,
        tracker_command.acceleration.y, tracker_command.acceleration.z,
        tracker_command.heading_acceleration);
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[%s]: desired state: jerk [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]",
        name_.c_str(), tracker_command.jerk.x, tracker_command.jerk.y,
        tracker_command.jerk.z, tracker_command.heading_jerk);
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[%s]: ---------------------------", name_.c_str());
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[%s]: current state: pos [x: %.2f, y: %.2f, z: %.2f, hdg: %.2f]",
        name_.c_str(), uav_state.pose.position.x, uav_state.pose.position.y,
        uav_state.pose.position.z, uav_heading);
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[%s]: current state: vel [x: %.2f, y: %.2f, z: %.2f, yaw rate: %.2f]",
        name_.c_str(), uav_state.velocity.linear.x, uav_state.velocity.linear.y,
        uav_state.velocity.linear.z, uav_state.velocity.angular.z);
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[%s]: ---------------------------", name_.c_str());
  }
...

  {
    ...
    double world_accel_x = (thrust_vector(0) / total_mass) -
                           (Iw_w_(0) / total_mass) - (Ib_w(0) / total_mass);
    double world_accel_y = (thrust_vector(1) / total_mass) -
                           (Iw_w_(1) / total_mass) - (Ib_w(1) / total_mass);
  ...
  // fill the unbiased desired accelerations
  last_control_output_.desired_unbiased_acceleration =
      Eigen::Vector3d(desired_x_accel, desired_y_accel, desired_z_accel);
  ...
  last_control_output_.diagnostics.horizontal_speed_constraint =
      0.5 * _max_speed_horizontal_;
  last_control_output_.diagnostics.horizontal_acc_constraint =
      0.5 * _max_acceleration_horizontal_;

  last_control_output_.diagnostics.vertical_asc_speed_constraint =
      0.5 * _max_speed_vertical_;
  last_control_output_.diagnostics.vertical_asc_acc_constraint =
      0.5 * _max_acceleration_vertical_;
  ...
  if (tracker_command.use_attitude_rate) {
    rate_feedforward << tracker_command.attitude_rate.x,
        tracker_command.attitude_rate.y, tracker_command.attitude_rate.z;
    ...
    try {
      desired_yaw_rate = mrs_lib::AttitudeConverter(Rd).getYawRateIntrinsic(
          tracker_command.heading_rate);
    } catch (...) {
      RCLCPP_ERROR(node_->get_logger(),
                   "[%s]: exception caught while calculating the "
                   "desired_yaw_rate feedforward",
                   name_.c_str());
    }
  ...
  if (tracker_command.use_jerk && drs_params.jerk_feedforward) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[%s]: using jerk feedforward", name_.c_str());

    Eigen::Matrix3d I;
    I << 0, 1, 0, -1, 0, 0, 0, 0, 0;
    Eigen::Vector3d desired_jerk = Eigen::Vector3d(
        tracker_command.jerk.x, tracker_command.jerk.y, tracker_command.jerk.z);
    jerk_feedforward = (I.transpose() * Rd.transpose() * desired_jerk) /
                       (desired_thrust_force / total_mass);
  }

  // | --------------- run the attitude controller -------------- |

  Eigen::Vector3d attitude_rate_saturation(
      constraints.roll_rate, constraints.pitch_rate, constraints.yaw_rate);

  auto attitude_rate_command = common::attitudeController(
      node_, uav_state, attitude_cmd, jerk_feedforward + rate_feedforward,
      attitude_rate_saturation, Kq, false);
...

  {
    try {
      last_control_output_.desired_heading_rate =
          mrs_lib::AttitudeConverter(R).getHeadingRate(
              attitude_rate_command->body_rate);
    } catch (...) {
    }
  }
  ...
  auto control_group_command = common::attitudeRateController(
      node_, uav_state, attitude_rate_command.value(), Kw);
  ...
  mrs_msgs::msg::HwApiActuatorCmd actuator_cmd = common::actuatorMixer(
      node_, control_group_command.value(),
      common_handlers_->detailed_model_params->control_group_mixer);
...
void MpcController::positionPassthrough(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command) {
  if (!tracker_command.use_position_vertical ||
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "[%s]: the tracker did not provide position+hdg reference",
                 name_.c_str());
    return;
  }
...
void MpcController::PIDVelocityOutput(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command,
  ...
  Eigen::Vector3d pos_ref =
      Eigen::Vector3d(tracker_command.position.x, tracker_command.position.y,
                      tracker_command.position.z);
  Eigen::Vector3d pos =
      Eigen::Vector3d(uav_state.pose.position.x, uav_state.pose.position.y,
                      uav_state.pose.position.z);
  ...
  if (tracker_command.use_velocity_horizontal &&
    ...
    vel_ff =
        Eigen::Vector3d(tracker_command.velocity.x, tracker_command.velocity.y,
                        tracker_command.velocity.z);
  }
  ...
  position_pid_z_.setSaturation(
      std::min(constraints.vertical_ascending_speed,
               constraints.vertical_descending_speed));
  ...
  Eigen::Vector3d des_vel =
      Eigen::Vector3d(des_vel_x, des_vel_y, des_vel_z) + vel_ff;
  ...
  if (control_output == common::VELOCITY_HDG) {
    ...
    RCLCPP_ERROR(
        node_->get_logger(),
        "[%s]: the required output of the position PID is not supported",
        name_.c_str());
    return;
  }
...
bool MpcController::callbackSetIntegralTerms(
    const std::shared_ptr<std_srvs::srv::SetBool::Request>& request,
    const std::shared_ptr<std_srvs::srv::SetBool::Response>& response) {
  ...
  ss << "integral terms %s"
     << (integral_terms_enabled_ ? "enabled" : "disabled");
...
void MpcController::timerGains() {
  ...
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "MpcController::timerGains", common_handlers_->scope_timer.logger,
      common_handlers_->scope_timer.enabled);
  ...
  double gain_coeff =
      (mute_gains_ || mute_gains_by_tracker_) ? _gain_mute_coefficient_ : 1.0;
  ...
  gains.kq_roll_pitch = calculateGainChange(
      dt, gains.kq_roll_pitch, drs_params.kq_roll_pitch * gain_coeff,
      bypass_filter, "kq_roll_pitch", updated);
  gains.kq_yaw =
      calculateGainChange(dt, gains.kq_yaw, drs_params.kq_yaw * gain_coeff,
                          bypass_filter, "kq_yaw", updated);
  gains.km = calculateGainChange(dt, gains.km, drs_params.km * gain_coeff,
                                 bypass_filter, "km", updated);
  gains.kiwxy =
      calculateGainChange(dt, gains.kiwxy, drs_params.kiwxy * gain_coeff,
                          bypass_filter, "kiwxy", updated);
  gains.kibxy =
      calculateGainChange(dt, gains.kibxy, drs_params.kibxy * gain_coeff,
                          bypass_filter, "kibxy", updated);

  // do not apply muting on these gains
  gains.kiwxy_lim = calculateGainChange(
      dt, gains.kiwxy_lim, drs_params.kiwxy_lim, false, "kiwxy_lim", updated);
  gains.kibxy_lim = calculateGainChange(
      dt, gains.kibxy_lim, drs_params.kibxy_lim, false, "kibxy_lim", updated);
  gains.km_lim = calculateGainChange(dt, gains.km_lim, drs_params.km_lim, false,
                                     "km_lim", updated);
  ...
  if (updated) {
    RCLCPP_INFO_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[%s]: filtering gains after a dynamic parameter update",
        name_.c_str());
  }
}
...
double MpcController::calculateGainChange(const double dt,
                                          const double current_value,
                                          const double desired_value,
                                          const bool bypass_rate,
                                          std::string name, bool& updated) {
  ...
  if (!bypass_rate) {
    ...
    if (std::abs(current_value) < 1e-6) {
      ...
      change_in_perc =
          ((current_value + saturated_change) / current_value) - 1.0;
      ...
      if (std::abs(saturated_change) <
          std::abs(change) * gains_filter_min_change) {
  ...
  if (std::abs(change) > 1e-3) {
    RCLCPP_DEBUG(node_->get_logger(),
                 "[%s]: changing gain '%s' from %.2f to %.2f", name_.c_str(),
                 name.c_str(), current_value, desired_value);
    updated = true;
  }
...
double MpcController::getHeadingSafely(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command) {
  try {
...
PLUGINLIB_EXPORT_CLASS(mrs_uav_controllers::mpc_controller::MpcController,
                       mrs_uav_managers::Controller)
