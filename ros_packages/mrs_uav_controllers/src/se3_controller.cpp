...
class Se3Controller : public mrs_uav_managers::Controller {
 ...
 private:
  ...
  void SE3Controller(const mrs_msgs::msg::UavState& uav_state,
                     const mrs_msgs::msg::TrackerCommand& tracker_command,
                     const double& dt,
...
bool Se3Controller::initialize(
  ...
  if (!private_handlers->parent_param_loader->loadedSuccessfully()) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[Se3Controller]: Could not load all parameters!");
    return false;
  }
  ...
  // lateral gains
  private_handlers->param_loader->loadParam("se3/default_gains/horizontal/kp",
                                            gains_.kpxy);
  private_handlers->param_loader->loadParam("se3/default_gains/horizontal/kv",
                                            gains_.kvxy);
  private_handlers->param_loader->loadParam("se3/default_gains/horizontal/ka",
                                            gains_.kaxy);

  private_handlers->param_loader->loadParam("se3/default_gains/horizontal/kiw",
                                            gains_.kiwxy);
  private_handlers->param_loader->loadParam("se3/default_gains/horizontal/kib",
                                            gains_.kibxy);

  // | ------------------------- rampup ------------------------- |

  private_handlers->param_loader->loadParam("se3/rampup/enabled",
                                            _rampup_enabled_);
  private_handlers->param_loader->loadParam("se3/rampup/speed", _rampup_speed_);

  // height gains
  private_handlers->param_loader->loadParam("se3/default_gains/vertical/kp",
                                            gains_.kpz);
  private_handlers->param_loader->loadParam("se3/default_gains/vertical/kv",
                                            gains_.kvz);
  private_handlers->param_loader->loadParam("se3/default_gains/vertical/ka",
                                            gains_.kaz);

  // attitude gains
  private_handlers->param_loader->loadParam(
      "se3/default_gains/attitude/kq_roll_pitch", gains_.kq_roll_pitch);
  private_handlers->param_loader->loadParam("se3/default_gains/attitude/kq_yaw",
                                            gains_.kq_yaw);

  // attitude rate gains
  private_handlers->param_loader->loadParam(
      "se3/attitude_rate_gains/kw_roll_pitch", gains_.kw_roll_pitch);
  private_handlers->param_loader->loadParam("se3/attitude_rate_gains/kw_yaw",
                                            gains_.kw_yaw);

  // mass estimator
  private_handlers->param_loader->loadParam(
      "se3/default_gains/mass_estimator/km", gains_.km);
  private_handlers->param_loader->loadParam(
      "se3/default_gains/mass_estimator/km_lim", gains_.km_lim);
  dynparam_mgr_->register_param("se3/mass_estimator/fuse_acceleration",
                                &drs_params_.fuse_acceleration);

  // integrator limits
  private_handlers->param_loader->loadParam(
      "se3/default_gains/horizontal/kiw_lim", gains_.kiwxy_lim);
  private_handlers->param_loader->loadParam(
      "se3/default_gains/horizontal/kib_lim", gains_.kibxy_lim);

  // constraints
  private_handlers->param_loader->loadParam(
      "se3/constraints/tilt_angle_failsafe/enabled",
      _tilt_angle_failsafe_enabled_);
  private_handlers->param_loader->loadParam(
      "se3/constraints/tilt_angle_failsafe/limit", _tilt_angle_failsafe_);
  ...
  if (_tilt_angle_failsafe_enabled_ && std::abs(_tilt_angle_failsafe_) < 1e-3) {
    RCLCPP_ERROR(node_->get_logger(),
                 "[Se3Controller]: constraints/tilt_angle_failsafe/enabled = "
                 "'TRUE' but the limit is too low");
    return false;
  }

  private_handlers->param_loader->loadParam(
      "se3/constraints/throttle_saturation", _throttle_saturation_);

  // gain filtering
  private_handlers->param_loader->loadParam(
      "se3/gain_filtering/perc_change_rate", _gains_filter_change_rate_);
  private_handlers->param_loader->loadParam(
      "se3/gain_filtering/min_change_rate", _gains_filter_min_change_rate_);
  private_handlers->param_loader->loadParam("se3/gain_filtering/rate",
                                            _gain_filtering_rate_);
  private_handlers->param_loader->loadParam(
      "se3/gain_filtering/gain_mute_coefficient", _gain_mute_coefficient_);

  // output mode
  dynparam_mgr_->register_param("se3/preferred_output",
                                &drs_params_.preferred_output_mode,
                                mrs_lib::DynparamMgr::range_t<int>(0, 3));
  dynparam_mgr_->register_param("se3/rotation_matrix",
                                &drs_params_.rotation_type,
                                mrs_lib::DynparamMgr::range_t<int>(0, 1));

  // angular rate feed forward
  dynparam_mgr_->register_param(
      "se3/angular_rate_feedforward/parasitic_pitch_roll",
      &drs_params_.pitch_roll_heading_rate_compensation);
  dynparam_mgr_->register_param("se3/angular_rate_feedforward/jerk",
  ...
  dynparam_mgr_->register_param(
      "horizontal.kpxy", &drs_params_.kpxy, gains_.kpxy,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 40.0));
  dynparam_mgr_->register_param(
      "horizontal.kvxy", &drs_params_.kvxy, gains_.kvxy,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 40.0));

  dynparam_mgr_->register_param(
      "horizontal.kaxy", &drs_params_.kaxy, gains_.kaxy,
  ...
  dynparam_mgr_->register_param(
      "vertical.kpz", &drs_params_.kpz, gains_.kpz,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 200.0));
  dynparam_mgr_->register_param(
      "vertical.kvz", &drs_params_.kvz, gains_.kvz,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 200.0));

  dynparam_mgr_->register_param(
      "vertical.kaz", &drs_params_.kaz, gains_.kaz,
      mrs_lib::DynparamMgr::range_t<double>(0.0, 2.0));
...

  {
    std::function<void()> callback_fcn =
        std::bind(&Se3Controller::timerGains, this);
  ...
  profiler_ = mrs_lib::Profiler(common_handlers->parent_node, "Se3Controller",
                                _profiler_enabled_);
...
void Se3Controller::destroy() { timer_gains_->stop(); }
...
bool Se3Controller::activate(const ControlOutput& last_control_output) {
  ...
  if (activation_control_output_.diagnostics.mass_estimator) {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: setting mass difference from the last "
                "control output: %.2f kg",
                uav_mass_difference_);
  }
  ...
  if (activation_control_output_.diagnostics.disturbance_estimator) {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: setting disturbances from the last control "
                "output: Ib_b_: %.2f, %.2f N, Iw_w_: "
                "%.2f, %.2f N",
  ...
  if (_rampup_enabled_ && throttle_last_controller) {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: activating rampup with initial throttle: "
                "%.4f, target: %.4f",
                throttle_last_controller.value(), hover_throttle);
...
void Se3Controller::updateInactive(
    const mrs_msgs::msg::UavState& uav_state,
...
Se3Controller::ControlOutput Se3Controller::updateActive(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command) {
  mrs_lib::Routine profiler_routine = profiler_.createRoutine("updateActive");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "Se3Controller::updateActive",
  ...
  if (std::abs(dt) < 0.001) {
    RCLCPP_DEBUG(
        node_->get_logger(),
        "[Se3Controller]: the last odometry message came too close (%.2f s)!",
        dt);
  ...
  if (!lowest_modality) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[Se3Controller]: output modalities are empty! This "
  ...
  if (drs_params.preferred_output_mode == OUTPUT_ATTITUDE_RATE &&
    ...
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[Se3Controller]: prioritizing attitude rate output");
    ...
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[Se3Controller]: prioritizing attitude output");
    ...
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[Se3Controller]: prioritizing control group output");
  ...
  switch (lowest_modality.value()) {
    ...
    case common::ACCELERATION_HDG_RATE: {
      SE3Controller(uav_state, tracker_command, dt,
                    common::ACCELERATION_HDG_RATE);
      break;
    }
...
void Se3Controller::switchOdometrySource(
    const mrs_msgs::msg::UavState& new_uav_state) {
  RCLCPP_INFO(node_->get_logger(),
              "[Se3Controller]: switching the odometry source");
  ...
  if (res) {
    ...
    RCLCPP_ERROR_THROTTLE(
        ...
        "[Se3Controller]: could not transform world integral to the new frame");
...
void Se3Controller::SE3Controller(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command, const double& dt,
    const common::CONTROL_OUTPUT& output_modality) {
  auto drs_params = mrs_lib::get_mutexed(mutex_drs_params_, drs_params_);
  ...
  if (tracker_command.use_acceleration) {
    Ra << tracker_command.acceleration.x, tracker_command.acceleration.y,
  ...
  const Eigen::Vector3d Op(uav_state.pose.position.x, uav_state.pose.position.y,
                           uav_state.pose.position.z);
  const Eigen::Vector3d Ov(uav_state.velocity.linear.x,
                           uav_state.velocity.linear.y,
                           uav_state.velocity.linear.z);
  const Eigen::Vector3d Oa(uav_state.acceleration.linear.x,
  ...
  if (tracker_command.use_position_horizontal ||
      tracker_command.use_position_vertical) {
    Ep = Rp - Op;
  }
  ...
  if (tracker_command.use_velocity_horizontal ||
      ...
      tracker_command
          .use_position_vertical) {  // use_position_vertical == true, not a
                                     // mistake, this provides dampening
    Ev = Rv - Ov;
  }
...

  {
    ...
    // special case: if want to control z-pos but not the velocity => at least
    // provide z dampening, therefore kvz_
    if (tracker_command.use_velocity_vertical ||
        tracker_command.use_position_vertical) {
      Kv(2) = gains.kvz;
    } else {
...

  {
    ...
    if (res) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[Se3Controller]: could not transform the Ib_b_ to the world frame");
    }
  }
...

  {
    ...
    if (!std::isfinite(Iw_w_(0))) {
      Iw_w_(0) = 0;
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "[Se3Controller]: NaN detected in variable "
                            "'Iw_w_(0)', setting it to 0!!!");
    ...
    if (gains.kiwxy_lim >= 0 && world_integral_saturated) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[Se3Controller]: SE3's world X integral is being saturated!");
    }
    ...
    if (gains.kiwxy_lim >= 0 && world_integral_saturated) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[Se3Controller]: SE3's world Y integral is being saturated!");
    }
  }
...

  {
    ...
    // get the position control error in the fcu_untilted frame
    {
      ...
      if (res) {
        ...
        RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "[Se3Controller]: could not transform the "
                              "position error to fcu_untilted");
      }
    }
    ...
    // get the velocity control error in the fcu_untilted frame
    {
      ...
      if (res) {
        ...
        RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                              "[Se3Controller]: could not transform the "
                              "velocity error to fcu_untilted");
      }
    }
    ...
    if (gains.kibxy_lim > 0 && body_integral_saturated) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[Se3Controller]: SE3's body pitch integral is being saturated!");
    }
    ...
    if (gains.kibxy_lim > 0 && body_integral_saturated) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[Se3Controller]: SE3's body roll integral is being saturated!");
    }
  }
  ...
  if (output_modality == common::ACCELERATION_HDG ||
    ...
    Eigen::Vector3d des_acc =
        (position_feedback + velocity_feedback + integral_feedback) /
            total_mass +
        Ra;
    ...
    {
      Eigen::Vector3d unbiased_des_acc_world =
          (position_feedback + velocity_feedback) / total_mass + Ra;
  ...
  if (!rampup_active_) {
    ...
    if (!std::isfinite(uav_mass_difference_)) {
      ...
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "[Se3Controller]: NaN detected in variable 'uav_mass_difference_', "
          "setting it to 0 and returning!!!");
    ...
    if (uav_mass_saturated) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[Se3Controller]: The UAV mass difference is being "
                           "saturated to %.2f!",
                           uav_mass_difference_);
    }
  }
  ...
  Eigen::Vector3d f =
      position_feedback + velocity_feedback + integral_feedback + feed_forward;
  ...
  if (f(2) < 0) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "[Se3Controller]: the calculated downwards desired "
                         "force is negative (%.2f) -> mitigating flip",
                         f(2));
  ...
  auto f_normed_sanitized =
      common::sanitizeDesiredForce(node_, f.normalized(), tilt_safety_limit,
                                   constraints.tilt, "Se3Controller");

  if (!f_normed_sanitized) {
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: position feedback: [%.2f, %.2f, %.2f]",
                position_feedback(0), position_feedback(1),
                position_feedback(2));
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: velocity feedback: [%.2f, %.2f, %.2f]",
                velocity_feedback(0), velocity_feedback(1),
                velocity_feedback(2));
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: integral feedback: [%.2f, %.2f, %.2f]",
                integral_feedback(0), integral_feedback(1),
                integral_feedback(2));
    RCLCPP_INFO(node_->get_logger(),
                "[Se3Controller]: tracker_cmd: x: %.2f, y: %.2f, z: %.2f, "
                "heading: %.2f",
                tracker_command.position.x, tracker_command.position.y,
                tracker_command.position.z, tracker_command.heading);
    RCLCPP_INFO(
        node_->get_logger(),
        "[Se3Controller]: odometry: x: %.2f, y: %.2f, z: %.2f, heading: %.2f",
  ...
  if (tracker_command.use_orientation) {
    ...
    if (tracker_command.use_heading) {
      try {
        ...
        RCLCPP_ERROR_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "[Se3Controller]: could not set the desired heading");
      }
    }
    ...
    if (tracker_command.use_heading) {
      ...
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 10000,
                           "[Se3Controller]: desired heading was not "
                           "specified, using current heading instead!");
      bxd << cos(uav_heading), sin(uav_heading), 0;
    }

    Rd = common::so3transform(node_, f_normed, bxd,
                              drs_params.rotation_type == 1);
  }
  ...
  if (tracker_command.use_throttle) {
    ...
    if (std::abs((clock_->now() - rampup_start_time_).seconds()) >=
      ...
      RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 100,
                           "[Se3Controller]: ramping up throttle, %.4f",
                           rampup_throttle_);
    }
    ...
    if (desired_thrust_force >= 0) {
      ...
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "[Se3Controller]: just so you know, the desired "
                           "throttle force is negative (%.2f)",
  ...
  if (throttle_saturated) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[Se3Controller]: ---------------------------");
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[Se3Controller]: desired state: pos [x: %.2f, y: "
                         "%.2f, z: %.2f, hdg: %.2f]",
                         tracker_command.position.x, tracker_command.position.y,
                         tracker_command.position.z, tracker_command.heading);
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[Se3Controller]: desired state: vel [x: %.2f, y: "
                         "%.2f, z: %.2f, hdg: %.2f]",
                         tracker_command.velocity.x, tracker_command.velocity.y,
                         tracker_command.velocity.z,
                         tracker_command.heading_rate);
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[Se3Controller]: desired state: acc [x: %.2f, y: %.2f, z: %.2f, hdg: "
        "%.2f]",
        tracker_command.acceleration.x, tracker_command.acceleration.y,
        tracker_command.acceleration.z, tracker_command.heading_acceleration);
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[Se3Controller]: desired state: jerk [x: %.2f, y: "
                         "%.2f, z: %.2f, hdg: %.2f]",
                         tracker_command.jerk.x, tracker_command.jerk.y,
                         tracker_command.jerk.z, tracker_command.heading_jerk);
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[Se3Controller]: ---------------------------");
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 100,
                         "[Se3Controller]: current state: pos [x: %.2f, y: "
                         "%.2f, z: %.2f, hdg: %.2f]",
                         uav_state.pose.position.x, uav_state.pose.position.y,
                         uav_state.pose.position.z, uav_heading);
    RCLCPP_WARN_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "[Se3Controller]: current state: vel [x: %.2f, y: %.2f, z: %.2f, yaw "
        "rate: %.2f]",
        uav_state.velocity.linear.x, uav_state.velocity.linear.y,
  ...
  if (tracker_command.use_attitude_rate) {
    ...
    try {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "[Se3Controller]: exception caught while calculating the "
  ...
  if (tracker_command.use_jerk && drs_params.jerk_feedforward) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "[Se3Controller]: using jerk feedforward");
...
void Se3Controller::positionPassthrough(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command) {
  if (!tracker_command.use_position_vertical ||
      !tracker_command.use_position_horizontal ||
...
void Se3Controller::PIDVelocityOutput(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command,
  ...
  if (control_output == common::VELOCITY_HDG) {
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "[Se3Controller]: the required output of the position PID is "
                 "not supported");
    return;
  }
...
void Se3Controller::timerGains() {
  ...
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "Se3Controller::timerGains", common_handlers_->scope_timer.logger,
      common_handlers_->scope_timer.enabled);
  ...
  if (!std::isfinite(dt) || (dt <= 0) ||
      (dt > 5 * (1.0 / _gain_filtering_rate_))) {
    return;
  }

  bool updated = false;

  gains.kpxy = calculateGainChange(dt, gains.kpxy, drs_params.kpxy * gain_coeff,
                                   bypass_filter, "kpxy", updated);
  gains.kvxy = calculateGainChange(dt, gains.kvxy, drs_params.kvxy * gain_coeff,
                                   bypass_filter, "kvxy", updated);
  gains.kaxy = calculateGainChange(dt, gains.kaxy, drs_params.kaxy * gain_coeff,
                                   bypass_filter, "kaxy", updated);
  gains.kiwxy =
  ...
  gains.kpz = calculateGainChange(dt, gains.kpz, drs_params.kpz * gain_coeff,
                                  bypass_filter, "kpz", updated);
  gains.kvz = calculateGainChange(dt, gains.kvz, drs_params.kvz * gain_coeff,
                                  bypass_filter, "kvz", updated);
  gains.kaz = calculateGainChange(dt, gains.kaz, drs_params.kaz * gain_coeff,
                                  bypass_filter, "kaz", updated);
  ...
  if (updated) {
    RCLCPP_INFO_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "[Se3Controller]: filtering gains after a dynamic parameter update");
  }
}
...
double Se3Controller::calculateGainChange(const double dt,
  ...
  if (std::abs(change) > 1e-3) {
    RCLCPP_DEBUG(node_->get_logger(),
                 "[Se3Controller]: changing gain '%s' from %.2f to %.2f",
...
double Se3Controller::getHeadingSafely(
    const mrs_msgs::msg::UavState& uav_state,
    const mrs_msgs::msg::TrackerCommand& tracker_command) {
  try {
...
PLUGINLIB_EXPORT_CLASS(mrs_uav_controllers::se3_controller::Se3Controller,
