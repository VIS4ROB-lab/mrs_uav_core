...
void Dummy::initialize(const rclcpp::Node::SharedPtr &node, const std::shared_ptr<CommonHandlers_t> &ch, const std::shared_ptr<PrivateHandlers_t> &ph) {
...

  {
    ...
    timer_update_ = std::make_shared<TimerType>(opts_no_autostart, rclcpp::Rate(ch_->desired_uav_state_rate), callback_fcn);
  }

  {
    std::function<void()> callback_fcn = std::bind(&Dummy::timerCheckHealth, this);

    timer_check_health_ = std::make_shared<TimerType>(opts_autostart, rclcpp::Rate(ch_->desired_uav_state_rate), callback_fcn);
  }
