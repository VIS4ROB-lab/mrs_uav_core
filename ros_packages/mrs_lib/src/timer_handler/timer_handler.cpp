...
ROSTimer::ROSTimer(const rclcpp::Node::SharedPtr& node,
                   const rclcpp::Rate& rate,
                   const std::function<void()>& callback) {
  this->node_ = node;

  this->timer_ = node_->create_wall_timer(
      std::chrono::nanoseconds(rate.period()), callback);
}

ROSTimer::ROSTimer(const mrs_lib::TimerHandlerOptions& opts,
                   const rclcpp::Rate& rate,
                   const std::function<void()>& callback) {
  ...
  if (this->callback_group_) {
    this->timer_ =
        node_->create_wall_timer(std::chrono::nanoseconds(rate.period()),
                                 callback, opts.callback_group.value());
  } else {
    this->timer_ = node_->create_wall_timer(
        std::chrono::nanoseconds(rate.period()), callback);
  }
...
void ROSTimer::setPeriod(const rclcpp::Duration& duration) {
  ...
  if (timer_) {
    ...
    if (this->callback_group_) {
      this->timer_ = node_->create_wall_timer(
          std::chrono::duration<double>(duration.seconds()), callback,
          this->callback_group_.value());
    } else {
      this->timer_ = node_->create_wall_timer(
          std::chrono::duration<double>(duration.seconds()), callback);
    }
...
void ROSTimer::setCallback(
    [[maybe_unused]] const std::function<void()>& callback) {
  RCLCPP_ERROR(node_->get_logger(),
               "[mrs_lib::ROSTimer]: setCallback(...) is not implemented for "
               "ROSTimer! Check [mrs_lib/src/timer/timer.cpp].");
}
//}
...
ThreadTimer::ThreadTimer(const rclcpp::Node::SharedPtr& node,
                         const rclcpp::Rate& rate,
                         const std::function<void()>& callback) {
...
ThreadTimer::ThreadTimer(const mrs_lib::TimerHandlerOptions& opts,
                         const rclcpp::Rate& rate,
                         const std::function<void()>& callback) {
...
ThreadTimer::Impl::Impl(const rclcpp::Node::SharedPtr& node,
                        const std::function<void()>& callback,
                        const rclcpp::Rate& rate, const bool oneshot)
    : oneshot_(oneshot) {
  node_ = node;
...
bool ThreadTimer::Impl::breakableSleep(const rclcpp::Time& until) {
  while (rclcpp::ok() && node_->get_clock()->now() < until) {
    const std::chrono::nanoseconds dur{
        (until - node_->get_clock()->now()).nanoseconds()};
    const std::chrono::time_point<std::chrono::steady_clock> until_stl =
  ...
  while (rclcpp::ok() && !ending_) {
    {
      ...
      // if the timer is not yet started, wait for condition variable
      // notification
      if (!running_ && !ending_) {
        wakeup_cond_.wait(lck);
      }

      // The thread either got cv-notified, or running_ was already true, or it
      // got woken up for some other reason. Check if the reason is that we
      // should end (and end if it is).
      if (ending_) {
        break;
      }

      // Check if the timer is still paused - probably was a spurious wake up
      // (and restart the wait if it is).
      if (!running_) {
        continue;
      }
    }

    // If the flow got here, the thread should attempt to wait for the specified
    // duration. first, copy the delay we should wait for in a thread-safe
    // manner
    rclcpp::Time next_expected;
    {
      ...
      next_expected =
          node_->get_clock()->now() +
          rclcpp::Duration(std::chrono::duration<double>(delay_dur_));
    }
...

    {
      ...
      // Again, check if everything is OK (the state may have changed while the
      // thread was a sleeping beauty). Check if we should end (and end if it is
      // so).
      if (ending_) {
        break;
      }
