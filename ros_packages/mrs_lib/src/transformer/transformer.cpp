...
template std::optional<test_t> mrs_lib::Transformer::transform<test_t>(
    const test_t& what, const geometry_msgs::msg::TransformStamped& to_frame);
template std::optional<test_t::SharedPtr> mrs_lib::Transformer::transform<
    test_t>(const test_t::SharedPtr& what,
            const geometry_msgs::msg::TransformStamped& to_frame);
template std::optional<test_t::SharedPtr> mrs_lib::Transformer::transform<
    test_t>(const test_t::ConstSharedPtr& what,
            const geometry_msgs::msg::TransformStamped& to_frame);
template std::optional<test_t> mrs_lib::Transformer::transformSingle<test_t>(
    const test_t& what, const std::string& to_frame);
template std::optional<test_t::SharedPtr>
mrs_lib::Transformer::transformSingle<test_t>(const test_t::SharedPtr& what,
                                              const std::string& to_frame);
template std::optional<test_t::SharedPtr> mrs_lib::Transformer::transformSingle<
    test_t>(const test_t::ConstSharedPtr& what, const std::string& to_frame);

#ifdef OPENCV_SPECIALIZATION
namespace tf2 {
template <>
void doTransform(const cv::Point3d& point_in, cv::Point3d& point_out,
...
Transformer::Transformer(const rclcpp::Node::SharedPtr& node,
                         ...
                         const rclcpp::Duration& cache_time,
                         const rclcpp::QoS& qos)
    : initialized_(true) {
  node_ = node;
  tf_buffer_ = std::make_unique<tf2_ros::Buffer>(
      clock, tf2::Duration(cache_time.nanoseconds()), node);
  tf_listener_ptr_ =
      std::make_unique<tf2_ros::TransformListener>(*tf_buffer_, node, true);
}
...
std::optional<geometry_msgs::msg::TransformStamped> Transformer::getTransform(
    const std::string& from_frame_raw, const std::string& to_frame_raw,
...
std::optional<geometry_msgs::msg::TransformStamped> Transformer::getTransform(
    const std::string& from_frame_raw, const rclcpp::Time& from_stamp,
  ...
  return getTransformImpl(from_frame, from_stamp, to_frame, to_stamp,
                          fixed_frame, latlon_frame);
}
//}
...
[[nodiscard]] std::optional<Eigen::Vector3d> Transformer::transformAsVector(
  ...
  const geometry_msgs::msg::TransformStamped tf_resolved =
      create_transform(from_frame, to_frame, tf.header.stamp, tf.transform);
...
[[nodiscard]] std::optional<Eigen::Vector3d> Transformer::transformAsVector(
    const std::string& from_frame_raw, const Eigen::Vector3d& what,
  ...
  // get the transform
  const auto tf_opt =
      getTransformImpl(from_frame, to_frame, time_stamp, latlon_frame);
  if (!tf_opt.has_value()) return std::nullopt;
...
[[nodiscard]] std::optional<Eigen::Vector3d> Transformer::transformAsPoint(
    const Eigen::Vector3d& what,
    const geometry_msgs::msg::TransformStamped& tf) {
  std::scoped_lock lck(mutex_);
...
std::optional<mrs_msgs::msg::ReferenceStamped> Transformer::transformImpl(
    const geometry_msgs::msg::TransformStamped& tf,
  ...
  pose.pose.orientation = geometry::fromEigen(
      geometry::quaternionFromHeading(what.reference.heading));
  ...
  ret.reference.heading =
      geometry::headingFromRot(geometry::toEigen(pose.pose.orientation));
  return ret;
}
...
std::optional<Eigen::Vector3d> Transformer::transformImpl(
    const geometry_msgs::msg::TransformStamped& tf,
    const Eigen::Vector3d& what) {
  // just transform it as you would a geometry_msgs::msg::Vector3
  const geometry_msgs::msg::Vector3 as_vec =
      mrs_lib::geometry::fromEigenVec(what);
...
Transformer::getTransformImpl(const std::string& from_frame,
                              const std::string& to_frame,
                              const rclcpp::Time& time_stamp,
  ...
  if (from_frame.empty() || to_frame.empty()) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *node_->get_clock(), 1e9,
        "Transformer: cannot transform, not from_frame or to_frame are empty");
    return std::nullopt;
  }

  // if the frames are the same, just return an identity transform
  if (from_frame == to_frame)
    return create_transform(from_frame, to_frame, time_stamp,
                            tf2::toMsg(tf2::Transform::getIdentity()));

  // check for a transform from/to latlon coordinates - that is a special case
  // handled separately
  if (from_frame == latlon_frame) {
    // find the transformation between the UTM frame and the non-latlon frame to
    // fill the returned tf
    const std::string utm_frame = getFramePrefix(from_frame) + "utm_origin";
    auto tf_opt =
        getTransformImpl(utm_frame, to_frame, time_stamp, latlon_frame);
    if (!tf_opt.has_value()) return std::nullopt;
    ...
    auto tf_opt =
        getTransformImpl(from_frame, utm_frame, time_stamp, latlon_frame);
    if (!tf_opt.has_value()) return std::nullopt;
  ...
  if (retry_lookup_newest_) {
    try {
      return tf_buffer_->lookupTransform(to_frame, from_frame,
                                         rclcpp::Time(0, 0), lookup_timeout_);
    } catch (tf2::TransformException& e) {
      ex = e;
    }
  }
  ...
  if (quiet_) {
    RCLCPP_DEBUG(node_->get_logger(),
                 "Transformer: Exception caught while looking up transform "
                 "from \"%s\" to \"%s\": %s",
                 from_frame.c_str(), to_frame.c_str(), ex.what());
  } else {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1e9,
                         "Transformer: Exception caught while looking up "
                         "transform from \"%s\" to \"%s\": %s",
                         from_frame.c_str(), to_frame.c_str(), ex.what());
  }
...
Transformer::getTransformImpl(const std::string& from_frame,
                              ...
                              const std::string& latlon_frame) {
  if (!initialized_) {
    std::cout << "Transformer: cannot provide transform, not initialized"
              << std::endl;
    return std::nullopt;
  }
  ...
  // if the frames are the same, just return an identity transform
  if (from_frame == to_frame)
    return create_transform(from_frame, to_frame, to_stamp,
                            tf2::toMsg(tf2::Transform::getIdentity()));
  ...
  if (from_frame == latlon_frame) {
    ...
    // fill the returned tf
    const std::string utm_frame = getFramePrefix(from_frame) + "utm_origin";
    auto tf_opt = getTransformImpl(utm_frame, from_stamp, to_frame, to_stamp,
                                   fixed_frame, latlon_frame);
    if (!tf_opt.has_value()) return std::nullopt;
    // change the transformation frames to point from latlon
    frame_from(*tf_opt) = from_frame;
    ...
    // fill the returned tf
    const std::string utm_frame = getFramePrefix(to_frame) + "utm_origin";
    auto tf_opt = getTransformImpl(from_frame, from_stamp, utm_frame, to_stamp,
                                   fixed_frame, latlon_frame);
    if (!tf_opt.has_value()) return std::nullopt;
    // change the transformation frames to point to latlon
    frame_to(*tf_opt) = to_frame;
  ...
  try {
    // try looking up and returning the transform
    return tf_buffer_->lookupTransform(to_frame, to_stamp, from_frame,
                                       from_stamp, fixed_frame,
                                       lookup_timeout_);
  } catch (tf2::TransformException& e) {
    ex = e;
  }
...
std::string Transformer::resolveFrameImpl(const std::string& frame_id) {
  ...
  if (frame_id.empty()) return default_frame_id_;

  // if there is no prefix set, just return the raw frame id
  if (prefix_.empty()) return frame_id;

  // if there is a default prefix set and the frame does not start with it,
  // prefix it
...
/* LLtoUTM() method //{ */
geometry_msgs::msg::Point Transformer::LLtoUTM(
...
geometry_msgs::msg::PointStamped Transformer::LLtoUTM(
    const geometry_msgs::msg::PointStamped& what, const std::string& prefix) {
...
geometry_msgs::msg::Pose Transformer::LLtoUTM(
    const geometry_msgs::msg::Pose& what, const std::string& prefix) {
...
geometry_msgs::msg::PoseStamped Transformer::LLtoUTM(
    const geometry_msgs::msg::PoseStamped& what, const std::string& prefix) {
...
/* UTMtoLL() method //{ */
std::optional<geometry_msgs::msg::Point> Transformer::UTMtoLL(
    const geometry_msgs::msg::Point& what,
    [[maybe_unused]] const std::string& prefix) {
  // if no UTM zone was specified by the user, we don't know which one to use...
  if (!got_utm_zone_) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1e9,
                         "cannot transform to latlong, missing UTM zone (did "
                         "you call setLatLon()?)");
    return std::nullopt;
  }
...
std::optional<geometry_msgs::msg::PointStamped> Transformer::UTMtoLL(
    const geometry_msgs::msg::PointStamped& what, const std::string& prefix) {
...
std::optional<geometry_msgs::msg::Pose> Transformer::UTMtoLL(
    const geometry_msgs::msg::Pose& what, const std::string& prefix) {
...
std::optional<geometry_msgs::msg::PoseStamped> Transformer::UTMtoLL(
    const geometry_msgs::msg::PoseStamped& what, const std::string& prefix) {
