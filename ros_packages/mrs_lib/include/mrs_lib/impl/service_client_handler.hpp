/**  \file
     \brief Implements ServiceClientHandler and related convenience classes for
   upgrading the ROS service client
...
ServiceClientHandler<ServiceType>::ServiceClientHandler(
    rclcpp::Node::SharedPtr& node, const std::string& address,
    const rclcpp::CallbackGroup::SharedPtr& callback_group)
    : ServiceClientHandler(node, address, rclcpp::ServicesQoS(),
                           callback_group) {}

//}

/* callSync(const ServiceType::Request& request, ServiceType::Response&
 * response) //{ */

template <class ServiceType>
std::optional<std::shared_ptr<typename ServiceType::Response>>
ServiceClientHandler<ServiceType>::callSync(
    const std::shared_ptr<typename ServiceType::Request>& request) {
  if (!impl_) {
    RCLCPP_ERROR(rclcpp::get_logger("ServiceClientHandler"),
...
/* callAsync(const ServiceType::Request& request, ServiceType::Response&
 * response) //{ */

template <class ServiceType>
std::optional<
    std::shared_future<std::shared_ptr<typename ServiceType::Response>>>
ServiceClientHandler<ServiceType>::callAsync(
    const std::shared_ptr<typename ServiceType::Request>& request) {
  if (!impl_) {
    RCLCPP_ERROR(rclcpp::get_logger("ServiceClientHandler"),
                 "Not initialized, cannot use callSync()!");
...
class ServiceClientHandler<ServiceType>::Impl {
 public:
   ...
   */
  Impl(rclcpp::Node::SharedPtr& node, const std::string& address,
       const rclcpp::QoS& qos,
       const rclcpp::CallbackGroup::SharedPtr& callback_group)
      : callback_group_(callback_group),
        service_client_(node->create_client<ServiceType>(
            address, qos.get_rmw_qos_profile(), callback_group)) {}
   ...
   */
  std::optional<std::shared_ptr<typename ServiceType::Response>> callSync(
      const std::shared_ptr<typename ServiceType::Request>& request) {
    /* always check if the service is ready before calling */
    if (!service_client_->service_is_ready()) return std::nullopt;

    /* the future done callback is being run in a separate thread under the
     * default callback group of the node */
    const auto future_msg =
        service_client_->async_send_request(request).future.share();

    /* it is a good practice to check if the future object is not already
     * invalid after the call */
    /* if valid() is false, the future has UNDEFINED behavior */
    if (!future_msg.valid()) return std::nullopt;
  ...
  std::optional<
      std::shared_future<std::shared_ptr<typename ServiceType::Response>>>
  callAsync(const std::shared_ptr<typename ServiceType::Request>& request) {
    /* always check if the service is ready before calling */
    if (!service_client_->service_is_ready()) return std::nullopt;
