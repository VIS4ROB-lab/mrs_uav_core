...
// implements the constructor, getMsg() method and data_callback method
// (non-thread-safe)
template <typename MessageType>
class SubscriberHandler<MessageType>::Impl {
 public:
  using timeout_callback_t =
      typename SubscriberHandler<MessageType>::timeout_callback_t;
  using message_callback_t =
      typename SubscriberHandler<MessageType>::message_callback_t;
  using data_callback_t =
      std::function<void(const typename MessageType::ConstSharedPtr&)>;
 ...
 public:
  /* constructor //{ */
  Impl(const SubscriberHandlerOptions& options,
    ...
    if (options.timeout_callback) {
      m_timeout_mgr_callback = std::bind(options.timeout_callback, m_topic_name,
                                         std::placeholders::_1);
    } else {
      m_timeout_mgr_callback = std::bind(&Impl::default_timeout_callback, this,
                                         m_topic_name, std::placeholders::_1);
    }
    ...
    if (options.no_message_timeout != mrs_lib::no_timeout) {
      ...
      if (!m_timeout_manager) {
        m_timeout_manager = std::make_shared<mrs_lib::TimeoutManager>(
            m_node,
            rclcpp::Rate(1.0 / (options.no_message_timeout.seconds() * 0.5)));
      }

      // register the timeout callback with the TimeoutManager
      m_timeout_id = m_timeout_manager->registerNew(options.no_message_timeout,
                                                    m_timeout_mgr_callback,
                                                    m_node->get_clock()->now());
    }

    const std::string msg =
        "Subscribed to topic '" + m_topic_name + "' -> '" + m_topic_name + "'";
 ...
 public:
  ...
  virtual typename MessageType::ConstSharedPtr peekMsg() const {
    /* assert(m_got_data); */
    /* if (!m_got_data) */
    /*   ROS_ERROR("[%s]: No data received yet from topic '%s' (forgot to check
     * hasMsg()?)! Returning empty message.", m_node_name.c_str(), */
    /*             topicName().c_str()); */
  ...
  /* hasMsg() method //{ */
  virtual bool hasMsg() const { return m_got_data; }
  //}

  /* newMsg() method //{ */
  virtual bool newMsg() const { return m_new_data; }
  //}

  /* usedMsg() method //{ */
  virtual bool usedMsg() const { return m_used_data; }
  //}

  /* waitForNew() method //{ */
  virtual typename MessageType::ConstSharedPtr waitForNew(
    ...
    // if new data is available, return immediately, otherwise attempt to wait
    // for new data using the respective condition variable
    if (m_new_data)
      return getMsg();
    else if (m_new_data_cv.wait_for(lock, chrono_timeout) ==
  ...
  /* lastMsgTime() method //{ */
  virtual rclcpp::Time lastMsgTime() const { return m_latest_message_time; };
  //}
  ...
  virtual void setNoMessageTimeout(const rclcpp::Duration& timeout) {
    if (timeout == mrs_lib::no_timeout) {
      // if there is a timeout callback already registered but the user wants to
      // disable it, pause it
      ...
      if (m_timeout_manager == nullptr)
        m_timeout_manager = std::make_shared<mrs_lib::TimeoutManager>(
            m_node, rclcpp::Rate(1.0 / (timeout.seconds() * 0.5)));

      // if there is an existing timeout callback registered, change its timeout
      if (m_timeout_id.has_value())
        m_timeout_manager->change(m_timeout_id.value(), timeout,
                                  m_timeout_mgr_callback,
                                  m_node->get_clock()->now());

      // otherwise, register it
      else
        m_timeout_id = m_timeout_manager->registerNew(
            timeout, m_timeout_mgr_callback, m_node->get_clock()->now());
    }
  }
  //}
  ...
  virtual void start() {
    ...
    const std::function<void(const typename MessageType::SharedPtr)> cbk =
        std::bind(&Impl::data_callback, this, std::placeholders::_1);

    m_sub = m_node->create_subscription<MessageType>(m_topic_name, m_qos, cbk,
                                                     m_sub_opts);
  }
  //}
 ...
 protected:
  ...
  typename rclcpp::Subscription<MessageType>::SharedPtr m_sub;
 ...
 protected:
  ...
  void default_timeout_callback(const std::string& topic_name,
    ...
    const std::string txt =
        "Did not receive any message from topic '" + topic_name + "' for " +
        std::to_string(since_msg.seconds()) + "s (" + std::to_string(n_pubs) +
  ...
  void process_new_message(const typename MessageType::ConstSharedPtr& msg) {
    ...
    // If the message callback is registered, the new data will immediately be
    // processed, so reset the flag. Otherwise, set the flag.
  ...
  virtual void data_callback(const typename MessageType::ConstSharedPtr& msg) {
    ...
    // execute the callback after unlocking the mutex to enable multi-threaded
    // callback execution
...
template <typename MessageType>
class SubscriberHandler<MessageType>::ImplThreadsafe
    : public SubscriberHandler<MessageType>::Impl {
 private:
 ...
 public:
  ImplThreadsafe(
      const SubscriberHandlerOptions& options,
      const message_callback_t& message_callback = message_callback_t())
 ...
 protected:
  virtual void data_callback(
      const typename MessageType::ConstSharedPtr& msg) override {
    {
    ...
    // execute the callback after unlocking the mutex to enable multi-threaded
    // callback execution
    if (this->m_message_callback) impl_class_t::m_message_callback(msg);
  }
