...
bool DynparamMgr::register_param(const std::string& name, MemT* param_var,
  ...
  return register_param_impl<MemT>(name, param_var, std::nullopt, std::nullopt,
                                   update_cbk);
}
...
bool DynparamMgr::register_param(const std::string& name, MemT* param_var,
                                 ...
                                 const update_cbk_t<MemT>& update_cbk) {
  return register_param_impl<MemT>(name, param_var, default_value, std::nullopt,
                                   update_cbk);
}
...
bool DynparamMgr::register_param(const std::string& name, MemT* param_var,
                                 ...
                                 const update_cbk_t<MemT>& update_cbk) {
  static_assert(is_numeric_v<MemT>,
                "range overload only valid for numeric types");
  return register_param_impl<MemT>(name, param_var, std::nullopt, valid_range,
                                   update_cbk);
}
...
bool DynparamMgr::register_param(const std::string& name, MemT* param_var,
  ...
  static_assert(is_numeric_v<MemT>,
                "range overload only valid for numeric types");
  return register_param_impl<MemT>(name, param_var, default_value, valid_range,
                                   update_cbk);
}

template <typename MemT>
bool DynparamMgr::register_param_impl(
  ...
  if (!get_success) {
    ...
    RCLCPP_ERROR_STREAM(m_node->get_logger(),
                        "[" << m_node->get_name()
                            << "]: Could not register dynamic parameter '"
                            << name << "'");
    return false;
  }

  // only assign the default value if everything is OK, otherwise leave
  // param_var untouched
  *param_var = current_value;

  // remember the registered parameter, the corresponding variable, etc.
  RCLCPP_INFO_STREAM(m_node->get_logger(), "[" << m_node->get_name()
                                               << "]: Dynamic parameter '"
                                               << name << "':\t" << *param_var);

  m_registered_params.emplace_back(
      *m_node, resolved_name, to_param_type<MemT>(), param_var, update_cbk);
  return true;
}
//}
...
struct DynparamMgr::registered_param_t {
  // | --------------------- Metaprogramming -------------------- |
  // some metaprogramming tooling to generate a variant of pointers from a tuple
  // of types
  ...
  // the same tooling for creating a variant of functions with the types as
  // const-ref arguments
  template <typename Type>
  ...
  // define the std::variant of std::functions taking a const-ref of the types
  // from valid_types_t
  using update_cbk_variant_t =
      function_variant_from_list_t<valid_types_t>::type;

  // | ------------------------- Members ------------------------ |
  rclcpp::Node& node;  // non-owning reference (the memory is share-owned by the
                       // DynparamMgr)
  ...
  // | ----------------------- Constructor ---------------------- |
  template <typename MemT>
  registered_param_t(rclcpp::Node& node_in,
                     const ParamProvider::resolved_name_t& resolved_in,
                     rclcpp::ParameterType type_in, MemT* param_ptr_in,
                     const std::function<void(const MemT&)>& update_cbk_in)
      : node(node_in),
        resolved_name(resolved_in),
        type(type_in),
        param_ptr(param_ptr_in),
        update_cbk(update_cbk_in) {}
        ...
        [&new_value, this](auto arg) {
          ...
          if constexpr (std::is_invocable_v<CbkT, NewValueT>) {
            ...
            RCLCPP_ERROR_STREAM_THROTTLE(
                ...
                "Cannot call update callback for parameter \""
                    << resolved_name.str << "\" - incompatible callback type!");
          }
        },
        update_cbk);
