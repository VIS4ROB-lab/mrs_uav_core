...
class Topic {
 ...
 public:
  Topic(const rclcpp::Node::SharedPtr node, std::string topic_name)
      : topic_name_(topic_name) {
    node_ = node;
    last_time_ = rclcpp::Time(0, 0, node->get_clock()->get_clock_type());
  }

  void updateTime(void) { last_time_ = node_->get_clock()->now(); }

  rclcpp::Time getTime(void) { return last_time_; }
...
// state machine
typedef enum { STATE_IDLE, STATE_TAKEOFF, STATE_FINISHED } LandingStates_t;
...
class AutomaticStart : public mrs_lib::Node {
 ...
 private:
  ...
  mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>
      service_client_toggle_control_output_;
  ...
  mrs_lib::ServiceClientHandler<mrs_msgs::srv::ValidateReference>
      service_client_validate_reference_;
  ...
  mrs_lib::SubscriberHandler<mrs_msgs::msg::UavManagerDiagnostics>
      sh_uav_manager_diag_;
  mrs_lib::SubscriberHandler<mrs_msgs::msg::GazeboSpawnerDiagnostics>
  ...
  void callbackHwApiCapabilities(
      const mrs_msgs::msg::HwApiCapabilities::ConstSharedPtr msg);

  // | --------------- Gazebo spawner diagnostics --------------- |

  void callbackGazeboSpawnerDiagnostics(
      const mrs_msgs::msg::GazeboSpawnerDiagnostics::ConstSharedPtr msg);
  ...
  void genericCallback(std::shared_ptr<rclcpp::SerializedMessage> msg,
...
void AutomaticStart::initialize() {
  ...
  cbkgrp_subs_ = this_node().create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  cbkgrp_sc_ = this_node().create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  ...
  param_loader.loadParam("preflight_check/time_window",
                         _preflight_check_time_window_);

  param_loader.loadParam("preflight_check/speed_check/enabled",
                         _speed_check_enabled_);
  param_loader.loadParam("preflight_check/speed_check/max_speed",
                         _speed_check_max_speed_);

  param_loader.loadParam("preflight_check/height_check/enabled",
                         _height_check_enabled_);
  param_loader.loadParam("preflight_check/height_check/max_height",
                         _height_check_max_height_);

  param_loader.loadParam("preflight_check/gyro_check/enabled",
                         _gyro_check_enabled_);
  param_loader.loadParam("preflight_check/gyro_check/max_rate",
                         _gyro_check_max_rate_);

  param_loader.loadParam("preflight_check/topic_check/enabled",
                         _topic_check_enabled_);
  param_loader.loadParam("preflight_check/topic_check/timeout",
                         _topic_check_timeout_);
  param_loader.loadParam("preflight_check/topic_check/topics",
                         _topic_check_topic_names_);
  ...
  sh_estimation_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::EstimationDiagnostics>(
          shopts, "~/estimation_diag_in");
  sh_hw_api_status_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::HwApiStatus>(
      shopts, "~/hw_api_status_in", &AutomaticStart::callbackHwApiStatus, this);
  sh_hw_api_capabilities_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::HwApiCapabilities>(
          shopts, "~/hw_api_capabilities_in",
          &AutomaticStart::callbackHwApiCapabilities, this);
  sh_distance_sensor_ = mrs_lib::SubscriberHandler<sensor_msgs::msg::Range>(
      shopts, "~/distance_sensor_in");
  sh_imu_ =
      mrs_lib::SubscriberHandler<sensor_msgs::msg::Imu>(shopts, "~/imu_in");
  sh_control_manager_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::ControlManagerDiagnostics>(
          shopts, "~/control_manager_diagnostics_in");
  sh_uav_manager_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::UavManagerDiagnostics>(
          shopts, "~/uav_manager_diagnostics_in");
  sh_gazebo_spawner_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::GazeboSpawnerDiagnostics>(
          shopts, "~/gazebo_spawner_diagnostics_in",
          &AutomaticStart::callbackGazeboSpawnerDiagnostics, this);

  // | ----------------------- publishers ----------------------- |

  ph_can_takeoff_ = mrs_lib::PublisherHandler<std_msgs::msg::Bool>(
      this_node_ptr(), "~/can_takeoff_out");

  // | --------------------- service clients -------------------- |

  rclcpp::Node::SharedPtr node = this_node_ptr();

  service_client_takeoff_ =
      mrs_lib::ServiceClientHandler<std_srvs::srv::Trigger>(
          node, "~/takeoff_out", cbkgrp_sc_);
  service_client_toggle_control_output_ =
      mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
          node, "~/toggle_control_output_out", cbkgrp_sc_);
  service_client_arm_ = mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
      node, "~/arm_out", cbkgrp_sc_);
  service_client_validate_reference_ =
      mrs_lib::ServiceClientHandler<mrs_msgs::srv::ValidateReference>(
          node, "~/validate_reference_out", cbkgrp_sc_);
  ...
  if (_topic_check_enabled_) {
    for (int i = 0; i < int(_topic_check_topic_names_.size()); i++) {
      ...
      std::string topic_type =
          topic.substr(topic.find(":") + 1, topic.length());
      ...
      std::function<void(std::shared_ptr<rclcpp::SerializedMessage> msg)>
          callback_fcn = std::bind(&AutomaticStart::genericCallback, this,
                                   std::placeholders::_1, topic_name, id);

      auto tmp_subscriber = this_node().create_generic_subscription(
          topic_name, topic_type, rclcpp::SystemDefaultsQoS(), callback_fcn);
...

  {
    std::function<void()> callback_fcn =
        std::bind(&AutomaticStart::timerMain, this);
...
void AutomaticStart::genericCallback(
...
void AutomaticStart::callbackHwApiStatus(
    const mrs_msgs::msg::HwApiStatus::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
...
void AutomaticStart::callbackHwApiCapabilities(
    [[maybe_unused]] const mrs_msgs::msg::HwApiCapabilities::ConstSharedPtr
        msg) {
  if (!is_initialized_) {
    return;
  }
...
void AutomaticStart::callbackGazeboSpawnerDiagnostics(
    const mrs_msgs::msg::GazeboSpawnerDiagnostics::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
...
void AutomaticStart::timerMain() {
  ...
  bool got_hw_api = sh_hw_api_status_.hasMsg() &&
                    sh_hw_api_capabilities_.hasMsg() && hw_api_connected_;

  if (!got_control_manager_diag || !got_hw_api || !got_uav_manager_diag ||
      !got_estimation_diag) {
    RCLCPP_WARN_THROTTLE(
        this_node().get_logger(), *clock_, 5000,
        "waiting for data: ControlManager=%s, UavManager=%s, HW "
        "Api=%s, EstimationManager=%s",
        got_control_manager_diag ? "true" : "FALSE",
        got_uav_manager_diag ? "true" : "FALSE", got_hw_api ? "true" : "FALSE",
        got_estimation_diag ? "true" : "FALSE");
    return;
  }

  auto [armed, offboard, armed_time, offboard_time] = mrs_lib::get_mutexed(
      mutex_hw_api_status_, armed_, offboard_, armed_time_, offboard_time_);
  ...
  switch (current_state) {
    case STATE_IDLE: {
      ...
      if (!offboard && possibly_in_the_air) {
        RCLCPP_WARN_THROTTLE(
            this_node().get_logger(), *clock_, 1000,
            "preflight check failed, the UAV is possibly in the air");
          ...
          if (we_toggled_output_) {
            ...
            if (!res) {
              RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                                   "could not set control output OFF");
            }
          }
      ...
      bool control_output_enabled =
          sh_control_manager_diag_.getMsg()->output_enabled;
      ...
      if (armed && !control_output_enabled) {
        if (can_takeoff) {
          ...
          if (!res) {
            RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                                 "could not set control output ON");
          } else {
        ...
        if (armed_time.seconds() > 0 &&
            time_from_arming > _control_output_timeout_) {
          RCLCPP_WARN_THROTTLE(
              this_node().get_logger(), *clock_, 1000,
              "could not set control output ON for %.2f secs, disarming",
      ...
      if (_simulation_ && isGazeboSimulation()) {
        ...
        if (got_gazebo_spawner_diagnostics) {
          if (!gazebo_spawner_diagnostics_.spawn_called ||
              gazebo_spawner_diagnostics_.processing) {
            RCLCPP_WARN_THROTTLE(
                this_node().get_logger(), *clock_, 1000,
                "(simulation) waiting for spawner to finish spawning UAVs");
            return;
          }

        } else {
          RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                               "(simulation) missing spawner diagnostics");
          return;
        }
      }
      ...
      if (armed && offboard && control_output_enabled) {
        if (!_handle_takeoff_) {
          ...
          if (armed_time_diff.seconds() > _safety_timeout_ &&
              offboard_time_diff.seconds() > _safety_timeout_) {
            changeState(STATE_TAKEOFF);

          } else {
            double min = (armed_time_diff < offboard_time_diff)
                             ? armed_time_diff.seconds()
                             : offboard_time_diff.seconds();

            RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                                 "taking off in %.0f",
                                 (_safety_timeout_ - min));
          }
        }
      }
    ...
    case STATE_TAKEOFF: {
      ...
      if (control_manager_diagnostics->flying_normally) {
        RCLCPP_INFO_THROTTLE(this_node().get_logger(), *clock_, 1000,
                             "takeoff finished");
        ...
        RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                             "waiting for the takeoff to finish");
      }
...
void AutomaticStart::changeState(LandingStates_t new_state) {
  RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                       "switching states %s -> %s", state_names[current_state],
  ...
  switch (new_state) {
    ...
    case STATE_TAKEOFF: {
      if (_pre_takeoff_sleep_ > 1.0) {
        RCLCPP_INFO(this_node().get_logger(),
                    "sleeping for %.2f secs before takeoff",
                    _pre_takeoff_sleep_);
...
bool AutomaticStart::takeoff() {
  ...
  if (response) {
    if (response.value()->success) {
      ...
      RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                            "taking off failed: %s",
    ...
    RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                          "service call for taking off failed");
  }
...
bool AutomaticStart::validateReference() {
  std::shared_ptr<mrs_msgs::srv::ValidateReference::Request> request =
      std::make_shared<mrs_msgs::srv::ValidateReference::Request>();
  ...
  if (response) {
    if (response.value()->success) {
      ...
      RCLCPP_ERROR_THROTTLE(
          this_node().get_logger(), *clock_, 1000,
          "current position is not valid (safety area, bumper)!");
      return false;
    }

  } else {
    RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                          "current position could not be validated");
    return false;
  }
}
...
bool AutomaticStart::toggleControlOutput(const bool& value) {
  RCLCPP_INFO_THROTTLE(this_node().get_logger(), *clock_, 1000,
                       "setting control output %s", value ? "ON" : "OFF");
  ...
  if (response) {
    if (response.value()->success) {
      ...
      RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                            "setting of control output failed: %s",
                            response.value()->message.c_str());
    }

  } else {
    RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                          "service call for toggling control output failed");
  }
...
bool AutomaticStart::disarm() {
  if (!hw_api_connected_) {
    RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                         "cannot disarm, missing HW API status!");
  ...
  if (offboard) {
    RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                         "cannot disarm, already in offboard mode!");
  ...
  if (response) {
    ...
    RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                          "service call for disarming failed");
  }
...
bool AutomaticStart::topicCheck(void) {
  ...
  if (_topic_check_enabled_) {
    for (int i = 0; i < int(topic_check_topics_.size()); i++) {
      if (topic_check_topics_.at(i).getTime().seconds() == 0 ||
          (clock_->now() - topic_check_topics_.at(i).getTime()).seconds() >
              _topic_check_timeout_) {
        missing_topics << std::endl
                       << "\t" << topic_check_topics_.at(i).getTopicName();
        got_topics = false;
      }
    }
  }
  ...
  if (!got_topics) {
    RCLCPP_WARN_STREAM_THROTTLE(
        this_node().get_logger(), *clock_, 1000,
        "missing data on topics: " << missing_topics.str());
  }
...
bool AutomaticStart::preflightCheckSpeed(void) {
  ...
  double speed = std::hypot(estimation_diag->velocity.linear.x,
                            estimation_diag->velocity.linear.y,
                            estimation_diag->velocity.linear.z);

  if (speed > _speed_check_max_speed_) {
    speed_check_violated_time_ = clock_->now();
    RCLCPP_WARN_THROTTLE(
        this_node().get_logger(), *clock_, 1000,
        "the estimated speed (%.2f ms^-2) is over the limit (%.2f ms^-2)",
        speed, _speed_check_max_speed_);
  }

  if (speed_check_violated_time_.seconds() > 0 &&
      (clock_->now() - speed_check_violated_time_).seconds() <
          _preflight_check_time_window_) {
    return false;
  } else {
    return true;
...
bool AutomaticStart::preflighCheckHeight(void) {
  ...
  if (height > _height_check_max_height_) {
    ...
    RCLCPP_WARN_THROTTLE(this_node().get_logger(), *clock_, 1000,
                         "the height (%.2f m) is over the limit (%.2f m)",
                         height, _height_check_max_height_);
  }

  if (height_check_violated_time_.seconds() > 0 &&
      (clock_->now() - height_check_violated_time_).seconds() <
          _preflight_check_time_window_) {
    return false;
  } else {
    return true;
...
bool AutomaticStart::preflighCheckGyro(void) {
  ...
  if (abs(gyros.x) > _gyro_check_max_rate_ ||
      abs(gyros.y) > _gyro_check_max_rate_ ||
      abs(gyros.z) > _gyro_check_max_rate_) {
  ...
  if (gyro_check_violated_time_.seconds() > 0 &&
      (clock_->now() - gyro_check_violated_time_).seconds() <
          _preflight_check_time_window_) {
    return false;
  } else {
    return true;
