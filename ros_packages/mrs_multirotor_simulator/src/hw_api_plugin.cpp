...
class Api : public mrs_uav_hw_api::MrsUavHwApi {
 ...
 private:
  ...
  mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiControlGroupCmd>
      ph_control_group_cmd_;
  mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAttitudeRateCmd>
      ph_attitude_rate_cmd_;
  mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAttitudeCmd> ph_attitude_cmd_;
  mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAccelerationHdgRateCmd>
      ph_acceleration_hdg_rate_cmd_;
  mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAccelerationHdgCmd>
      ph_acceleration_hdg_cmd_;
  mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiVelocityHdgRateCmd>
      ph_velocity_hdg_rate_cmd_;
...
void Api::initialize(
  ...
  cbgrp_subs_ = node_->create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  cbgrp_sc_ = node_->create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  cbgrp_timers_ = node_->create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  ...
  common_handlers_->main_param_loader->loadParam("custom_config",
                                                 custom_config_path);
  ...
  for (auto config_file : config_files) {
    RCLCPP_INFO(node_->get_logger(), "loading config file '%s'",
                config_file.c_str());
  ...
  local_param_loader.loadParam("input_mode/actuators",
                               (bool&)_capabilities_.accepts_actuator_cmd);
  local_param_loader.loadParam("input_mode/control_group",
                               (bool&)_capabilities_.accepts_control_group_cmd);
  local_param_loader.loadParam("input_mode/attitude_rate",
  ...
  local_param_loader.loadParam(
      "input_mode/acceleration_hdg_rate",
      (bool&)_capabilities_.accepts_acceleration_hdg_rate_cmd);
  local_param_loader.loadParam(
      "input_mode/acceleration_hdg",
      (bool&)_capabilities_.accepts_acceleration_hdg_cmd);
  local_param_loader.loadParam(
      "input_mode/velocity_hdg_rate",
      (bool&)_capabilities_.accepts_velocity_hdg_rate_cmd);
  local_param_loader.loadParam("input_mode/velocity_hdg",
  ...
  local_param_loader.loadParam("outputs/gnss",
                               (bool&)_capabilities_.produces_gnss);
  local_param_loader.loadParam("outputs/rtk",
                               (bool&)_capabilities_.produces_rtk);
  local_param_loader.loadParam("outputs/imu",
                               (bool&)_capabilities_.produces_imu);
  local_param_loader.loadParam("outputs/altitude",
                               (bool&)_capabilities_.produces_altitude);
  local_param_loader.loadParam(
      "outputs/magnetometer_heading",
      (bool&)_capabilities_.produces_magnetometer_heading);
  local_param_loader.loadParam("outputs/rc_channels",
                               (bool&)_capabilities_.produces_rc_channels);
  local_param_loader.loadParam("outputs/battery_state",
                               (bool&)_capabilities_.produces_battery_state);
  local_param_loader.loadParam("outputs/position",
                               (bool&)_capabilities_.produces_position);
  local_param_loader.loadParam("outputs/orientation",
                               (bool&)_capabilities_.produces_orientation);
  local_param_loader.loadParam("outputs/velocity",
                               (bool&)_capabilities_.produces_velocity);
  local_param_loader.loadParam("outputs/angular_velocity",
                               (bool&)_capabilities_.produces_angular_velocity);
  local_param_loader.loadParam("outputs/odometry",
                               (bool&)_capabilities_.produces_odometry);
  local_param_loader.loadParam("outputs/ground_truth",
                               (bool&)_capabilities_.produces_ground_truth);
  ...
  sh_odom_ = mrs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(
      shopts, "~/simulator_odom_in", &Api::callbackOdom, this);

  sh_imu_ = mrs_lib::SubscriberHandler<sensor_msgs::msg::Imu>(
      shopts, "~/simulator_imu_in", &Api::callbackImu, this);

  sh_range_ = mrs_lib::SubscriberHandler<sensor_msgs::msg::Range>(
      shopts, "~/simulator_rangefinder_in", &Api::callbackRangefinder, this);

  // | ----------------------- publishers ----------------------- |

  if (_capabilities_.accepts_actuator_cmd) {
    ph_actuators_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiActuatorCmd>(
            node_, "~/simulator_actuators_cmd_out");
  }

  if (_capabilities_.accepts_control_group_cmd) {
    ph_control_group_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiControlGroupCmd>(
            node_, "~/simulator_control_group_cmd_out");
  }

  if (_capabilities_.accepts_attitude_rate_cmd) {
    ph_attitude_rate_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAttitudeRateCmd>(
            node_, "~/simulator_attitude_rate_cmd_out");
  }

  if (_capabilities_.accepts_attitude_cmd) {
    ph_attitude_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAttitudeCmd>(
            node_, "~/simulator_attitude_cmd_out");
  }

  if (_capabilities_.accepts_acceleration_hdg_rate_cmd) {
    ph_acceleration_hdg_rate_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAccelerationHdgRateCmd>(
            node_, "~/simulator_acceleration_hdg_rate_cmd_out");
  }

  if (_capabilities_.accepts_acceleration_hdg_cmd) {
    ph_acceleration_hdg_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiAccelerationHdgCmd>(
            node_, "~/simulator_acceleration_hdg_cmd_out");
  }

  if (_capabilities_.accepts_velocity_hdg_rate_cmd) {
    ph_velocity_hdg_rate_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiVelocityHdgRateCmd>(
            node_, "~/simulator_velocity_hdg_rate_cmd_out");
  }

  if (_capabilities_.accepts_velocity_hdg_cmd) {
    ph_velocity_hdg_cmd_ =
        mrs_lib::PublisherHandler<mrs_msgs::msg::HwApiVelocityHdgCmd>(
            node_, "~/simulator_velocity_hdg_cmd_out");
  }
  ...
  if (_feedforward_enabled_) {
    ph_tracker_cmd_ = mrs_lib::PublisherHandler<mrs_msgs::msg::TrackerCommand>(
        node_, "~/simulator_tracker_cmd_out");
  }
...

  {
    ...
    timer_main_ =
        std::make_shared<TimerType>(opts, rclcpp::Rate(10.0), callback_fcn);
  }
...
void Api::destroy() { timer_main_->stop(); }
...
std::tuple<bool, std::string> Api::callbackArming(
...
std::tuple<bool, std::string> Api::callbackOffboard(void) {
  ...
  auto last_cmd_time =
      mrs_lib::get_mutexed(mutex_last_cmd_time_, last_cmd_time_);
...
bool Api::callbackControlGroupCmd(
    const mrs_msgs::msg::HwApiControlGroupCmd::ConstSharedPtr msg) {
  if (!_capabilities_.accepts_control_group_cmd) {
    return false;
  }
...
bool Api::callbackAttitudeRateCmd(
    const mrs_msgs::msg::HwApiAttitudeRateCmd::ConstSharedPtr msg) {
  if (!_capabilities_.accepts_attitude_rate_cmd) {
    return false;
  }
...
bool Api::callbackAccelerationHdgRateCmd(
    const mrs_msgs::msg::HwApiAccelerationHdgRateCmd::ConstSharedPtr msg) {
  if (!_capabilities_.accepts_acceleration_hdg_rate_cmd) {
    return false;
  }
...
bool Api::callbackAccelerationHdgCmd(
    const mrs_msgs::msg::HwApiAccelerationHdgCmd::ConstSharedPtr msg) {
  if (!_capabilities_.accepts_acceleration_hdg_cmd) {
    return false;
  }
...
bool Api::callbackVelocityHdgRateCmd(
    const mrs_msgs::msg::HwApiVelocityHdgRateCmd::ConstSharedPtr msg) {
  if (!_capabilities_.accepts_velocity_hdg_rate_cmd) {
    return false;
  }
...
bool Api::callbackVelocityHdgCmd(
    const mrs_msgs::msg::HwApiVelocityHdgCmd::ConstSharedPtr msg) {
  if (!_capabilities_.accepts_velocity_hdg_cmd) {
    return false;
  }
...
void Api::callbackOdom(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
  ...
  if (_capabilities_.produces_gnss) {
    ...
    mrs_lib::UTMtoLL(odom->pose.pose.position.y + _utm_y_,
                     odom->pose.pose.position.x + _utm_x_, _utm_zone_, lat,
                     lon);
  ...
  if (_capabilities_.produces_magnetometer_heading) {
    ...
    try {
      heading =
          mrs_lib::AttitudeConverter(odom->pose.pose.orientation).getHeading();
...
void Api::timeoutInputs(void) {
  ...
  if (last_cmd_time_.seconds() > 0 &&
      (clock_->now() - last_cmd_time).seconds() > _input_timeout_) {
    offboard_ = false;
    mode_ = "NORMAL";
  }
}
