...
class StateMachine {
 ...
 public:
  StateMachine(const rclcpp::Node::SharedPtr& node,
  ...
  bool isInPublishableState() const {
    const SMState_t current_state =
        mrs_lib::get_mutexed(mtx_state_, current_state_);
    return current_state == READY_FOR_FLIGHT_STATE ||
           current_state == TAKING_OFF_STATE || current_state == HOVER_STATE ||
           current_state == FLYING_STATE || current_state == LANDING_STATE ||
           current_state == DUMMY_STATE || current_state == FAILSAFE_STATE;
  }
  ...
  bool isInTheAir() const {
    const SMState_t current_state =
        mrs_lib::get_mutexed(mtx_state_, current_state_);
    return current_state == TAKING_OFF_STATE || current_state == HOVER_STATE ||
           current_state == FLYING_STATE || current_state == LANDING_STATE;
  }
  ...
  bool changeState(const SMState_t& target_state) {
    if (target_state == current_state_) {
      RCLCPP_WARN(
          node_->get_logger(), "[%s]: requested change to same state %s -> %s",
          getPrintName().c_str(), getStateAsString(current_state_).c_str(),
    ...
    switch (target_state) {
      case UNINITIALIZED_STATE: {
        RCLCPP_ERROR_THROTTLE(
            ...
            "[%s]: transition to %s is not possible from any state",
            getPrintName().c_str(),
      ...
      case INITIALIZED_STATE: {
        if (current_state_ != UNINITIALIZED_STATE) {
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s",
              getPrintName().c_str(),
      ...
      case READY_FOR_FLIGHT_STATE: {
        if (current_state_ != INITIALIZED_STATE &&
            current_state_ != LANDED_STATE &&
            current_state_ != ESTIMATOR_SWITCHING_STATE) {
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s, %s or %s",
              getPrintName().c_str(),
      ...
      case TAKING_OFF_STATE: {
        if (current_state_ != READY_FOR_FLIGHT_STATE &&
          ...
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s or %s",
              getPrintName().c_str(),
              getStateAsString(TAKING_OFF_STATE).c_str(),
              getStateAsString(READY_FOR_FLIGHT_STATE).c_str(),
              getStateAsString(ESTIMATOR_SWITCHING_STATE).c_str());
          return false;
        }
      ...
      case FLYING_STATE: {
        if (current_state_ != TAKING_OFF_STATE &&
            current_state_ != READY_FOR_FLIGHT_STATE &&
            current_state_ != HOVER_STATE &&
            current_state_ != ESTIMATOR_SWITCHING_STATE) {
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s or %s or %s",
              getPrintName().c_str(), getStateAsString(FLYING_STATE).c_str(),
              getStateAsString(TAKING_OFF_STATE).c_str(),
      ...
      case HOVER_STATE: {
        if (current_state_ != FLYING_STATE &&
          ...
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s or %s",
              getPrintName().c_str(), getStateAsString(HOVER_STATE).c_str(),
      ...
      case ESTIMATOR_SWITCHING_STATE: {
        if (current_state_ != READY_FOR_FLIGHT_STATE &&
            current_state_ != TAKING_OFF_STATE &&
            current_state_ != HOVER_STATE && current_state_ != FLYING_STATE &&
            current_state_ != LANDING_STATE) {
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s, %s, %s, %s or "
              "%s",
              getPrintName().c_str(),
              getStateAsString(ESTIMATOR_SWITCHING_STATE).c_str(),
              getStateAsString(READY_FOR_FLIGHT_STATE).c_str(),
      ...
      case LANDING_STATE: {
        if (current_state_ != FLYING_STATE && current_state_ != HOVER_STATE &&
          ...
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s, %s or %s",
              getPrintName().c_str(), getStateAsString(LANDING_STATE).c_str(),
              getStateAsString(FLYING_STATE).c_str(),
              getStateAsString(HOVER_STATE).c_str(),
              getStateAsString(ESTIMATOR_SWITCHING_STATE).c_str());
          return false;
      ...
      case LANDED_STATE: {
        if (current_state_ != LANDING_STATE) {
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s",
              getPrintName().c_str(), getStateAsString(LANDED_STATE).c_str(),
      ...
      case DUMMY_STATE: {
        if (current_state_ != INITIALIZED_STATE) {
          RCLCPP_ERROR_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "[%s]: transition to %s is possible only from %s",
              getPrintName().c_str(), getStateAsString(DUMMY_STATE).c_str(),
              getStateAsString(INITIALIZED_STATE).c_str());
          return false;
        }
        break;
      ...
      case EMERGENCY_STATE: {
        RCLCPP_WARN(node_->get_logger(), "[%s]: transition to %s",
                    getPrintName().c_str(),
                    getStateAsString(EMERGENCY_STATE).c_str());
        break;
      }
      ...
      case ERROR_STATE: {
        RCLCPP_WARN(node_->get_logger(), "[%s]: transition to %s",
                    getPrintName().c_str(),
                    getStateAsString(ERROR_STATE).c_str());
        break;
      }
      ...
      case FAILSAFE_STATE: {
        RCLCPP_WARN(node_->get_logger(), "[%s]: transition to %s",
                    getPrintName().c_str(),
                    getStateAsString(FAILSAFE_STATE).c_str());
        break;
      }
      ...
      default: {
        RCLCPP_ERROR_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "[%s]: rejected transition to unknown state id %d",
            getPrintName().c_str(), target_state);
        return false;
        break;
      }
    }
    ...
    RCLCPP_INFO(
        node_->get_logger(), "[%s]: successfully changed states %s -> %s",
        getPrintName().c_str(), getStateAsString(previous_state_).c_str(),
  ...
  void changeToPreSwitchState() { changeState(pre_switch_state_); }
  /*//}*/
 ...
 private:
  ...
  std::string getPrintName() const { return name_; }
...
class EstimationManager : public mrs_lib::Node {
 private:
  ...
  mrs_lib::PublisherHandler<mrs_msgs::msg::EstimationDiagnostics>
      ph_diagnostics_;
  ...
  mrs_lib::PublisherHandler<nav_msgs::msg::Odometry>
      ph_odom_slow_;  // use topic throttler instead?
  ...
  mrs_lib::PublisherHandler<geometry_msgs::msg::QuaternionStamped>
      ph_orientation_;
  ...
  bool callbackResetEstimator(
      const std::shared_ptr<mrs_msgs::srv::String::Request> request,
      const std::shared_ptr<mrs_msgs::srv::String::Response> response);

  mrs_lib::ServiceServerHandler<mrs_msgs::srv::ReferenceStampedSrv>
      srvs_set_world_origin_;
  bool callbackSetWorldOrigin(
  ...
  bool callbackToggleServiceCallbacks(
  ...
  std::unique_ptr<pluginlib::ClassLoader<mrs_uav_managers::StateEstimator>>
      state_estimator_loader_;  // pluginlib loader of dynamically loaded
                                // estimators
  std::vector<std::string> estimator_names_;  // list of estimator names
  std::vector<std::shared_ptr<mrs_uav_managers::StateEstimator>>
      estimator_list_;  // list of estimators
  ...
  std::unique_ptr<pluginlib::ClassLoader<mrs_uav_managers::AglEstimator>>
      agl_estimator_loader_;  // pluginlib loader of dynamically loaded
                              // estimators
  ...
  void switchToEstimator(
      const std::shared_ptr<mrs_uav_managers::StateEstimator>&
          target_estimator);
...
EstimationManager::EstimationManager(rclcpp::NodeOptions options)
    : mrs_lib::Node("estimation_manager", options) {
  node_ = this_node_ptr();
  clock_ = node_->get_clock();
  ...
  sh_control_manager_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::ControlManagerDiagnostics>(
          shopts, "~/control_manager_diagnostics_in");

  time_preinit_started_ = clock_->now();

  timer_preinit_ = node_->create_wall_timer(
      std::chrono::duration<double>(1.0),
      std::bind(&EstimationManager::timerPreinit, this));
}
...
void EstimationManager::timerPreinit() {
  ...
  if (!sh_hw_api_capabilities_.hasMsg()) {
    RCLCPP_INFO(node_->get_logger(),
                "%s hw_api_capabilities message at topic: %s",
  ...
  if (!sh_control_manager_diag_.hasMsg() &&
      (clock_->now() - time_preinit_started_).seconds() < 10.0) {
    RCLCPP_INFO(node_->get_logger(),
                "%s control_manager_diagnostics message at topic: %s",
                Support::waiting_for_string.c_str(),
...
void EstimationManager::initialize() {
  ...
  if (Support::toLowercase(world_origin_units) == "utm") {
    ...
    RCLCPP_ERROR(node_->get_logger(),
                 "world_origin_units must be (\"UTM\"|\"LATLON\"). Got '%s'",
                 world_origin_units.c_str());
    rclcpp::shutdown();
    exit(1);
  }
  ...
  if (!is_origin_param_ok) {
    RCLCPP_ERROR(node_->get_logger(),
                 "Could not load all mandatory parameters from world file. "
                 "Please check your world file.");
    rclcpp::shutdown();
    exit(1);
  }
  /*//}*/
  ...
  param_loader.loadParam(
      "mrs_uav_managers/uav_manager/takeoff/during_takeoff/tracker",
      takeoff_tracker_name_);
  param_loader.loadParam(
      "mrs_uav_managers/uav_manager/midair_activation/after_activation/tracker",
      after_midair_activation_tracker_name_);
  /*//}*/

  param_loader.setPrefix(ch_->package_name + "/" +
                         Support::toSnakeCase(ch_->nodelet_name) + "/");
  ...
  param_loader.loadParam("debug_topics/covariance",
                         ch_->debug_topics.covariance);
  param_loader.loadParam("debug_topics/innovation",
                         ch_->debug_topics.innovation);
  param_loader.loadParam("debug_topics/diagnostics", ch_->debug_topics.diag);
  param_loader.loadParam("debug_topics/correction",
                         ch_->debug_topics.correction);
  param_loader.loadParam("debug_topics/correction_delay",
                         ch_->debug_topics.corr_delay);
  /*//}*/
  ...
  mrs_msgs::msg::HwApiCapabilities::ConstSharedPtr hw_api_capabilities =
      sh_hw_api_capabilities_.getMsg();
  ...
  if (!sh_control_manager_diag_.hasMsg()) {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[%s]: received control_manager_diagnostics",
                Support::waiting_for_string.c_str());
    mrs_msgs::msg::ControlManagerDiagnostics::ConstSharedPtr
        control_manager_diag_msg = sh_control_manager_diag_.getMsg();
    ch_->desired_uav_state_rate =
        control_manager_diag_msg->desired_uav_state_rate;
  }

  RCLCPP_INFO(node_->get_logger(), "The estimation is running at: %.2f Hz",
              ch_->desired_uav_state_rate);
  ...
  sh_control_input_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::EstimatorInput>(
      shopts, "~/control_input_in");
  ...
  state_estimator_loader_ = std::make_unique<
      pluginlib::ClassLoader<mrs_uav_managers::StateEstimator>>(
      "mrs_uav_managers", "mrs_uav_managers::StateEstimator");
  ...
  for (size_t i = 0; i < estimator_names_.size(); i++) {
    ...
    try {
      RCLCPP_INFO(node_->get_logger(), "loading the estimator '%s'",
                  address.c_str());
      estimator_list_.push_back(
          state_estimator_loader_->createSharedInstance(address.c_str()));
    } catch (pluginlib::CreateClassException& ex1) {
      RCLCPP_ERROR(node_->get_logger(),
                   "CreateClassException for the estimator '%s'",
                   address.c_str());
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "PluginlibException for the estimator '%s'",
                   address.c_str());
  ...
  if (!is_using_agl_estimator_) {
    RCLCPP_WARN(node_->get_logger(),
                "not using AGL estimator for min height safe checking");
  }

  if (is_using_agl_estimator_) {
    agl_estimator_loader_ = std::make_unique<
        pluginlib::ClassLoader<mrs_uav_managers::AglEstimator>>(
        "mrs_uav_managers", "mrs_uav_managers::AglEstimator");
    ...
    try {
      RCLCPP_INFO(node_->get_logger(), "loading the estimator '%s'",
                  address.c_str());
      est_alt_agl_ =
          agl_estimator_loader_->createSharedInstance(address.c_str());
    } catch (pluginlib::CreateClassException& ex1) {
      RCLCPP_ERROR(node_->get_logger(),
                   "CreateClassException for the estimator '%s'",
                   address.c_str());
  ...
  if (!initial_estimator_found) {
    RCLCPP_ERROR(node_->get_logger(),
                 "initial estimator %s could not be found among loaded "
                 "estimators. shutting down",
  ...
  for (auto estimator : estimator_list_) {
    rclcpp::Node::SharedPtr subnode =
        node_->create_sub_node(estimator->getName());

    // create private handlers
    std::shared_ptr<mrs_uav_managers::estimation_manager::PrivateHandlers_t>
        ph = std::make_shared<
            mrs_uav_managers::estimation_manager::PrivateHandlers_t>();

    ph->loadConfigFile = std::bind(&EstimationManager::loadConfigFile, this,
                                   std::placeholders::_1);
    ph->param_loader =
        std::make_unique<mrs_lib::ParamLoader>(subnode, estimator->getName());
    ph->param_loader->copyYamls(param_loader);
    ph->param_loader->setPrefix(ch_->package_name + "/" +
                                Support::toSnakeCase(ch_->nodelet_name) + "/");

    try {
      RCLCPP_INFO(node_->get_logger(), "initializing the estimator '%s'",
                  estimator->getName().c_str());
      estimator->initialize(subnode, ch_, ph);
    } catch (std::runtime_error& ex) {
      RCLCPP_ERROR(node_->get_logger(),
                   "exception caught during estimator initialization: '%s'",
                   ex.what());
      rclcpp::shutdown();
      exit(1);
    }
    ...
    if (!estimator->isCompatibleWithHwApi(hw_api_capabilities)) {
      RCLCPP_ERROR(
          node_->get_logger(),
          "estimator %s is not compatible with the hw api. Shutting down.",
          estimator->getName().c_str());
      rclcpp::shutdown();
      exit(1);
    }
  ...
  if (is_using_agl_estimator_) {
    rclcpp::Node::SharedPtr subnode =
        node_->create_sub_node(est_alt_agl_->getName());
    ...
    ph->loadConfigFile = std::bind(&EstimationManager::loadConfigFile, this,
                                   std::placeholders::_1);
    ph->param_loader = std::make_unique<mrs_lib::ParamLoader>(
        subnode, est_alt_agl_->getName());
    ph->param_loader->copyYamls(param_loader);
    ph->param_loader->setPrefix(ch_->package_name + "/" +
                                Support::toSnakeCase(ch_->nodelet_name) + "/");
  ...
  ph_uav_state_ = mrs_lib::PublisherHandler<mrs_msgs::msg::UavState>(
      node_, "~/uav_state_out");
  ph_odom_main_ = mrs_lib::PublisherHandler<nav_msgs::msg::Odometry>(
      node_, "~/odom_main_out");
  ph_innovation_ = mrs_lib::PublisherHandler<nav_msgs::msg::Odometry>(
      node_, "~/innovation_out");
  ph_diagnostics_ =
      mrs_lib::PublisherHandler<mrs_msgs::msg::EstimationDiagnostics>(
          node_, "~/diagnostics_out");
  ph_max_flight_z_ = mrs_lib::PublisherHandler<mrs_msgs::msg::Float64Stamped>(
      node_, "~/max_flight_z_agl_out");
  ph_altitude_agl_ = mrs_lib::PublisherHandler<mrs_msgs::msg::Float64Stamped>(
      node_, "~/height_agl_out");
...

  {
    std::function<void()> callback_fcn =
        std::bind(&EstimationManager::timerPublish, this);

    timer_publish_ = std::make_shared<TimerType>(
        opts, rclcpp::Rate(ch_->desired_uav_state_rate), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&EstimationManager::timerPublishDiagnostics, this);

    timer_publish_diagnostics_ = std::make_shared<TimerType>(
        opts, rclcpp::Rate(ch_->desired_diagnostics_rate), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&EstimationManager::timerCheckHealth, this);

    timer_check_health_ = std::make_shared<TimerType>(
        opts, rclcpp::Rate(ch_->desired_uav_state_rate), callback_fcn);
  }
  ...
  srvch_failsafe_ = mrs_lib::ServiceClientHandler<std_srvs::srv::Trigger>(
      node_, "~/failsafe_out", cbkgrp_sc_);
  srvch_set_world_origin_ =
      mrs_lib::ServiceClientHandler<mrs_msgs::srv::ReferenceStampedSrv>(
  ...
  srvs_change_estimator_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::String>(
      node_, "~/change_estimator_in",
      std::bind(&EstimationManager::callbackChangeEstimator, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);

  srvs_reset_estimator_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::String>(
      node_, "~/reset_estimator_in",
      std::bind(&EstimationManager::callbackResetEstimator, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);

  srvs_set_world_origin_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ReferenceStampedSrv>(
          node_, "~/set_world_origin_in",
          std::bind(&EstimationManager::callbackSetWorldOrigin, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);

  srvs_toggle_callbacks_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
          node_, "~/toggle_service_callbacks_in",
          std::bind(&EstimationManager::callbackToggleServiceCallbacks, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ...
  std::string filepath;
  const std::string time_logger_filepath =
      ament_index_cpp::get_package_share_directory(package_name_) +
      "/scope_timer/scope_timer.txt";
  ch_->scope_timer.logger = std::make_shared<mrs_lib::ScopeTimerLogger>(
      node_, time_logger_filepath, ch_->scope_timer.enabled);

  /*//}*/

  if (!param_loader.loadedSuccessfully()) {
    RCLCPP_ERROR(node_->get_logger(),
                 "Could not load all non-optional parameters. Shutting down.");
    rclcpp::shutdown();
    exit(1);
  }
...
void EstimationManager::timerPublish() {
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "EstimationManager::timerPublish",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);

  if (sm_->isInState(StateMachine::ESTIMATOR_SWITCHING_STATE)) {
    RCLCPP_WARN(node_->get_logger(),
                "Not publishing during estimator switching.");
    return;
  }

  if (!sm_->isInPublishableState()) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "not publishing uav state in %s",
                         sm_->getCurrentStateString().c_str());
    return;
  }
  ...
  if (ret) {
    ...
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "Active estimator did not provide uav_state.");
    return;
  }
  ...
  const std::vector<double> pose_covariance =
      active_estimator_->getPoseCovariance();
  for (size_t i = 0; i < pose_covariance.size(); i++) {
    odom_main.pose.covariance[i] = pose_covariance[i];
  }

  const std::vector<double> twist_covariance =
      active_estimator_->getTwistCovariance();
...
void EstimationManager::timerPublishDiagnostics() {
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "EstimationManager::timerPublishDiagnostics",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);

  if (sm_->isInState(StateMachine::ESTIMATOR_SWITCHING_STATE)) {
    RCLCPP_WARN(node_->get_logger(),
                "Not publishing diagnostics during estimator switching.");
    return;
  }
  ...
  RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 5000,
                       "%s. pos: [%.2f, %.2f, %.2f] m. Estimator: %s. Max. z.: "
                       "%.2f m. Estimator switches: %d.",
                       sm_->getCurrentStateString().c_str(),
                       uav_state.pose.position.x, uav_state.pose.position.y,
                       uav_state.pose.position.z,
                       active_estimator_->getName().c_str(), max_flight_z_,
                       estimator_switch_count_);
}
...
void EstimationManager::timerCheckHealth() {
  ...
  mrs_lib::ScopeTimer scope_timer =
      mrs_lib::ScopeTimer(node_, "EstimationManager::timerCheckHealth",
                          ch_->scope_timer.logger, ch_->scope_timer.enabled);
  ...
  for (auto estimator : estimator_list_) {
    if (estimator->isReady()) {
      try {
        RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                             "starting the estimator '%s'",
                             estimator->getName().c_str());
        estimator->start();
      } catch (std::runtime_error& ex) {
        RCLCPP_ERROR(node_->get_logger(),
                     "exception caught during estimator starting: '%s'",
                     ex.what());
        rclcpp::shutdown();
        exit(1);
      }
    ...
    if (estimator->isRunning() && estimator->getName() != "dummy" &&
        estimator->getName() != "ground_truth") {
  ...
  if (!callbacks_disabled_by_service_ &&
      (sm_->isInState(StateMachine::FLYING_STATE) ||
  ...
  if (sm_->isInState(StateMachine::INITIALIZED_STATE) &&
      initial_estimator_->isRunning()) {
    std::scoped_lock lock(mutex_active_estimator_);
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "activating the initial estimator %s",
    ...
    if (active_estimator_->getName() == "dummy") {
      ...
      if (!is_using_agl_estimator_ || est_alt_agl_->isRunning()) {
        ...
        RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                             "%s agl estimator: %s to be running",
                             Support::waiting_for_string.c_str(),
                             est_alt_agl_->getName().c_str());
      }
    }
  ...
  if (sm_->isInState(StateMachine::ESTIMATOR_SWITCHING_STATE)) {
    if (switchToHealthyEstimator()) {
      ...
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "Cannot switch to any healthy estimator. Triggering failsafe.");
  ...
  if (sm_->isInState(StateMachine::FAILSAFE_STATE)) {
    if (!failsafe_call_succeeded_ && callFailsafeService()) {
      RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "failsafe called successfully");
      failsafe_call_succeeded_ = true;
    }
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "we are in failsafe state");
  }
  ...
  if (sm_->isInState(StateMachine::FLYING_STATE)) {
    if (!sh_control_input_.hasMsg()) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "not received control input since starting EstimationManager, "
          "estimation suboptimal, potentially unstable");
    } else if ((clock_->now() - sh_control_input_.lastMsgTime()).seconds() >
               0.1) {
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "not received control input for %.4fs, estimation suboptimal, "
...
void EstimationManager::shutdown() {
  ...
  for (int i = 0; i < int(estimator_list_.size()); i++) {
    std::cout << "EstimationManager: stopping " << estimator_names_[i]
              << std::endl;

    estimator_list_.at(i)->pause();

    std::cout << "EstimationManager: reseting pointer to "
              << estimator_names_[i] << std::endl;

    estimator_list_.at(i).reset();

    std::cout << "EstimationManager: pointer to " << estimator_names_[i]
              << " was reset" << std::endl;
  }
  ...
  if (est_alt_agl_) {
    ...
    std::cout << "EstimationManager: reseting pointer to agl estimator"
              << std::endl;

    est_alt_agl_.reset();

    std::cout << "EstimationManager: pointer to agl estimator was reset"
              << std::endl;
  }
...
bool EstimationManager::callbackChangeEstimator(
    const std::shared_ptr<mrs_msgs::srv::String::Request> request,
    const std::shared_ptr<mrs_msgs::srv::String::Response> response) {
  if (!sm_->isInitialized()) {
    return false;
  ...
  if (!callbacks_enabled_ && active_estimator_->getName() != "vins_kickoff") {
    ...
    RCLCPP_WARN(node_->get_logger(),
                "Ignoring service call. Callbacks are disabled.");
    return true;
  }

  // switching into these estimators during flight is dangerous with realhw, so
  // we don't allow it
  if (request->value == "dummy" || request->value == "ground_truth" ||
      request->value == "vins_kickoff") {
    response->success = false;
    std::stringstream ss;
    ss << "Switching to " << request->value << " estimator is not allowed.";
    response->message = ss.str();
    RCLCPP_WARN(node_->get_logger(),
                "Switching to %s estimator is not allowed.",
                request->value.c_str());
    return true;
  }
  ...
  if (target_estimator_found) {
    if (target_estimator->isRunning()) {
      ...
      RCLCPP_WARN(node_->get_logger(),
                  "Switch to not running estimator %s requested",
                  request->value.c_str());
      response->success = false;
    ...
    RCLCPP_WARN(node_->get_logger(), "Switch to invalid estimator %s requested",
                request->value.c_str());
    response->success = false;
...
bool EstimationManager::callbackResetEstimator(
    const std::shared_ptr<mrs_msgs::srv::String::Request> request,
    const std::shared_ptr<mrs_msgs::srv::String::Response> response) {
  if (!sm_->isInitialized()) {
    return false;
  ...
  if (target_estimator_found) {
    if (target_estimator->getName() == active_estimator_->getName()) {
      ...
      RCLCPP_WARN(node_->get_logger(),
                  "Ignoring service call. Cannot reset active estimator.");
      return true;
    }
    ...
    RCLCPP_WARN(node_->get_logger(), "Reset of invalid estimator %s requested",
                request->value.c_str());
    response->success = false;
    response->message = ("Not a valid estimator type");
    return true;
...
bool EstimationManager::callbackSetWorldOrigin(
...
bool EstimationManager::callbackToggleServiceCallbacks(
  ...
  if (!sm_->isInitialized()) {
    RCLCPP_ERROR(node_->get_logger(),
                 "service for toggling callbacks is not available before "
                 "initialization.");
    return false;
  }
  ...
  response->success = true;
  response->message =
      (callbacks_disabled_by_service_ ? "Service callbacks disabled"
                                      : "Service callbacks enabled");
...
bool EstimationManager::switchToHealthyEstimator() {
  // available estimators should be specified in decreasing priority order in
  // config file
  for (auto estimator : estimator_list_) {
    if (estimator->isRunning()) {
...
void EstimationManager::switchToEstimator(
    const std::shared_ptr<mrs_uav_managers::StateEstimator>& target_estimator) {
  std::scoped_lock lock(mutex_active_estimator_);

  RCLCPP_INFO(node_->get_logger(), "switching estimator from %s to %s",
              active_estimator_->getName().c_str(),
              target_estimator->getName().c_str());
...
std::string EstimationManager::getName() const { return nodelet_name_; }
...
bool EstimationManager::loadConfigFile(const std::string& file_path) {
  ...
  RCLCPP_INFO(node_->get_logger(), "'%s' under the namespace '%s'",
              file_path.c_str(), name_space.c_str());
  ...
  // load the user-requested file
  {
    ...
    if (result != 0) {
      RCLCPP_ERROR(node_->get_logger(), "failed to load '%s'",
                   file_path.c_str());
      return false;
    }
  }
  ...
  if (_platform_config_ != "") {
    std::string command =
        "rosparam load " + _platform_config_ + " " + name_space;
    int result = std::system(command.c_str());

    if (result != 0) {
      RCLCPP_ERROR(node_->get_logger(),
                   "failed to load the platform config file '%s'",
                   _platform_config_.c_str());
      return false;
    }
  }
  ...
  if (_custom_config_ != "") {
    ...
    if (result != 0) {
      RCLCPP_ERROR(node_->get_logger(),
                   "failed to load the custom config file '%s'",
                   _custom_config_.c_str());
      return false;
    }
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    mrs_uav_managers::estimation_manager::EstimationManager)
