...
class ControllerParams {
 public:
  ControllerParams(std::string address, std::string name_space,
                   double eland_threshold, double failsafe_threshold,
...
ControllerParams::ControllerParams(std::string address, std::string name_space,
                                   double eland_threshold,
                                   double failsafe_threshold,
                                   double odometry_innovation_threshold,
                                   bool human_switchable) {
...
class TrackerParams {
 public:
  TrackerParams(std::string address, std::string name_space,
                bool human_switchable);
...
TrackerParams::TrackerParams(std::string address, std::string name_space,
                             bool human_switchable) {
  this->address = address;
  this->name_space = name_space;
  this->human_switchable = human_switchable;
...
class ControlManager : public mrs_lib::Node {
 ...
 private:
  ...
  std::unique_ptr<pluginlib::ClassLoader<mrs_uav_managers::Tracker>>
      tracker_loader_;  // pluginlib loader of dynamically loaded trackers
  std::vector<std::string> _tracker_names_;  // list of tracker names
  std::map<std::string, TrackerParams>
      trackers_;  // map between tracker names and tracker param
  std::vector<std::shared_ptr<mrs_uav_managers::Tracker>>
      tracker_list_;  // list of trackers, routines are callable from this
  std::mutex mutex_tracker_list_;

  // | ------------- dynamic loading of controllers ------------- |

  std::unique_ptr<pluginlib::ClassLoader<mrs_uav_managers::Controller>>
      controller_loader_;  // pluginlib loader of dynamically loaded controllers
  std::vector<std::string> _controller_names_;  // list of controller names
  std::map<std::string, ControllerParams>
      controllers_;  // map between controller names and controller params
  std::vector<std::shared_ptr<mrs_uav_managers::Controller>>
      controller_list_;  // list of controllers, routines are callable from this
  std::mutex mutex_controller_list_;

  // | ------------------------- HW API ------------------------- |

  mrs_lib::SubscriberHandler<mrs_msgs::msg::HwApiCapabilities>
  ...
  std::tuple<bool, std::string> switchController(
  ...
  std::string
      _null_tracker_name_;  // null tracker is active when UAV is not in the air
  std::string
      _ehover_tracker_name_;  // ehover tracker is used for emergency hovering
  std::string
      _landoff_tracker_name_;  // landoff is used for landing and takeoff

  // names of important controllers
  std::string
      _failsafe_controller_name_;  // controller used for feed-forward failsafe
  ...
  double _uav_state_max_missing_time_ =
      0;  // how long should we tolerate missing state estimate?
  double uav_roll_ = 0;
  double uav_pitch_ = 0;
  double uav_yaw_ = 0;
  ...
  std::shared_ptr<mrs_uav_managers::control_manager::CommonHandlers_t>
      common_handlers_;
  ...
  mrs_lib::PublisherHandler<mrs_msgs::msg::ControlManagerDiagnostics>
      ph_diagnostics_;
  ...
  mrs_lib::ServiceServerHandler<mrs_msgs::srv::DynamicsConstraintsSrv>
      ss_set_constraints_;
  ...
  mrs_lib::ServiceServerHandler<mrs_msgs::srv::VelocityReferenceStampedSrv>
      ss_velocity_reference_;
  mrs_lib::SubscriberHandler<mrs_msgs::msg::VelocityReferenceStamped>
      sh_velocity_reference_;

  // trajectory tracking
  mrs_lib::ServiceServerHandler<mrs_msgs::srv::TrajectoryReferenceSrv>
      ss_trajectory_reference_;
  mrs_lib::SubscriberHandler<mrs_msgs::msg::TrajectoryReference>
      sh_trajectory_reference_;

  mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>
      ss_start_trajectory_tracking_;
  mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>
      ss_stop_trajectory_tracking_;
  mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>
      ss_resume_trajectory_tracking_;
  mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>
      ss_goto_trajectory_start_;
  ...
  mrs_lib::ServiceServerHandler<mrs_msgs::srv::ValidateReference>
      ss_validate_reference_;
  mrs_lib::ServiceServerHandler<mrs_msgs::srv::ValidateReference>
      ss_validate_reference_2d_;
  mrs_lib::ServiceServerHandler<mrs_msgs::srv::ValidateReferenceArray>
      ss_validate_reference_array_;
  ...
  double _odometry_innovation_threshold_ =
      0;  // innovation size for triggering eland
  ...
  bool callbackSwitchTracker(
      const std::shared_ptr<mrs_msgs::srv::String::Request> request,
      const std::shared_ptr<mrs_msgs::srv::String::Response> response);
  bool callbackSwitchController(
      const std::shared_ptr<mrs_msgs::srv::String::Request> request,
      const std::shared_ptr<mrs_msgs::srv::String::Response> response);
  bool callbackTrackerResetStatic(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);

  // reference callbacks
  void callbackReferenceTopic(
      const mrs_msgs::msg::ReferenceStamped::ConstSharedPtr msg);
  void callbackVelocityReferenceTopic(
      const mrs_msgs::msg::VelocityReferenceStamped::ConstSharedPtr msg);
  void callbackTrajectoryReferenceTopic(
      const mrs_msgs::msg::TrajectoryReference::ConstSharedPtr msg);
  bool callbackGoto(
      const std::shared_ptr<mrs_msgs::srv::Vec4::Request> request,
      const std::shared_ptr<mrs_msgs::srv::Vec4::Response> response);
  bool callbackGotoFcu(
      const std::shared_ptr<mrs_msgs::srv::Vec4::Request> request,
      const std::shared_ptr<mrs_msgs::srv::Vec4::Response> response);
  bool callbackGotoRelative(
      const std::shared_ptr<mrs_msgs::srv::Vec4::Request> request,
      const std::shared_ptr<mrs_msgs::srv::Vec4::Response> response);
  bool callbackGotoAltitude(
      const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
      const std::shared_ptr<mrs_msgs::srv::Vec1::Response> response);
  bool callbackSetHeading(
      const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
      const std::shared_ptr<mrs_msgs::srv::Vec1::Response> response);
  bool callbackSetHeadingRelative(
      const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
      const std::shared_ptr<mrs_msgs::srv::Vec1::Response> response);
  bool callbackReferenceService(
      ...
      const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Response>
          response);
  bool callbackVelocityReferenceService(
      const std::shared_ptr<mrs_msgs::srv::VelocityReferenceStampedSrv::Request>
          request,
      const std::shared_ptr<
          mrs_msgs::srv::VelocityReferenceStampedSrv::Response>
          response);
  bool callbackTrajectoryReferenceService(
      const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Request>
          request,
      const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Response>
          response);
  bool callbackEmergencyReference(
      const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Request>
          request,
      const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Response>
          response);
  ...
  bool callbackHover(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackStartTrajectoryTracking(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackStopTrajectoryTracking(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackResumeTrajectoryTracking(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackGotoTrajectoryStart(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackEHover(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackFailsafe(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackFailsafeEscalating(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);
  bool callbackEland(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  ...
  bool callbackGetMinZ(
      const std::shared_ptr<mrs_msgs::srv::GetFloat64::Request> request,
      const std::shared_ptr<mrs_msgs::srv::GetFloat64::Response> response);

  bool callbackValidateReference(
      const std::shared_ptr<mrs_msgs::srv::ValidateReference::Request> request,
      const std::shared_ptr<mrs_msgs::srv::ValidateReference::Response>
          response);
  bool callbackValidateReference2d(
      const std::shared_ptr<mrs_msgs::srv::ValidateReference::Request> request,
      const std::shared_ptr<mrs_msgs::srv::ValidateReference::Response>
          response);
  bool callbackValidateReferenceArray(
      const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Request>
          request,
      const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Response>
          response);

  // transformation callbacks
  bool callbackTransformReference(
      const std::shared_ptr<mrs_msgs::srv::TransformReferenceSrv::Request>
          request,
      const std::shared_ptr<mrs_msgs::srv::TransformReferenceSrv::Response>
          response);
  bool callbackTransformReferenceArray(
      const std::shared_ptr<mrs_msgs::srv::TransformReferenceArraySrv::Request>
          request,
      const std::shared_ptr<mrs_msgs::srv::TransformReferenceArraySrv::Response>
          response);
  bool callbackTransformPose(
      const std::shared_ptr<mrs_msgs::srv::TransformPoseSrv::Request> request,
      const std::shared_ptr<mrs_msgs::srv::TransformPoseSrv::Response>
          response);
  bool callbackTransformVector3(
      const std::shared_ptr<mrs_msgs::srv::TransformVector3Srv::Request>
          request,
      const std::shared_ptr<mrs_msgs::srv::TransformVector3Srv::Response>
          response);

  // | ----------------------- constraints ---------------------- |

  // sets constraints to all trackers
  bool callbackSetConstraints(
      const std::shared_ptr<mrs_msgs::srv::DynamicsConstraintsSrv::Request>
          request,
  ...
  bool callbackUseJoystick(
      const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
      const std::shared_ptr<std_srvs::srv::Trigger::Response> response);

  // joystick buttons mappings
  int _channel_A_, _channel_B_, _channel_X_, _channel_Y_, _channel_start_,
      _channel_back_, _channel_LT_, _channel_RT_, _channel_L_joy_,
      _channel_R_joy_;

  // channel numbers and channel multipliers
  int _channel_pitch_, _channel_roll_, _channel_heading_, _channel_throttle_;
  double _channel_mult_pitch_, _channel_mult_roll_, _channel_mult_heading_,
  ...
  double _rc_channel_pitch_, _rc_channel_roll_, _rc_channel_heading_,
      _rc_channel_throttle_;
  ...
  mrs_lib::PublisherHandler<geometry_msgs::msg::PoseArray>
      pub_debug_original_trajectory_poses_;
  mrs_lib::PublisherHandler<visualization_msgs::msg::MarkerArray>
      pub_debug_original_trajectory_markers_;

  // | --------------------- other routines --------------------- |

  // this is called to update the trackers and to receive position control
  // command from the active one
  void updateTrackers(void);

  // this is called to update the controllers and to receive attitude control
  // command from the active one
  void updateControllers(const mrs_msgs::msg::UavState& uav_state);

  // sets the reference to the active tracker
  std::tuple<bool, std::string> setReference(
      const mrs_msgs::msg::ReferenceStamped reference_in);

  // sets the velocity reference to the active tracker
  std::tuple<bool, std::string> setVelocityReference(
      const mrs_msgs::msg::VelocityReferenceStamped& reference_in);

  // sets the reference trajectory to the active tracker
  std::tuple<bool, std::string, bool, std::vector<std::string>,
  ...
  void publishControlReferenceOdom(
  ...
  mrs_msgs::msg::ReferenceStamped velocityReferenceToReference(
...
ControlManager::ControlManager(rclcpp::NodeOptions options)
    : mrs_lib::Node("control_manager", options) {
  node_ = this_node_ptr();
  ...
  sh_hw_api_capabilities_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::HwApiCapabilities>(
          shopts, "~/hw_api_capabilities_in");

  timer_hw_api_capabilities_ = node_->create_wall_timer(
      std::chrono::duration<double>(1.0),
      std::bind(&ControlManager::timerHwApiCapabilities, this));
}
...
void ControlManager::initialize(void) {
  ...
  common_handlers_ =
      std::make_shared<mrs_uav_managers::control_manager::CommonHandlers_t>();
  ...
  param_loader_->loadParam("motor_params/a",
                           common_handlers_->throttle_model.A);
  param_loader_->loadParam("motor_params/b",
  ...
  if (!(_state_input_ == INPUT_UAV_STATE || _state_input_ == INPUT_ODOMETRY)) {
    RCLCPP_ERROR(node_->get_logger(),
                 "the state_input parameter has to be in {0, 1}");
    rclcpp::shutdown();
    exit(1);
  }

  param_loader_->loadParam("safety/min_throttle_null_tracker",
                           _min_throttle_null_tracker_);
  param_loader_->loadParam("safety/ehover_tracker", _ehover_tracker_name_);
  param_loader_->loadParam("safety/failsafe_controller",
                           _failsafe_controller_name_);

  param_loader_->loadParam("safety/eland/controller", _eland_controller_name_);
  param_loader_->loadParam("safety/eland/cutoff_mass_factor",
                           _elanding_cutoff_mass_factor_);
  param_loader_->loadParam("safety/eland/cutoff_timeout",
                           _elanding_cutoff_timeout_);
  ...
  param_loader_->loadParam("safety/escalating_failsafe/service/enabled",
                           _service_escalating_failsafe_enabled_);
  param_loader_->loadParam("safety/escalating_failsafe/rc/enabled",
                           _rc_escalating_failsafe_enabled_);
  param_loader_->loadParam("safety/escalating_failsafe/rc/channel_number",
                           _rc_escalating_failsafe_channel_);
  param_loader_->loadParam("safety/escalating_failsafe/rc/threshold",
                           _rc_escalating_failsafe_threshold_);
  param_loader_->loadParam("safety/escalating_failsafe/timeout",
                           _escalating_failsafe_timeout_);
  param_loader_->loadParam("safety/escalating_failsafe/ehover",
                           _escalating_failsafe_ehover_);
  param_loader_->loadParam("safety/escalating_failsafe/eland",
                           _escalating_failsafe_eland_);
  param_loader_->loadParam("safety/escalating_failsafe/failsafe",
                           _escalating_failsafe_failsafe_);

  param_loader_->loadParam("safety/tilt_limit/eland/enabled",
                           _tilt_limit_eland_enabled_);
  ...
  if (_tilt_limit_eland_enabled_ && fabs(_tilt_limit_eland_) < 1e-3) {
    RCLCPP_ERROR(
        node_->get_logger(),
        "safety/tilt_limit/eland/enabled = 'TRUE' but the limit is too low");
    rclcpp::shutdown();
    exit(1);
  }

  param_loader_->loadParam("safety/tilt_limit/disarm/enabled",
                           _tilt_limit_disarm_enabled_);
  param_loader_->loadParam("safety/tilt_limit/disarm/limit",
                           _tilt_limit_disarm_);
  ...
  if (_tilt_limit_disarm_enabled_ && fabs(_tilt_limit_disarm_) < 1e-3) {
    RCLCPP_ERROR(
        node_->get_logger(),
        "safety/tilt_limit/disarm/enabled = 'TRUE' but the limit is too low");
    rclcpp::shutdown();
    exit(1);
  }

  param_loader_->loadParam("safety/yaw_error_eland/enabled",
                           _yaw_error_eland_enabled_);
  ...
  if (_yaw_error_eland_enabled_ && fabs(_yaw_error_eland_) < 1e-3) {
    RCLCPP_ERROR(
        node_->get_logger(),
        "safety/yaw_error_eland/enabled = 'TRUE' but the limit is too low");
    rclcpp::shutdown();
    exit(1);
  }
  ...
  param_loader_->loadParam("safety/rc_emergency_handoff/enabled",
                           _rc_emergency_handoff_);

  param_loader_->loadParam("safety/odometry_max_missing_time",
                           _uav_state_max_missing_time_);
  param_loader_->loadParam("safety/odometry_innovation_eland/enabled",
                           _odometry_innovation_check_enabled_);

  param_loader_->loadParam("safety/tilt_error_disarm/enabled",
                           _tilt_error_disarm_enabled_);
  param_loader_->loadParam("safety/tilt_error_disarm/timeout",
  ...
  if (_tilt_error_disarm_enabled_ &&
      fabs(_tilt_error_disarm_threshold_) < 1e-3) {
    RCLCPP_ERROR(
        node_->get_logger(),
        "safety/tilt_error_disarm/enabled = 'TRUE' but the limit is too low");
    rclcpp::shutdown();
    exit(1);
  }
  ...
  param_loader_->loadParam("default_constraints/horizontal/speed",
                           current_constraints_.constraints.horizontal_speed);
  param_loader_->loadParam(
      "default_constraints/horizontal/acceleration",
      current_constraints_.constraints.horizontal_acceleration);
  param_loader_->loadParam("default_constraints/horizontal/jerk",
                           current_constraints_.constraints.horizontal_jerk);
  param_loader_->loadParam("default_constraints/horizontal/snap",
                           current_constraints_.constraints.horizontal_snap);

  param_loader_->loadParam(
      "default_constraints/vertical/ascending/speed",
      current_constraints_.constraints.vertical_ascending_speed);
  param_loader_->loadParam(
      "default_constraints/vertical/ascending/acceleration",
      current_constraints_.constraints.vertical_ascending_acceleration);
  param_loader_->loadParam(
      "default_constraints/vertical/ascending/jerk",
      current_constraints_.constraints.vertical_ascending_jerk);
  param_loader_->loadParam(
      "default_constraints/vertical/ascending/snap",
      current_constraints_.constraints.vertical_ascending_snap);

  param_loader_->loadParam(
      "default_constraints/vertical/descending/speed",
      current_constraints_.constraints.vertical_descending_speed);
  param_loader_->loadParam(
      "default_constraints/vertical/descending/acceleration",
      current_constraints_.constraints.vertical_descending_acceleration);
  param_loader_->loadParam(
      "default_constraints/vertical/descending/jerk",
      current_constraints_.constraints.vertical_descending_jerk);
  param_loader_->loadParam(
      "default_constraints/vertical/descending/snap",
      current_constraints_.constraints.vertical_descending_snap);

  param_loader_->loadParam("default_constraints/heading/speed",
                           current_constraints_.constraints.heading_speed);
  param_loader_->loadParam(
      "default_constraints/heading/acceleration",
      current_constraints_.constraints.heading_acceleration);
  param_loader_->loadParam("default_constraints/heading/jerk",
                           current_constraints_.constraints.heading_jerk);
  param_loader_->loadParam("default_constraints/heading/snap",
                           current_constraints_.constraints.heading_snap);

  param_loader_->loadParam("default_constraints/angular_speed/roll",
                           current_constraints_.constraints.roll_rate);
  param_loader_->loadParam("default_constraints/angular_speed/pitch",
                           current_constraints_.constraints.pitch_rate);
  param_loader_->loadParam("default_constraints/angular_speed/yaw",
                           current_constraints_.constraints.yaw_rate);

  param_loader_->loadParam("default_constraints/tilt",
                           current_constraints_.constraints.tilt);

  current_constraints_.constraints.tilt =
      M_PI * (current_constraints_.constraints.tilt / 180.0);
  ...
  // Override joystick carrot distance based on safety area size if enabled
  if (use_safety_area_ && safety_zone_) {
    ...
    // Calculate bounding box of safety area
    ...
    // Calculate characteristic size (use smaller dimension)
    double width = max_x - min_x;
    double height = max_y - min_y;
    double safety_area_size = std::min(width, height);

    // Set carrot distance as a fraction of the safety area size
    // Use 10% of the smaller dimension as a reasonable default
    _joystick_carrot_distance_ = safety_area_size * 0.1;

    RCLCPP_INFO(node_->get_logger(),
                "Joystick carrot distance set to %.2f m based on safety area "
                "(width: %.2f m, height: %.2f m)",
                _joystick_carrot_distance_, width, height);
  }

  param_loader_->loadParam("joystick/joystick_timer_rate",
                           _joystick_timer_rate_);
  param_loader_->loadParam("joystick/attitude_control/tracker",
                           _joystick_tracker_name_);
  param_loader_->loadParam("joystick/attitude_control/controller",
                           _joystick_controller_name_);
  param_loader_->loadParam("joystick/attitude_control/fallback/tracker",
                           _joystick_fallback_tracker_name_);
  param_loader_->loadParam("joystick/attitude_control/fallback/controller",
                           _joystick_fallback_controller_name_);
  ...
  param_loader_->loadParam("joystick/channel_multipliers/pitch",
                           _channel_mult_pitch_);
  param_loader_->loadParam("joystick/channel_multipliers/roll",
                           _channel_mult_roll_);
  param_loader_->loadParam("joystick/channel_multipliers/heading",
  ...
  param_loader_->loadParam("obstacle_bumper/switch_tracker",
                           _bumper_switch_tracker_);
  param_loader_->loadParam("obstacle_bumper/switch_controller",
                           _bumper_switch_controller_);
  param_loader_->loadParam("obstacle_bumper/tracker", _bumper_tracker_name_);
  param_loader_->loadParam("obstacle_bumper/controller",
                           _bumper_controller_name_);
  param_loader_->loadParam("obstacle_bumper/timer_rate", _bumper_timer_rate_);

  param_loader_->loadParam(
      "obstacle_bumper/horizontal/min_distance_to_obstacle",
      _bumper_horizontal_distance_);
  param_loader_->loadParam("obstacle_bumper/horizontal/derived_from_dynamics",
                           _bumper_horizontal_derive_from_dynamics_);

  param_loader_->loadParam("obstacle_bumper/vertical/min_distance_to_obstacle",
                           _bumper_vertical_distance_);
  param_loader_->loadParam("obstacle_bumper/vertical/derived_from_dynamics",
                           _bumper_vertical_derive_from_dynamics_);

  param_loader_->loadParam("obstacle_bumper/horizontal/overshoot",
                           _bumper_horizontal_overshoot_);
  param_loader_->loadParam("obstacle_bumper/vertical/overshoot",
                           _bumper_vertical_overshoot_);

  param_loader_->loadParam("safety/tracker_error_action",
                           _tracker_error_action_);

  param_loader_->loadParam("trajectory_tracking/snap_to_safety_area",
                           _snap_trajectory_to_safety_area_);

  // check the values of tracker error action
  if (_tracker_error_action_ != ELAND_STR &&
      _tracker_error_action_ != EHOVER_STR) {
    RCLCPP_ERROR(node_->get_logger(),
                 "the tracker_error_action parameter (%s) is not correct, "
                 "requires {%s, %s}",
                 _tracker_error_action_.c_str(), ELAND_STR, EHOVER_STR);
    rclcpp::shutdown();
    exit(1);
  }
  ...
  param_loader_->loadParam("rc_joystick/horizontal_speed",
                           _rc_horizontal_speed_);
  ...
  param_loader_->loadParam("rc_joystick/channels/heading",
                           _rc_channel_heading_);
  param_loader_->loadParam("rc_joystick/channels/throttle",
                           _rc_channel_throttle_);
  ...
  const std::string scope_timer_log_filename =
      param_loader_->loadParam2("scope_timer/log_filename", std::string(""));
  scope_timer_logger_ = std::make_shared<mrs_lib::ScopeTimerLogger>(
      node_, scope_timer_log_filename, scope_timer_enabled_);
  ...
  common_handlers_->safety_area.isPointInSafetyArea3d = std::bind(
      &ControlManager::isPointInSafetyArea3d, this, std::placeholders::_1);
  common_handlers_->safety_area.getMinZ =
      std::bind(&ControlManager::getMinZ, this, std::placeholders::_1);
  common_handlers_->safety_area.getMaxZ =
      std::bind(&ControlManager::getMaxZ, this, std::placeholders::_1);

  common_handlers_->getMass = std::bind(&ControlManager::getMass, this);

  common_handlers_->detailed_model_params =
      loadDetailedUavModelParams(node_, _platform_config_, _custom_config_);
  ...
  if (!custom_trackers.empty()) {
    _tracker_names_.insert(_tracker_names_.end(), custom_trackers.begin(),
                           custom_trackers.end());
  }
  ...
  tracker_loader_ =
      std::make_unique<pluginlib::ClassLoader<mrs_uav_managers::Tracker>>(
          "mrs_uav_managers", "mrs_uav_managers::Tracker");
  ...
  for (int i = 0; i < int(_tracker_names_.size()); i++) {
    ...
    trackers_.insert(
        std::pair<std::string, TrackerParams>(tracker_name, new_tracker));

    try {
      RCLCPP_INFO(node_->get_logger(), "loading the tracker '%s'",
                  new_tracker.address.c_str());
      tracker_list_.push_back(
          tracker_loader_->createSharedInstance(new_tracker.address.c_str()));
    } catch (pluginlib::CreateClassException& ex1) {
      RCLCPP_ERROR(node_->get_logger(),
                   "CreateClassException for the tracker '%s'",
                   new_tracker.address.c_str());
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "PluginlibException for the tracker '%s'",
                   new_tracker.address.c_str());
  ...
  for (int i = 0; i < int(tracker_list_.size()); i++) {
    ...
    rclcpp::Node::SharedPtr subnode =
        trackers_subnode_->create_sub_node(it->second.name_space);

    // create private handlers
    std::shared_ptr<mrs_uav_managers::control_manager::PrivateHandlers_t>
        private_handlers = std::make_shared<
            mrs_uav_managers::control_manager::PrivateHandlers_t>();
    ...
    private_handlers->param_loader =
        std::make_unique<mrs_lib::ParamLoader>(subnode, _tracker_names_.at(i));
    ...
    try {
      RCLCPP_INFO(node_->get_logger(), "initializing the tracker '%s'",
                  it->second.address.c_str());
      success = tracker_list_.at(i)->initialize(subnode, common_handlers_,
                                                private_handlers);
    } catch (std::runtime_error& ex) {
      RCLCPP_ERROR(node_->get_logger(),
                   "exception caught during tracker initialization: '%s'",
                   ex.what());
    }

    if (!success) {
      RCLCPP_ERROR(node_->get_logger(), "failed to initialize the tracker '%s'",
                   it->second.address.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  ...
  // check if the hover_tracker is within the loaded trackers
  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(
          node_->get_logger(),
          "the safety/hover_tracker (%s) is not within the loaded trackers",
          _ehover_tracker_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }
...

  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "the landoff tracker (%s) is not within the loaded trackers",
                   _landoff_tracker_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }
...

  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "the null tracker (%s) is not within the loaded trackers",
                   _null_tracker_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }
  ...
  if (_joystick_enabled_) {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(
          node_->get_logger(),
          "the joystick tracker (%s) is not within the loaded trackers",
  ...
  if (_bumper_switch_tracker_) {
    ...
    if (!idx) {
      RCLCPP_ERROR(node_->get_logger(),
                   "the bumper tracker (%s) is not within the loaded trackers",
                   _bumper_tracker_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }

  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "the joystick fallback tracker (%s) is not within the "
                   "loaded trackers",
                   _joystick_fallback_tracker_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }
  ...
  if (!custom_controllers.empty()) {
    _controller_names_.insert(_controller_names_.end(),
                              custom_controllers.begin(),
                              custom_controllers.end());
  }

  controller_loader_ =
      std::make_unique<pluginlib::ClassLoader<mrs_uav_managers::Controller>>(
          "mrs_uav_managers", "mrs_uav_managers::Controller");
  ...
  for (int i = 0; i < int(_controller_names_.size()); i++) {
    ...
    param_loader_->loadParam(controller_name + "/eland_threshold",
                             eland_threshold);
    param_loader_->loadParam(controller_name + "/failsafe_threshold",
                             failsafe_threshold);
    ...
    // check if the controller can output some of the required outputs
    {
      ...
      param_loader_->loadParam(controller_name + "/outputs/actuators",
                               outputs.actuators, false);
      param_loader_->loadParam(controller_name + "/outputs/control_group",
                               outputs.control_group, false);
      param_loader_->loadParam(controller_name + "/outputs/attitude_rate",
                               outputs.attitude_rate, false);
      param_loader_->loadParam(controller_name + "/outputs/attitude",
                               outputs.attitude, false);
      param_loader_->loadParam(
          controller_name + "/outputs/acceleration_hdg_rate",
          outputs.acceleration_hdg_rate, false);
      param_loader_->loadParam(controller_name + "/outputs/acceleration_hdg",
                               outputs.acceleration_hdg, false);
      param_loader_->loadParam(controller_name + "/outputs/velocity_hdg_rate",
                               outputs.velocity_hdg_rate, false);
      param_loader_->loadParam(controller_name + "/outputs/velocity_hdg",
      ...
      bool meets_control_group =
          (_hw_api_inputs_.control_group && outputs.control_group);
      bool meets_attitude_rate =
          (_hw_api_inputs_.attitude_rate && outputs.attitude_rate);
      bool meets_attitude = (_hw_api_inputs_.attitude && outputs.attitude);
      bool meets_acceleration_hdg_rate =
          (_hw_api_inputs_.acceleration_hdg_rate &&
           outputs.acceleration_hdg_rate);
      bool meets_acceleration_hdg =
          (_hw_api_inputs_.acceleration_hdg && outputs.acceleration_hdg);
      bool meets_velocity_hdg_rate =
      ...
      if (!meets_requirements) {
        RCLCPP_ERROR(node_->get_logger(),
                     "the controller '%s' does not meet the control output "
                     "requirements, which are some of the following",
      ...
      if ((_hw_api_inputs_.actuators || _hw_api_inputs_.control_group) &&
          !common_handlers_->detailed_model_params) {
        RCLCPP_ERROR(
            node_->get_logger(),
            "the HW API supports 'actuators' or 'control_group' input, but the "
            "'detailed uav model params' were not loaded sucessfully");
        rclcpp::shutdown();
        exit(1);
      }
    }
    ...
    ControllerParams new_controller(
        address, name_space, eland_threshold, failsafe_threshold,
        odometry_innovation_threshold, human_switchable);
    controllers_.insert(std::pair<std::string, ControllerParams>(
        controller_name, new_controller));

    try {
      RCLCPP_INFO(node_->get_logger(), "loading the controller '%s'",
                  new_controller.address.c_str());
      controller_list_.push_back(controller_loader_->createSharedInstance(
          new_controller.address.c_str()));
    } catch (pluginlib::CreateClassException& ex1) {
      RCLCPP_ERROR(node_->get_logger(),
                   "CreateClassException for the controller '%s'",
                   new_controller.address.c_str());
      RCLCPP_ERROR(node_->get_logger(), "Error: %s", ex1.what());
      rclcpp::shutdown();
      exit(1);
      ...
      RCLCPP_ERROR(node_->get_logger(),
                   "PluginlibException for the controller '%s'",
                   new_controller.address.c_str());
      RCLCPP_ERROR(node_->get_logger(), "Error: %s", ex.what());
      rclcpp::shutdown();
      exit(1);
  ...
  for (int i = 0; i < int(controller_list_.size()); i++) {
    ...
    rclcpp::Node::SharedPtr subnode =
        controllers_subnode_->create_sub_node(it->second.name_space);
    ...
    private_handlers->param_loader = std::make_unique<mrs_lib::ParamLoader>(
        subnode, _controller_names_.at(i));
    private_handlers->param_loader->copyYamls(*param_loader_);
    private_handlers->parent_param_loader = param_loader_;
    ...
    try {
      RCLCPP_INFO(node_->get_logger(), "initializing the controller '%s'",
                  it->second.address.c_str());
      success = controller_list_.at(i)->initialize(subnode, common_handlers_,
                                                   private_handlers);
    } catch (std::runtime_error& ex) {
      RCLCPP_ERROR(node_->get_logger(),
                   "exception caught during controller initialization: '%s'",
                   ex.what());
    }

    if (!success) {
      RCLCPP_ERROR(node_->get_logger(),
                   "failed to initialize the controller '%s'",
                   it->second.address.c_str());
      rclcpp::shutdown();
      exit(1);
    }
...

  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(
          node_->get_logger(),
          "the failsafe controller (%s) is not within the loaded controllers",
...

  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(
          node_->get_logger(),
          "the eland controller (%s) is not within the loaded controllers",
          _eland_controller_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }

  {
    ...
    if (idx) {
      ...
      RCLCPP_ERROR(
          node_->get_logger(),
          "the joystick controller (%s) is not within the loaded controllers",
  ...
  if (_bumper_switch_controller_) {
    ...
    if (!idx) {
      RCLCPP_ERROR(
          node_->get_logger(),
          "the bumper controller (%s) is not within the loaded controllers",
          _bumper_controller_name_.c_str());
      rclcpp::shutdown();
      exit(1);
    }
  }
  ...
  {
    auto idx =
        idxInVector(_joystick_fallback_controller_name_, _controller_names_);

    if (idx) {
      _joystick_fallback_controller_idx_ = idx.value();
    } else {
      RCLCPP_ERROR(node_->get_logger(),
                   "the joystick fallback controller (%s) is not within the "
                   "loaded controllers",
  ...
  RCLCPP_INFO(
      node_->get_logger(),
      "activating the the eland controller (%s) as the first controller",
      _controller_names_.at(_eland_controller_idx_).c_str());
  ...
  ph_tracker_cmd_ = mrs_lib::PublisherHandler<mrs_msgs::msg::TrackerCommand>(
      node_, "~/tracker_cmd_out");
  ph_mrs_odom_input_ = mrs_lib::PublisherHandler<mrs_msgs::msg::EstimatorInput>(
      node_, "~/estimator_input_out");
  ph_control_reference_odom_ =
      mrs_lib::PublisherHandler<nav_msgs::msg::Odometry>(
          node_, "~/control_reference_out");
  ph_diagnostics_ =
      mrs_lib::PublisherHandler<mrs_msgs::msg::ControlManagerDiagnostics>(
          node_, "~/diagnostics_out");
  ph_offboard_on_ = mrs_lib::PublisherHandler<std_msgs::msg::Empty>(
      node_, "~/offboard_on_out");
  ph_tilt_error_ = mrs_lib::PublisherHandler<mrs_msgs::msg::Float64Stamped>(
      node_, "~/tilt_error_out");
  ph_mass_nominal_ = mrs_lib::PublisherHandler<std_msgs::msg::Float64>(
      node_, "~/mass_nominal_out");  // TODO latch
  ph_control_error_ = mrs_lib::PublisherHandler<mrs_msgs::msg::ControlError>(
      node_, "~/control_error_out");

  {
    ...
    ph_throttle_ = mrs_lib::PublisherHandler<std_msgs::msg::Float64>(
        opts, "~/throttle_out");
  }

  {
    ...
    ph_thrust_ =
        mrs_lib::PublisherHandler<std_msgs::msg::Float64>(opts, "~/thrust_out");
  }

  {
    ...
    ph_mass_estimate_ = mrs_lib::PublisherHandler<std_msgs::msg::Float64>(
        opts, "~/mass_estimate_out");
  }
...

  {
    ...
    ph_disturbances_markers_ =
        mrs_lib::PublisherHandler<visualization_msgs::msg::MarkerArray>(
            opts, "~/disturbances_markers_out");
  }

  ph_current_constraints_ =
      mrs_lib::PublisherHandler<mrs_msgs::msg::DynamicsConstraints>(
          node_, "~/current_constraints_out");
  ph_heading_ = mrs_lib::PublisherHandler<mrs_msgs::msg::Float64Stamped>(
      node_, "~/heading_out");
  ph_speed_ = mrs_lib::PublisherHandler<mrs_msgs::msg::Float64Stamped>(
      node_, "~/speed_out");

  {
    ...
    pub_debug_original_trajectory_poses_ =
        mrs_lib::PublisherHandler<geometry_msgs::msg::PoseArray>(
            opts, "~/trajectory_original/poses_out");
    pub_debug_original_trajectory_markers_ =
        mrs_lib::PublisherHandler<visualization_msgs::msg::MarkerArray>(
            opts, "~/trajectory_original/markers_out");
  }
  ...
  if (_state_input_ == INPUT_UAV_STATE) {
    sh_uav_state_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::UavState>(
        shopts, "~/uav_state_in", &ControlManager::callbackUavState, this);
  } else if (_state_input_ == INPUT_ODOMETRY) {
    sh_odometry_ = mrs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(
        shopts, "~/odometry_in", &ControlManager::callbackOdometry, this);
  }

  if (_odometry_innovation_check_enabled_) {
    sh_odometry_innovation_ =
        mrs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(
            shopts, "~/odometry_innovation_in");
  }

  sh_bumper_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::ObstacleSectors>(
  ...
  ss_switch_tracker_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::String>(
      node_, "~/switch_tracker_in",
      std::bind(&ControlManager::callbackSwitchTracker, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_switch_controller_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::String>(
      node_, "~/switch_controller_in",
      std::bind(&ControlManager::callbackSwitchController, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_reset_tracker_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/tracker_reset_static_in",
      std::bind(&ControlManager::callbackTrackerResetStatic, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_hover_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/hover_in",
      std::bind(&ControlManager::callbackHover, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_ehover_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/ehover_in",
      std::bind(&ControlManager::callbackEHover, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_failsafe_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/failsafe_in",
      std::bind(&ControlManager::callbackFailsafe, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_failsafe_escalating_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
          node_, "~/failsafe_escalating_in",
          std::bind(&ControlManager::callbackFailsafeEscalating, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_toggle_output_ = mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
      node_, "~/toggle_output_in",
      std::bind(&ControlManager::callbackToggleOutput, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_arm_ = mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
      node_, "~/arm_in",
      std::bind(&ControlManager::callbackArm, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_enable_callbacks_ = mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
      node_, "~/enable_callbacks_in",
      std::bind(&ControlManager::callbackEnableCallbacks, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_set_constraints_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::DynamicsConstraintsSrv>(
          node_, "~/set_constraints_in",
          std::bind(&ControlManager::callbackSetConstraints, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_use_joystick_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/use_joystick_in",
  ...
  ss_set_min_z_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::Float64StampedSrv>(
          node_, "~/set_min_z_in",
          std::bind(&ControlManager::callbackSetMinZ, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_transform_reference_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::TransformReferenceSrv>(
  ...
  ss_bumper_enabler_ = mrs_lib::ServiceServerHandler<std_srvs::srv::SetBool>(
      node_, "~/bumper_in",
      std::bind(&ControlManager::callbackEnableBumper, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_get_min_z_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::GetFloat64>(
      node_, "~/get_min_z_in",
      std::bind(&ControlManager::callbackGetMinZ, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_validate_reference_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ValidateReference>(
          node_, "~/validate_reference_in",
          std::bind(&ControlManager::callbackValidateReference, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_validate_reference_2d_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ValidateReference>(
          node_, "~/validate_reference_2d_in",
          std::bind(&ControlManager::callbackValidateReference2d, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_validate_reference_array_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ValidateReferenceArray>(
          node_, "~/validate_reference_array_in",
          std::bind(&ControlManager::callbackValidateReferenceArray, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_start_trajectory_tracking_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
          node_, "~/start_trajectory_tracking_in",
          std::bind(&ControlManager::callbackStartTrajectoryTracking, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_stop_trajectory_tracking_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
          node_, "~/stop_trajectory_tracking_in",
          std::bind(&ControlManager::callbackStopTrajectoryTracking, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_resume_trajectory_tracking_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
          node_, "~/resume_trajectory_tracking_in",
          std::bind(&ControlManager::callbackResumeTrajectoryTracking, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_goto_trajectory_start_ =
      mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
          node_, "~/goto_trajectory_start_in",
          std::bind(&ControlManager::callbackGotoTrajectoryStart, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);

  sch_arming_ = mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
  ...
  sch_set_odometry_callbacks_ =
      mrs_lib::ServiceClientHandler<std_srvs::srv::SetBool>(
          node_, "~/set_odometry_callbacks_out", cbkgrp_sc_);
  sch_ungrip_ = mrs_lib::ServiceClientHandler<std_srvs::srv::Trigger>(
  ...
  // human callable
  ss_goto_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec4>(
      node_, "~/goto_in",
      std::bind(&ControlManager::callbackGoto, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_goto_fcu_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec4>(
      node_, "~/goto_fcu_in",
      std::bind(&ControlManager::callbackGotoFcu, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_goto_relative_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec4>(
      node_, "~/goto_relative_in",
      std::bind(&ControlManager::callbackGotoRelative, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_goto_altitude_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec1>(
      node_, "~/goto_altitude_in",
      std::bind(&ControlManager::callbackGotoAltitude, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_goto_heading_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec1>(
      node_, "~/set_heading_in",
      std::bind(&ControlManager::callbackSetHeading, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_set_heading_relative_ = mrs_lib::ServiceServerHandler<mrs_msgs::srv::Vec1>(
      node_, "~/set_heading_relative_in",
      std::bind(&ControlManager::callbackSetHeadingRelative, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ...
  ss_reference_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ReferenceStampedSrv>(
          node_, "~/reference_in",
          std::bind(&ControlManager::callbackReferenceService, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  sh_reference_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::ReferenceStamped>(
      shopts, "~/reference_in", &ControlManager::callbackReferenceTopic, this);
  ss_velocity_reference_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::VelocityReferenceStampedSrv>(
          node_, "~/velocity_reference_in",
          std::bind(&ControlManager::callbackVelocityReferenceService, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  sh_velocity_reference_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::VelocityReferenceStamped>(
          shopts, "~/velocity_reference_in",
          &ControlManager::callbackVelocityReferenceTopic, this);
  ss_trajectory_reference_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::TrajectoryReferenceSrv>(
          node_, "~/trajectory_reference_in",
          std::bind(&ControlManager::callbackTrajectoryReferenceService, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  sh_trajectory_reference_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::TrajectoryReference>(
          shopts, "~/trajectory_reference_in",
  ...
  ss_emergency_reference_ =
      mrs_lib::ServiceServerHandler<mrs_msgs::srv::ReferenceStampedSrv>(
          node_, "~/emergency_reference_in",
          std::bind(&ControlManager::callbackEmergencyReference, this,
                    std::placeholders::_1, std::placeholders::_2),
          rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
  ss_pirouette_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node_, "~/pirouette_in",
      std::bind(&ControlManager::callbackPirouette, this, std::placeholders::_1,
                std::placeholders::_2),
      rclcpp::SystemDefaultsQoS(), cbkgrp_ss_);
...

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerStatus, this);

    timer_status_ = std::make_shared<TimerType>(
        timer_opts_start, rclcpp::Rate(_status_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerSafety, this);

    timer_safety_ = std::make_shared<TimerType>(
        timer_opts_start, rclcpp::Rate(_safety_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerBumper, this);

    timer_bumper_ = std::make_shared<TimerType>(
        timer_opts_start, rclcpp::Rate(_bumper_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerJoystick, this);

    timer_joystick_ = std::make_shared<TimerType>(
        timer_opts_start, rclcpp::Rate(_joystick_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerEland, this);

    timer_eland_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_elanding_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerFailsafe, this);

    timer_failsafe_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_failsafe_timer_rate_), callback_fcn);
  }

  {
    std::function<void()> callback_fcn =
        std::bind(&ControlManager::timerPirouette, this);

    timer_pirouette_ = std::make_shared<TimerType>(
        timer_opts_no_start, rclcpp::Rate(_pirouette_timer_rate_),
        callback_fcn);
  }
...
void ControlManager::shutdown() {
...

  {
    ...
    for (int i = 0; i < int(tracker_list_.size()); i++) {
      std::cout << "ControlManager: unloading " << _tracker_names_[i]
                << std::endl;
...

  {
    ...
    for (int i = 0; i < int(controller_list_.size()); i++) {
      std::cout << "ControlManager: unloading " << _controller_names_[i]
                << std::endl;
...
void ControlManager::timerHwApiCapabilities() {
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("timerHwApiCapabilities");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerHwApiCapabilities",
  ...
  if (!sh_hw_api_capabilities_.hasMsg()) {
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "waiting for HW API capabilities");
    return;
  }
  ...
  RCLCPP_INFO(node_->get_logger(),
              "got HW API capabilities, the possible control modes are:");
...
void ControlManager::timerStatus() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerStatus",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  auto position_error =
      mrs_lib::get_mutexed(mutex_position_error_, position_error_);
...

  {
    ...
    RCLCPP_INFO_THROTTLE(
        ...
        "tracker: '%s', controller: '%s', mass: '%.2f kg', disturbances: body "
        "[%.2f, %.2f] N, world [%.2f, %.2f] N",
  ...
  if (last_control_output.diagnostics.mass_estimator) {
    ...
    mass_estimate_out.data =
        _uav_mass_ + last_control_output.diagnostics.mass_difference;
  ...
  if (_state_input_ == INPUT_UAV_STATE && sh_uav_state_.hasMsg()) {
    try {
      ...
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "exception caught, could not transform heading");
    }
  }
  ...
  if (_state_input_ == INPUT_UAV_STATE && sh_uav_state_.hasMsg()) {
    double speed = sqrt(pow(uav_state.velocity.linear.x, 2) +
                        pow(uav_state.velocity.linear.y, 2) +
                        pow(uav_state.velocity.linear.z, 2));
  ...
  if (last_control_output.diagnostics.disturbance_estimator && got_uav_state_) {
    ...
    Eigen::Quaterniond quat_eigen =
        mrs_lib::AttitudeConverter(uav_state.pose.orientation);
    ...
    /* world disturbance //{ */
    {
      ...
      point.x =
          uav_x + multiplier * last_control_output.diagnostics.disturbance_wx_w;
      point.y =
          uav_y + multiplier * last_control_output.diagnostics.disturbance_wy_w;
      point.z = uav_z;
    ...
    /* body disturbance //{ */
    {
      ...
      vec3d << multiplier * last_control_output.diagnostics.disturbance_bx_b,
          multiplier * last_control_output.diagnostics.disturbance_by_b, 0;
  ...
  if (got_constraints_) {
    auto sanitized_constraints =
        mrs_lib::get_mutexed(mutex_constraints_, sanitized_constraints_);

    mrs_msgs::msg::DynamicsConstraints constraints =
...
void ControlManager::timerSafety() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerSafety",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  auto [uav_state, uav_yaw] =
      mrs_lib::get_mutexed(mutex_uav_state_, uav_state_, uav_yaw_);
  auto active_controller_idx =
      mrs_lib::get_mutexed(mutex_controller_list_, active_controller_idx_);
  auto active_tracker_idx =
      mrs_lib::get_mutexed(mutex_tracker_list_, active_tracker_idx_);
  ...
  if (odometry_switch_in_progress_) {
    RCLCPP_WARN(node_->get_logger(),
                "timerSafety tried to run while odometry switch in progress");
    return;
  }
  ...
  if (_state_input_ == INPUT_UAV_STATE && sh_uav_state_.hasMsg()) {
    double missing_for =
        (clock_->now() - sh_uav_state_.lastMsgTime()).seconds();
  ...
  // This means that the timerFailsafe only does its work when Controllers and
  // Trackers produce valid output. Cases when the commands are not valid should
  // be handle in updateControllers() and updateTrackers() methods.
...

  {
    ...
    if (last_tracker_cmd->use_position_horizontal &&
        !std::holds_alternative<mrs_msgs::msg::HwApiPositionCmd>(
            last_control_output.control_output.value())) {
      position_error(0) =
          last_tracker_cmd->position.x - uav_state.pose.position.x;
      position_error(1) =
          last_tracker_cmd->position.y - uav_state.pose.position.y;
    ...
    if (last_tracker_cmd->use_position_vertical &&
        !std::holds_alternative<mrs_msgs::msg::HwApiPositionCmd>(
            last_control_output.control_output.value())) {
      position_error(2) =
          last_tracker_cmd->position.z - uav_state.pose.position.z;

      position_error_set = true;
    }

    if (position_error_set) {
      mrs_lib::set_mutexed(mutex_position_error_, {position_error},
                           position_error_);
    }
  }

  // rotate the drone's z axis
  tf2::Transform uav_state_transform =
      mrs_lib::AttitudeConverter(uav_state.pose.orientation);
  ...
  if (last_control_output.desired_orientation) {
    ...
    tf2::Transform attitude_cmd_transform = mrs_lib::AttitudeConverter(
        last_control_output.desired_orientation.value());
    tf2::Vector3 uav_z_in_world_desired =
        attitude_cmd_transform * tf2::Vector3(0, 0, 1);

    {
      ...
      double cmd_yaw = mrs_lib::AttitudeConverter(
                           last_control_output.desired_orientation.value())
                           .getYaw();
  ...
  auto position_error =
      mrs_lib::get_mutexed(mutex_position_error_, position_error_);
  auto [tilt_error, yaw_error] =
      mrs_lib::get_mutexed(mutex_attitude_error_, tilt_error_, yaw_error_);
  ...
  if (position_error) {
    if (position_error->norm() > _failsafe_threshold_ && !failsafe_triggered_) {
      auto controller_tracker_switch_time =
          mrs_lib::get_mutexed(mutex_controller_tracker_switch_time_,
                               controller_tracker_switch_time_);

      if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
        if (!failsafe_triggered_) {
          RCLCPP_ERROR(node_->get_logger(),
                       "activating failsafe land: control_error=%.2f/%.2f m "
                       "(x: %.2f, y: %.2f, z: %.2f)",
                       position_error->norm(), _failsafe_threshold_,
                       position_error.value()(0), position_error.value()(1),
                       position_error.value()(2));
  ...
  if (_odometry_innovation_check_enabled_) {
    ...
    if (sh_odometry_innovation_.hasMsg()) {
      ...
      RCLCPP_WARN_THROTTLE(
          ...
          "missing estimator innnovation but the innovation check is enabled!");
    }
    ...
    if (innovation) {
      ...
      double last_innovation =
          mrs_lib::geometry::dist(vec3_t(x, y, z), vec3_t(0, 0, 0));

      if (last_innovation > _odometry_innovation_threshold_ ||
          radians::diff(heading, 0) > M_PI_2) {
        auto controller_tracker_switch_time =
            mrs_lib::get_mutexed(mutex_controller_tracker_switch_time_,
                                 controller_tracker_switch_time_);
        ...
        if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
          if (!failsafe_triggered_ && !eland_triggered_) {
            RCLCPP_ERROR(
                node_->get_logger(),
                "activating emergency land: odometry innovation too large: "
                "%.2f/%.2f (x: %.2f, y: %.2f, z: %.2f, heading: %.2f)",
                last_innovation, _odometry_innovation_threshold_, x, y, z,
                heading);
  ...
  if (_tilt_limit_eland_enabled_ && tilt_angle > _tilt_limit_eland_) {
    auto controller_tracker_switch_time = mrs_lib::get_mutexed(
        mutex_controller_tracker_switch_time_, controller_tracker_switch_time_);

    if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
      if (!failsafe_triggered_ && !eland_triggered_) {
        RCLCPP_ERROR(
            node_->get_logger(),
            "activating emergency land: tilt angle too large (%.2f/%.2f deg)",
            (180.0 / M_PI) * tilt_angle, (180.0 / M_PI) * _tilt_limit_eland_);
  ...
  if (position_error) {
      ...
      if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
        if (!failsafe_triggered_ && !eland_triggered_) {
          RCLCPP_DEBUG_THROTTLE(
              node_->get_logger(), *clock_, 1000,
              "releasing payload due to large position error");
    ...
    if (error_size > _eland_threshold_) {
      ...
      if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
        if (!failsafe_triggered_ && !eland_triggered_) {
          RCLCPP_ERROR(node_->get_logger(),
                       "activating emergency land: position error %.2f/%.2f m "
                       "(error x: %.2f, y: %.2f, z: %.2f)",
                       error_size, _eland_threshold_, position_error.value()(0),
                       position_error.value()(1), position_error.value()(2));
  ...
  // | -------------------- yaw control error ------------------- |
  // do not have to mutex the yaw_error_ here since I am filling it in this
  // function
  ...
  if (_yaw_error_eland_enabled_ && yaw_error) {
      ...
      if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
        if (!failsafe_triggered_ && !eland_triggered_) {
          RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                                "releasing payload: yaw error %.2f/%.2f deg",
                                (180.0 / M_PI) * yaw_error.value(),
    ...
    if (yaw_error.value() > _yaw_error_eland_) {
      ...
      if ((clock_->now() - controller_tracker_switch_time).seconds() > 1.0) {
        if (!failsafe_triggered_ && !eland_triggered_) {
          RCLCPP_ERROR(node_->get_logger(),
                       "activating emergency land: yaw error %.2f/%.2f deg",
                       (180.0 / M_PI) * yaw_error.value(),
  ...
  if (_tilt_limit_disarm_enabled_ && tilt_angle > _tilt_limit_disarm_) {
    RCLCPP_ERROR(node_->get_logger(),
                 "tilt angle too large, disarming: tilt angle=%.2f/%.2f deg",
                 (180.0 / M_PI) * tilt_angle,
  ...
  if (_tilt_error_disarm_enabled_ && tilt_error) {
    ...
    // fyi: we should not
    double time_from_ctrl_tracker_switch =
        (clock_->now() - controller_tracker_switch_time).seconds();

    // if the tile error is over the threshold
    // && we are not ramping up during takeoff
    if (fabs(tilt_error.value()) > _tilt_error_disarm_threshold_ &&
        !last_control_output.diagnostics.ramping_up) {
      // only account for the error if some time passed from the last
      ...
      if (time_from_ctrl_tracker_switch > 1.0) {
        ...
        if (!tilt_error_disarm_over_thr_) {
          ...
          RCLCPP_WARN(node_->get_logger(),
                      "tilt error exceeded threshold (%.2f/%.2f deg)",
                      (180.0 / M_PI) * tilt_error.value(),
          ...
          RCLCPP_WARN_THROTTLE(
              ...
              "tilt error (%.2f deg) over threshold for %.2f s",
              (180.0 / M_PI) * tilt_error.value(),
    ...
    if (tilt_error_disarm_over_thr_ && (tot > _tilt_error_disarm_timeout_)) {
      bool is_flying =
          offboard_mode_ && active_tracker_idx != _null_tracker_idx_;

      // only when flying and not in failsafe
      if (is_flying) {
        RCLCPP_ERROR(node_->get_logger(),
                     "tilt error too large for %.2f s, disarming", tot);
  ...
  // if we are not in offboard and the drone is in mid air (NullTracker is not
  // active)
...
void ControlManager::timerEland() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerEland",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (!throttle) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "TODO: implement landing detection mechanism for the "
  ...
  if (current_state_landing_ == IDLE_STATE) {
    ...
    if (!last_control_output.control_output) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "timerEland: last_control_output has not been "
                           "initialized, returning");
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                           "tip: the RC eland is probably triggered");
      return;
    }

    // recalculate the mass based on the throttle
    throttle_mass_estimate_ =
        mrs_lib::quadratic_throttle_model::throttleToForce(
            common_handlers_->throttle_model, throttle.value()) /
        common_handlers_->g;
    RCLCPP_INFO_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "landing: initial mass: %.2f throttle_mass_estimate: %.2f",
        landing_uav_mass_, throttle_mass_estimate_);

    // condition for automatic motor turn off
    if (((throttle_mass_estimate_ <
          _elanding_cutoff_mass_factor_ * landing_uav_mass_) ||
         throttle < 0.01)) {
      ...
      RCLCPP_INFO_THROTTLE(
          node_->get_logger(), *clock_, 100,
          "throttle is under cutoff factor for %.2f s",
    ...
    if (throttle_under_threshold_ &&
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "reached cutoff throttle, disabling output");
      toggleOutput(false);
...
void ControlManager::timerFailsafe() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerFailsafe",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (!last_control_output.control_output) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "timerFailsafe: the control output produced by the "
                          "failsafe controller is empty!");
    return;
  }
  ...
  if (!throttle) {
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "FailsafeTimer: could not extract throttle out of "
                          "the last control output");
    return;
  }
  ...
  RCLCPP_INFO_THROTTLE(
      node_->get_logger(), *clock_, 1000,
      "failsafe: initial mass: %.2f throttle_mass_estimate: %.2f",
      landing_uav_mass_, throttle_mass_estimate_);
  ...
  if (throttle_under_threshold_ &&
      ((clock_->now() - throttle_mass_estimate_first_time_).seconds() >
       _elanding_cutoff_timeout_)) {
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "detecting zero throttle, disarming");
...
void ControlManager::timerJoystick() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerJoystick",
                          scope_timer_logger_, scope_timer_enabled_);
...
void ControlManager::timerBumper() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerBumper",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (sh_bumper_.hasMsg() &&
      (clock_->now() - sh_bumper_.lastMsgTime()).seconds() > 1.0) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "obstacle bumper is missing messages although we got them before");
...
void ControlManager::timerPirouette() {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::timerPirouette",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (rc_escalating_failsafe_triggered_ || failsafe_triggered_ ||
      eland_triggered_ ||
      (pirouette_iterator_ > pirouette_duration * _pirouette_timer_rate_)) {
  ...
  reference_request.reference.heading =
      pirouette_initial_heading_ + pirouette_iterator_ * pirouette_step_size;
...
void ControlManager::asyncControl(void) {
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::asyncControl",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (!failsafe_triggered_) {  // when failsafe is triggered,
                               // updateControllers() and publish() is called in
                               // timerFailsafe()

    // run the safety timer
    // in the case of large control errors, the safety mechanisms will be
    // triggered before the controllers and trackers are updated...
    ...
    if (got_constraints_) {
      ...
      if (enforce && !constraints_being_enforced_) {
        ...
        mrs_lib::set_mutexed(mutex_constraints_, enforce.value(),
                             sanitized_constraints_);
        ...
        RCLCPP_WARN_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "the controller '%s' is enforcing constraints over the "
            "ConstraintManager",
            _controller_names_.at(active_controller_idx).c_str());

      } else if (!enforce && constraints_being_enforced_) {
        RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                             "constraint values returned to original values");

        constraints_being_enforced_ = false;

        mrs_lib::set_mutexed(mutex_constraints_, current_constraints,
                             sanitized_constraints_);
  ...
  // if odometry switch happened, we finish it here and turn the safety timer
  // back on
  if (odometry_switch_in_progress_) {
...

    {
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "odometry after switch: x=%.2f, y=%.2f, z=%.2f, heading=%.2f",
                  uav_state.pose.position.x, uav_state.pose.position.y,
...
void ControlManager::callbackOdometry(
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackOdometry");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackOdometry",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (!validateOdometry(node_, *msg, "callbackOdometry(): msg")) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "incoming 'odometry' contains invalid values, throwing it away");
    return;
  }
  ...
  if (got_uav_state_) {
    if (msg->header.frame_id != uav_state_.header.frame_id) {
      RCLCPP_INFO(node_->get_logger(), "detecting switch of odometry frame");
      {
        ...
        RCLCPP_INFO(
            node_->get_logger(),
            "odometry before switch: x=%.2f, y=%.2f, z=%.2f, heading=%.2f",
            uav_state_.pose.position.x, uav_state_.pose.position.y,
      ...
      while (running_async_control_) {
        RCLCPP_DEBUG(node_->get_logger(),
                     "waiting for control timer to finish");
        clock_->sleep_for(0.001s);
...

      {
        ...
        tracker_list_.at(active_tracker_idx_)
            ->switchOdometrySource(uav_state_odom);
...

  {
    ...
    // transform the twist into the header's frame
    {
      ...
      auto res = transformer_->transformSingle(speed_child_frame,
                                               msg->header.frame_id);
      ...
      if (res) {
        ...
        RCLCPP_ERROR_THROTTLE(
            node_->get_logger(), *clock_, 1000,
            "could not transform the odometry speed from '%s' to '%s'",
    ...
    std::tie(uav_roll_, uav_pitch_, uav_yaw_) =
        mrs_lib::AttitudeConverter(msg->pose.pose.orientation);

    try {
      uav_heading_ =
          mrs_lib::AttitudeConverter(msg->pose.pose.orientation).getHeading();
    } catch (...) {
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "could not calculate UAV heading");
    }
  ...
  if (!running_async_control_) {
    ...
    async_control_result_ =
        std::async(std::launch::async, &ControlManager::asyncControl, this);
  }
}
...
void ControlManager::callbackUavState(
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackUavState");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackUavState",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (!validateUavState(node_, *msg, "callbackUavState(): msg")) {
    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 1000,
        "incoming 'uav_state' contains invalid values, throwing it away");
    return;
  }
  ...
  double uav_state_dt =
      (clock_->now() - previous_uav_state_.header.stamp).seconds();
  ...
  if (uav_state_count_ == uav_state_count_lim) {
    /* RCLCPP_INFO_STREAM(node_->get_logger(), "uav_state_dt = " <<
     * uav_state_dt); */

    if (uav_state_dt < uav_state_avg_dt_ && uav_state_dt > 0.0001) {
      uav_state_hiccup_factor_ =
          alpha2 * uav_state_hiccup_factor_ +
          (1 - alpha2) * (uav_state_avg_dt_ / uav_state_dt);

    } else if (uav_state_avg_dt_ > 0.0001) {
      uav_state_hiccup_factor_ =
          alpha2 * uav_state_hiccup_factor_ +
          (1 - alpha2) * (uav_state_dt / uav_state_avg_dt_);
    }

    if (uav_state_hiccup_factor_ > 3.141592653) {
      /* ROS_ERROR_STREAM_THROTTLE(0.1, "hiccup factor = " <<
       * uav_state_hiccup_factor_); */
      ...
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 2000,
          "// |            UAV_STATE has a large hiccup factor!            |");
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 2000,
          "// |           hint, hint: you are probably rosbagging          |");
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 2000,
          "// |           lot of data or publishing lot of large           |");
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 2000,
          "// |          messages without mutual nodelet managers.         |");
      RCLCPP_WARN_THROTTLE(
          node_->get_logger(), *clock_, 2000,
          "// |                                                            |");
      RCLCPP_WARN_THROTTLE(
...

  {
    ...
    std::tie(uav_roll_, uav_pitch_, uav_yaw_) =
        mrs_lib::AttitudeConverter(uav_state_.pose.orientation);

    try {
      uav_heading_ =
          mrs_lib::AttitudeConverter(uav_state_.pose.orientation).getHeading();
    } catch (...) {
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "could not calculate UAV heading, not updating it");
    }
...
void ControlManager::callbackGNSS(
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackGNSS",
                          scope_timer_logger_, scope_timer_enabled_);
...
void ControlManager::callbackJoystick(
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackJoystick",
                          scope_timer_logger_, scope_timer_enabled_);
...
void ControlManager::callbackHwApiStatus(
    const mrs_msgs::msg::HwApiStatus::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackHwApiStatus");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackHwApiStatus",
                          scope_timer_logger_, scope_timer_enabled_);
...
void ControlManager::callbackRC(
    const mrs_msgs::msg::HwApiRcChannels::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackRC",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  if (_rc_goto_enabled_) {
    if (_rc_joystick_channel_ >= int(rc->channels.size())) {
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "RC joystick activation channel number (%d) is out of range [0-%d]",
          _rc_joystick_channel_, int(rc->channels.size()));

    } else {
      bool channel_low =
          rc->channels.at(_rc_joystick_channel_) < (0.5 - RC_DEADBAND) ? true
                                                                       : false;
      bool channel_high =
          rc->channels.at(_rc_joystick_channel_) > (0.5 + RC_DEADBAND) ? true
                                                                       : false;
      ...
      if (!rc_goto_active_) {
        if (rc_joystick_channel_last_value_ < (0.5 - RC_DEADBAND) &&
            channel_high) {
          if (isFlyingNormally()) {
            RCLCPP_INFO_THROTTLE(node_->get_logger(), *clock_, 1000,
                                 "activating RC joystick");

            callbacks_enabled_ = false;

            /* std::shared_ptr<std_srvs::srv::SetBool::Request> req_goto_out =
             * std::make_shared<std_srvs::srv::SetBool::Request>(); */
            /* req_goto_out->data = false; */

            std::shared_ptr<std_srvs::srv::SetBool::Request>
                req_enable_callbacks =
                    std::make_shared<std_srvs::srv::SetBool::Request>();
            ...
            RCLCPP_WARN_THROTTLE(
                node_->get_logger(), *clock_, 1000,
                "can not activate RC joystick, not flying normally");
          }

        } else if (channel_high && !rc_joystick_channel_was_low_) {
          RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                               "can not activate RC joystick, the switch is ON "
                               "from the beginning");
        }
      }
      ...
      if (channel_high || channel_low) {
        rc_joystick_channel_last_value_ =
            rc->channels.at(_rc_joystick_channel_);
      }
    }
  }
  ...
  if (_rc_escalating_failsafe_enabled_) {
    if (_rc_escalating_failsafe_channel_ >= int(rc->channels.size())) {
      RCLCPP_ERROR_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "RC eland channel number (%d) is out of range [0-%d]",
          _rc_escalating_failsafe_channel_, int(rc->channels.size()));

    } else {
      if (rc->channels.at(_rc_escalating_failsafe_channel_) >=
          _rc_escalating_failsafe_threshold_) {
        RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                             "triggering escalating failsafe by RC");
...
void ControlManager::timeoutUavState(const double& missing_for) {
  ...
  if (output_enabled_ && last_control_output.control_output &&
    ...
    // controllers in place of the callbackUavState/callbackOdometry().

    RCLCPP_ERROR_THROTTLE(
        node_->get_logger(), *clock_, 100,
        "not receiving uav_state/odometry for %.3f s, initiating failsafe land",
        missing_for);
...
bool ControlManager::callbackSwitchTracker(
    const std::shared_ptr<mrs_msgs::srv::String::Request> request,
    const std::shared_ptr<mrs_msgs::srv::String::Response> response) {
  if (!is_initialized_) {
    return false;
...
bool ControlManager::callbackSwitchController(
    const std::shared_ptr<mrs_msgs::srv::String::Request> request,
    const std::shared_ptr<mrs_msgs::srv::String::Response> response) {
  if (!is_initialized_) {
    return false;
...
bool ControlManager::callbackTrackerResetStatic(
...
bool ControlManager::callbackEHover(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackFailsafe(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
  ...
  RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                       "failsafe triggered by callback");
...
bool ControlManager::callbackFailsafeEscalating(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
  ...
  if (_service_escalating_failsafe_enabled_) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "escalating failsafe triggered by callback");
...
bool ControlManager::callbackEland(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackParachute(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
  ...
  RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                       "parachute triggered by callback");
...
bool ControlManager::callbackToggleOutput(
    const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
    const std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
  if (!is_initialized_) {
    return false;
  ...
  if (request->data && (failsafe_triggered_ || eland_triggered_ ||
                        rc_escalating_failsafe_triggered_)) {
    ss << "cannot toggle output ON, we landed in emergency";
    prereq_check = false;
  }

  if (!sh_hw_api_status_.hasMsg() ||
      (clock_->now() - sh_hw_api_status_.lastMsgTime()).seconds() > 1.0) {
...
bool ControlManager::callbackArm(
    const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
    const std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
  if (!is_initialized_) {
    return false;
  ...
  if (failsafe_triggered_ || eland_triggered_) {
    ss << "can not " << (request->data ? "arm" : "disarm")
       << ", eland or failsafe active";
...
bool ControlManager::callbackEnableCallbacks(
    const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
    const std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
  if (!is_initialized_) {
    return false;
...
bool ControlManager::callbackSetConstraints(
    const std::shared_ptr<mrs_msgs::srv::DynamicsConstraintsSrv::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::DynamicsConstraintsSrv::Response>
        response) {
...
bool ControlManager::callbackEmergencyReference(
  ...
  std::shared_ptr<mrs_msgs::srv::ReferenceSrv::Response> tracker_response =
      std::make_shared<mrs_msgs::srv::ReferenceSrv::Response>();
  ...
  auto ret = transformer_->transformSingle(original_reference,
...

  {
    ...
    if (tracker_response != nullptr) {
      ...
      ss << "the tracker '" << _tracker_names_.at(active_tracker_idx_)
         << "' does not implement the 'setReference()' function!";
      response->message = ss.str();
      response->success = false;
    }
  }
...
bool ControlManager::callbackPirouette(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackUseJoystick(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...

  {
    ...
    if (!success) {
      ss << "switching to '" << _joystick_tracker_name_
         << "' was unsuccessfull: '" << tracker_response << "'";
      RCLCPP_ERROR_STREAM(node_->get_logger(), "" << ss.str());
  ...
  auto [success, controller_response] =
      switchController(_joystick_controller_name_);

  if (!success) {
    ss << "switching to '" << _joystick_controller_name_
       << "' was unsuccessfull: '" << controller_response << "'";
    RCLCPP_ERROR_STREAM(node_->get_logger(), "" << ss.str());
...
bool ControlManager::callbackHover(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackStartTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackStopTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackResumeTrajectoryTracking(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackGotoTrajectoryStart(
    [[maybe_unused]] const std::shared_ptr<std_srvs::srv::Trigger::Request>
        request,
    const std::shared_ptr<std_srvs::srv::Trigger::Response> response) {
  if (!is_initialized_) {
...
bool ControlManager::callbackTransformReference(
    const std::shared_ptr<mrs_msgs::srv::TransformReferenceSrv::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::TransformReferenceSrv::Response>
        response) {
  ...
  if (auto ret = transformer_->transformSingle(transformed_reference,
                                               request->frame_id)) {
...
bool ControlManager::callbackTransformReferenceArray(
    const std::shared_ptr<mrs_msgs::srv::TransformReferenceArraySrv::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::TransformReferenceArraySrv::Response>
        response) {
  ...
  const auto tf_opt = transformer_->getTransform(request->array.header.frame_id,
                                                 request->to_frame_id,
                                                 request->array.header.stamp);
  if (!tf_opt.has_value()) {
...
bool ControlManager::callbackTransformPose(
    const std::shared_ptr<mrs_msgs::srv::TransformPoseSrv::Request> request,
  ...
  if (auto ret =
          transformer_->transformSingle(transformed_pose, request->frame_id)) {
...
bool ControlManager::callbackTransformVector3(
    const std::shared_ptr<mrs_msgs::srv::TransformVector3Srv::Request> request,
    const std::shared_ptr<mrs_msgs::srv::TransformVector3Srv::Response>
        response) {
  if (!is_initialized_) {
    return false;
  ...
  if (auto ret = transformer_->transformSingle(transformed_vector3,
                                               request->frame_id)) {
...
bool ControlManager::callbackEnableBumper(
    const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
    const std::shared_ptr<std_srvs::srv::SetBool::Response> response) {
  if (!is_initialized_) {
    return false;
...
bool ControlManager::callbackGetMinZ(
    [[maybe_unused]] const std::shared_ptr<mrs_msgs::srv::GetFloat64::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::GetFloat64::Response> response) {
  if (!is_initialized_) {
    return false;
...
bool ControlManager::callbackValidateReference(
    const std::shared_ptr<mrs_msgs::srv::ValidateReference::Request> request,
    const std::shared_ptr<mrs_msgs::srv::ValidateReference::Response>
        response) {
  if (!is_initialized_) {
  ...
  if (!validateReference(
          node_, request->reference.reference,
          "callbackValidateReference(): request->reference.reference")) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "NaN detected in variable 'request->reference'!!!");
  ...
  if (!ret) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "the reference could not be transformed");
    response->message = "the reference could not be transformed";
    response->success = false;
    return true;
  }
  ...
  if (last_tracker_cmd) {
    ...
    if (!isPathToPointInSafetyArea3d(from_point, transformed_reference)) {
      RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                            "reference validation failed, the path is going "
...
bool ControlManager::callbackValidateReference2d(
    const std::shared_ptr<mrs_msgs::srv::ValidateReference::Request> request,
    const std::shared_ptr<mrs_msgs::srv::ValidateReference::Response>
        response) {
  if (!is_initialized_) {
  ...
  if (!validateReference(
          node_, request->reference.reference,
          "callbackValidateReference2d(): request->reference.reference")) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *clock_, 1000,
                          "NaN detected in variable 'request->reference'!!!");
    response->message = "NaNs/infs in input!";
    response->success = false;
...
bool ControlManager::callbackValidateReferenceArray(
    const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::ValidateReferenceArray::Response>
        response) {
  ...
  // get the transformer
  auto opt_tf = transformer_->getTransform(uav_state.header.frame_id,
                                           request->array.header.frame_id,
                                           request->array.header.stamp);

  if (!opt_tf) {
    RCLCPP_WARN(node_->get_logger(),
                "could not find transform for the reference's frame_id");
  ...
  for (int i = 0; i < int(request->array.array.size()); i++) {
    ...
    if (!validateReference(
            node_, original_reference.reference,
            "callbackValidateReferenceArray(): original_reference.reference")) {
      RCLCPP_WARN(
          node_->get_logger(),
          "the reference for validation ([%d]) contains invalid numbers", i);
    ...
    if (!ret) {
      RCLCPP_WARN(
          node_->get_logger(),
          "the reference for validation ([%d]) could not be transformed", i);
      response->success.at(i) = false;
    }
...
bool ControlManager::callbackReferenceService(
    const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Request> request,
    const std::shared_ptr<mrs_msgs::srv::ReferenceStampedSrv::Response>
        response) {
  if (!is_initialized_) {
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackReferenceService");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackReferenceService",
                          scope_timer_logger_, scope_timer_enabled_);
...
void ControlManager::callbackReferenceTopic(
    const mrs_msgs::msg::ReferenceStamped::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackReferenceTopic");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackReferenceTopic",
                          scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackVelocityReferenceService(
    const std::shared_ptr<mrs_msgs::srv::VelocityReferenceStampedSrv::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::VelocityReferenceStampedSrv::Response>
        response) {
  if (!is_initialized_) {
    response->message = "not initialized";
    response->success = false;
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackVelocityReferenceService");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "ControlManager::callbackVelocityReferenceService",
      scope_timer_logger_, scope_timer_enabled_);
...
void ControlManager::callbackVelocityReferenceTopic(
    const mrs_msgs::msg::VelocityReferenceStamped::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackVelocityReferenceTopic");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "ControlManager::callbackVelocityReferenceTopic",
      scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackTrajectoryReferenceService(
    const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Request>
        request,
    const std::shared_ptr<mrs_msgs::srv::TrajectoryReferenceSrv::Response>
        response) {
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackTrajectoryReferenceService");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "ControlManager::callbackTrajectoryReferenceService",
      scope_timer_logger_, scope_timer_enabled_);

  auto [success, message, modified, tracker_names, tracker_successes,
        tracker_messages] = setTrajectoryReference(request->trajectory);
...
void ControlManager::callbackTrajectoryReferenceTopic(
    const mrs_msgs::msg::TrajectoryReference::ConstSharedPtr msg) {
  if (!is_initialized_) {
    return;
  }
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackTrajectoryReferenceTopic");
  mrs_lib::ScopeTimer timer = mrs_lib::ScopeTimer(
      node_, "ControlManager::callbackTrajectoryReferenceTopic",
      scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackGoto(
    const std::shared_ptr<mrs_msgs::srv::Vec4::Request> request,
    const std::shared_ptr<mrs_msgs::srv::Vec4::Response> response) {
  if (!is_initialized_) {
    response->message = "not initialized";
  ...
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackGoto",
                          scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackGotoFcu(
    const std::shared_ptr<mrs_msgs::srv::Vec4::Request> request,
    const std::shared_ptr<mrs_msgs::srv::Vec4::Response> response) {
  if (!is_initialized_) {
    response->message = "not initialized";
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackGotoFcu");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackGotoFcu",
                          scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackGotoRelative(
    const std::shared_ptr<mrs_msgs::srv::Vec4::Request> request,
    const std::shared_ptr<mrs_msgs::srv::Vec4::Response> response) {
  if (!is_initialized_) {
    response->message = "not initialized";
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackGotoRelative");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackGotoRelative",
                          scope_timer_logger_, scope_timer_enabled_);
  ...
  des_reference.header.frame_id = "";
  des_reference.header.stamp = rclcpp::Time(0, 0, clock_->get_clock_type());
  des_reference.reference.position.x =
      last_tracker_cmd->position.x + request->goal.at(REF_X);
  des_reference.reference.position.y =
      last_tracker_cmd->position.y + request->goal.at(REF_Y);
  des_reference.reference.position.z =
      last_tracker_cmd->position.z + request->goal.at(REF_Z);
  des_reference.reference.heading =
      last_tracker_cmd->heading + request->goal.at(REF_HEADING);
...
bool ControlManager::callbackGotoAltitude(
    const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
    const std::shared_ptr<mrs_msgs::srv::Vec1::Response> response) {
  if (!is_initialized_) {
    response->message = "not initialized";
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackGotoAltitude");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackGotoAltitude",
                          scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackSetHeading(
    const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
    const std::shared_ptr<mrs_msgs::srv::Vec1::Response> response) {
  if (!is_initialized_) {
    response->message = "not initialized";
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackSetHeading");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackSetHeading",
                          scope_timer_logger_, scope_timer_enabled_);
...
bool ControlManager::callbackSetHeadingRelative(
    const std::shared_ptr<mrs_msgs::srv::Vec1::Request> request,
    const std::shared_ptr<mrs_msgs::srv::Vec1::Response> response) {
  if (!is_initialized_) {
    response->message = "not initialized";
  ...
  mrs_lib::Routine profiler_routine =
      profiler_.createRoutine("callbackSetHeadingRelative");
  mrs_lib::ScopeTimer timer =
      mrs_lib::ScopeTimer(node_, "ControlManager::callbackSetHeadingRelative",
                          scope_timer_logger_, scope_timer_enabled_);
...
std::tuple<bool, std::string> ControlManager::setReference(
    const mrs_msgs::msg::ReferenceStamped reference_in) {
  std::stringstream ss;
  ...
  if (!validateReference(node_, reference_in.reference,
                         "setReference(): reference_in.reference")) {
  ...
  // transform the reference to the current frame
  auto ret =
      transformer_->transformSingle(reference_in, uav_state.header.frame_id);
  ...
  // safety area check - automatically correct if outside
  if (!isPointInSafetyArea3d(transformed_reference)) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
  ...
  std::shared_ptr<mrs_msgs::srv::ReferenceSrv::Request> reference_request =
      std::make_shared<mrs_msgs::srv::ReferenceSrv::Request>();
...

  {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "setting reference to x=%.2f, y=%.2f, z=%.2f, hdg=%.2f "
                "(expressed in '%s')",
                transformed_reference.reference.position.x,
                transformed_reference.reference.position.y,
                transformed_reference.reference.position.z,
                transformed_reference.reference.heading,
                transformed_reference.header.frame_id.c_str());

    auto tracker_response =
        tracker_list_.at(active_tracker_idx_)->setReference(reference_request);
    ...
    if (tracker_response != nullptr) {
      ...
      ss << "the tracker '" << _tracker_names_.at(active_tracker_idx_)
         << "' does not implement the 'setReference()' function!";
      RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                   "failed to set the reference: " << ss.str());
      return std::tuple(false, ss.str());
    }
  }
}
...
std::tuple<bool, std::string> ControlManager::setVelocityReference(
    const mrs_msgs::msg::VelocityReferenceStamped& reference_in) {
  std::stringstream ss;
  ...
  if (!validateVelocityReference(
          node_, reference_in.reference,
          "setVelocityReference(): reference_in.reference")) {
  ...
  auto ret = transformer_->getTransform(reference_in.header.frame_id,
                                        uav_state.header.frame_id,
                                        reference_in.header.stamp);

  geometry_msgs::msg::TransformStamped tf;

  if (!ret) {
    ss << "could not find tf from " << reference_in.header.frame_id << " to "
       << uav_state.header.frame_id;
    RCLCPP_WARN_STREAM_THROTTLE(node_->get_logger(), *clock_, 1000,
                                "" << ss.str());
    return std::tuple(false, ss.str());
  ...
  // transform the z and the heading
  {
    ...
    pose.pose.orientation =
        mrs_lib::AttitudeConverter(0, 0, reference_in.reference.heading);
    ...
    if (!ret) {
      ...
      transformed_reference.reference.heading =
          mrs_lib::AttitudeConverter(ret->pose.orientation).getHeading();
    }
  }

  // the heading rate doees not need to be transformed
  transformed_reference.reference.heading_rate =
      reference_in.reference.heading_rate;
  ...
  if (!isPointInSafetyArea3d(eqivalent_reference)) {
    RCLCPP_WARN_THROTTLE(node_->get_logger(), *clock_, 1000,
                         "Velocity reference equivalent point outside safety "
                         "area, correcting to closest valid point");
  ...
  // prepare the message for current tracker
  std::shared_ptr<mrs_msgs::srv::VelocityReferenceSrv::Request>
      reference_request =
          std::make_shared<mrs_msgs::srv::VelocityReferenceSrv::Request>();
  reference_request->reference = transformed_reference.reference;

  {
    std::scoped_lock lock(mutex_tracker_list_);

    auto tracker_response = tracker_list_.at(active_tracker_idx_)
                                ->setVelocityReference(reference_request);
    ...
    if (tracker_response != nullptr) {
      ...
      ss << "the tracker '" << _tracker_names_.at(active_tracker_idx_)
         << "' does not implement the 'setVelocityReference()' function!";
      RCLCPP_ERROR_STREAM_THROTTLE(
          node_->get_logger(), *clock_, 1000,
          "failed to set the velocity reference: " << ss.str());
      return std::tuple(false, ss.str());
    }
  }
}
...
std::tuple<bool, std::string, bool, std::vector<std::string>, std::vector<bool>,
           std::vector<std::string>>
ControlManager::setTrajectoryReference(
    const mrs_msgs::msg::TrajectoryReference trajectory_in) {
  auto uav_state = mrs_lib::get_mutexed(mutex_uav_state_, uav_state_);
  ...
  for (int i = 0; i < int(trajectory_in.points.size()); i++) {
    ...
    bool valid = validateReference(
        node_, trajectory_in.points.at(i),
        "setTrajectoryReference(): trajectory_in.points.at(i),");
...

  {
    ...
    debug_trajectory_out.header.frame_id =
        transformer_->resolveFrame(debug_trajectory_out.header.frame_id);

    if (debug_trajectory_out.header.stamp ==
        rclcpp::Time(0, 0, clock_->get_clock_type())) {
    ...
    for (int i = 0; i < int(trajectory_in.points.size()) - 1; i++) {
      ...
      new_pose.orientation =
          mrs_lib::AttitudeConverter(0, 0, trajectory_in.points.at(i).heading);
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    mrs_uav_managers::control_manager::ControlManager)
