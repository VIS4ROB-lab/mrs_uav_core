...
class PathRandomFlier : public mrs_lib::Node {
 ...
 private:
  ...
  bool checkReference(const std::string frame, const double x, const double y,
  ...
  std::optional<mrs_msgs::msg::TrackerCommand> transformTrackerCmd(
      const mrs_msgs::msg::TrackerCommand& tracker_cmd,
...
PathRandomFlier::PathRandomFlier(rclcpp::NodeOptions options)
    : mrs_lib::Node("path_random_flier", options),
      clock_(this_node().get_clock()) {
  cbkgrp_subs_ = this_node().create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  cbkgrp_ss_ = this_node().create_callback_group(
      rclcpp::CallbackGroupType::MutuallyExclusive);
  ...
  param_loader.loadParam("override_constraints/enabled",
                         _override_constraints_);
  param_loader.loadParam("override_constraints/speed", _override_speed_);
  param_loader.loadParam("override_constraints/acceleration",
                         _override_acceleration_);
  ...
  sh_tracker_cmd_ = mrs_lib::SubscriberHandler<mrs_msgs::msg::TrackerCommand>(
      shopts, "~/tracker_command_in");
  sh_control_manager_diag_ =
      mrs_lib::SubscriberHandler<mrs_msgs::msg::ControlManagerDiagnostics>(
          shopts, "~/control_manager_diag_in");

  // | --------------------- service clients -------------------- |

  rclcpp::Node::SharedPtr node = this_node_ptr();
  sc_path_ = mrs_lib::ServiceClientHandler<mrs_msgs::srv::PathSrv>(
      node, "~/path_out", cbkgrp_sc_);
  sc_check_reference_ =
      mrs_lib::ServiceClientHandler<mrs_msgs::srv::ValidateReference>(
          node, "~/check_reference_out", cbkgrp_sc_);

  // | --------------------- service servers -------------------- |

  ss_activate_ = mrs_lib::ServiceServerHandler<std_srvs::srv::Trigger>(
      node, "~/activate_in",
      std::bind(&PathRandomFlier::callbackActivate, this, std::placeholders::_1,
                std::placeholders::_2),
      cbkgrp_ss_);
...

  {
    std::function<void()> callback_fcn =
        std::bind(&PathRandomFlier::timerMain, this);
...
bool PathRandomFlier::callbackActivate(
...
void PathRandomFlier::timerMain(void) {
  ...
  if (!sh_tracker_cmd_.hasMsg()) {
    RCLCPP_INFO_THROTTLE(this_node().get_logger(), *clock_, 1000,
                         "waiting for tracker command");
    return;
  }

  if (!sh_tracker_cmd_.getMsg()->use_full_state_prediction) {
    RCLCPP_INFO_THROTTLE(this_node().get_logger(), *clock_, 1000,
                         "waiting for MPC prediction");
    return;
  }

  if (!sh_control_manager_diag_.hasMsg()) {
    RCLCPP_INFO_THROTTLE(this_node().get_logger(), *clock_, 1000,
                         "waiting for control manager diagnostics");
    return;
  }

  bool has_goal = sh_control_manager_diag_.getMsg()->tracker_status.have_goal;

  auto tracker_cmd_transformed = transformTrackerCmd(
      *sh_tracker_cmd_.getMsg(), _uav_name_ + "/" + _frame_id_);
  ...
  auto [cmd_x, cmd_y, cmd_z] =
      mrs_lib::getPosition(tracker_cmd_transformed.value());

  // if the uav reached the previousy set destination
  if ((clock_->now() - last_successfull_command_).seconds() > 1.0 &&
      (!has_goal || (!next_wait_for_finish_ &&
                     (next_replan_time_ - clock_->now()).seconds() < 0))) {
    ...
    if (!next_wait_for_finish_) {
      ...
      if (has_goal) {
        path.header.stamp =
            clock_->now() + std::chrono::duration<double>(time_offset);
      } else {
    ...
    RCLCPP_INFO(this_node().get_logger(),
                "pos_x: %.2f, pos_y: %.2f, pos_z: %.2f", pos_x, pos_y, pos_z);

    int n_points = randi(_n_points_min_, _n_points_max_);

    for (int it = 0; it < n_points; it++) {
      RCLCPP_INFO(this_node().get_logger(),
                  "check pos_x: %.2f, pos_y: %.2f, pos_z: %.2f", pos_x, pos_y,
                  pos_z);
      if (!checkReference(_uav_name_ + "/" + _frame_id_, pos_x, pos_y, pos_z,
                          bearing_)) {
        break;
      }
    ...
    next_wait_for_finish_ =
        randd(0, 10) <= 10 * _future_stamp_prob_ ? false : true;

    if (!next_wait_for_finish_) {
      double replan_time = randd(_replanning_time_min_, _replanning_time_max_);
      next_replan_time_ =
          clock_->now() + std::chrono::duration<double>(replan_time);
      RCLCPP_INFO(this_node().get_logger(), "replanning in %.2f s",
                  replan_time);
    }
    ...
    if (_override_constraints_) {
      ...
      RCLCPP_INFO_THROTTLE(
          ...
          "overriding constraints to speed: %.2f m/s, acc: %.2f m/s2",
          path.override_max_velocity_horizontal,
...
bool PathRandomFlier::setPathSrv(const mrs_msgs::msg::Path path_in) {
  std::shared_ptr<mrs_msgs::srv::PathSrv::Request> request =
      std::make_shared<mrs_msgs::srv::PathSrv::Request>();
  ...
  if (response) {
    if (!response.value()->success) {
      RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                            "service call for setting path failed: %s",
...
bool PathRandomFlier::checkReference(const std::string frame, const double x,
  ...
  if (response) {
    ...
    RCLCPP_ERROR_THROTTLE(this_node().get_logger(), *clock_, 1000,
                          "service call for reference validation");
    return false;
  }
}
...
PathRandomFlier::transformTrackerCmd(
    const mrs_msgs::msg::TrackerCommand& tracker_cmd,
    const std::string& target_frame) {
  // if we transform to the current control frame, which is in fact the same
  // frame as the tracker_cmd is in
  if (target_frame == "") {
    return tracker_cmd;
  }

  // find the transformation
  auto tf = transformer_->getTransform(tracker_cmd.header.frame_id,
                                       target_frame, tracker_cmd.header.stamp);

  if (!tf) {
    RCLCPP_ERROR(this_node().get_logger(),
                 "could not find transform from '%s' to '%s' in time %f",
                 tracker_cmd.header.frame_id.c_str(), target_frame.c_str(),
...

  {
    ...
    pos.pose.orientation =
        mrs_lib::AttitudeConverter(0, 0, tracker_cmd.heading);

    if (auto ret = transformer_->transform(pos, tf.value())) {
      cmd_out.position = ret.value().pose.position;
      try {
        cmd_out.heading =
            mrs_lib::AttitudeConverter(ret.value().pose.orientation)
                .getHeading();
      } catch (...) {
        RCLCPP_ERROR(this_node().get_logger(),
                     "failed to transform heading in tracker_cmd");
        cmd_out.heading = 0;
      }
    } else {
      return {};
